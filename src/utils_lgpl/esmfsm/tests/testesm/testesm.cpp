...
int main(int argc, char* argv[], char* envp[])
{
    ...
    for (int i = 0; i < NUMCELLS; i++) cellsize[i] = 0;
    ...
    for (int i = 0; i < NUMCELLS; i++) Validate(i, cellsize[i]);
    ...
    for (int i = 0; i < NUMCELLS; i++) Free(i);
...
static void WorkerProcess(int nprocs, int procid)
{
    ...
    for (int round = 0; round < NUMROUNDS; round++)
    {
        ...
        while ((*baton % nprocs) != procid) usleep(SLEEP);
...
static void Allocate(int number, int size)
{
    char name[20];
    ...
    int* integer = (int*)ptr;
    for (int i = 0; i < size; i++) integer[i] = 100 * number + size;

    //// printf ("Allocated (\"%s\", %d)\n", name, size);
}

static void Validate(int number, int size)
{
    char name[20];
    sprintf(name, "Cell-%04d", number);
