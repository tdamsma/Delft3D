...
#define LOOKUP_CONTEXT(C, I, T, F, E)                                                        \
    {                                                                                        \
        ...
        {                                                                                    \
            ...
            {                                                                                \
                if (Contexts[I].contextid == (C)) break;                                     \
            }                                                                                \
...
static void SetError(int thid, const char* message, ...);

static void ClearError(int thid);

static pointer InsertKey(KeyTable* ktab, char* name, int length, int type, int eltsize, pointer ptr);

static int LookupKey(KeyTable* ktab, char* name);

static int Hash(char* name);

static void CstrToFortran(char* cstr, int len, char* fstr);

//--------------------------------------------------------------------------
//  Functions in FSM API

int STDCALL FSM_Init(int* contextid, int* flags)
{
    int cid = *contextid;
...
pointer STDCALL FSM_MakePointer(
#if defined(WIN32)
    char* name,  // key name
...
)
{
    ...
    FSM_DEBUG((output, "Entering FSM_MakePointer (name=\"%s\", length=%d, type=%d, eltsize=%d)", name, *length, *type,
               *eltsize))
    ClearError(thid);
    ...
    if ((kindex = LookupKey(FSM.Threads[thid].keytable, name)) >= 0)
    {
        ...
        {
            SetError(thid, "Trying to resize (%d != %d) existing key \"%s\" in FSM_MakePointer", *length, key->length,
                     name);
            UNLOCK(NULL)
            RETURN(NULL)
        }
        ...
        if (*type != key->type)
        {
            SetError(thid, "Incompatible type (%d != %d) existing key \"%s\" in FSM_MakePointer", *type, key->type,
                     name);
            UNLOCK(NULL)
            RETURN(NULL)
        }
...
return_label: {
    FSM_DEBUG((output, "Returning %ld from FSM_MakePointer (name=\"%s\", length=%d, type=%d, eltsize=%d)",
               (long)return_value, name, *length, *type, *eltsize))
    return return_value;
}
}
...
pointer STDCALL FSM_GetPointer(
#if defined(WIN32)
    char* name, int* type
#else
    char* name, int* type
...
pointer STDCALL FSM_ReleasePointer(
#if !defined(WIN32)
    char* name, int* type
#else
    char* name, int* type
    ...
    ktab = FSM.Threads[thid].keytable;
    {
        for (int i = 0; i < FSM_MAX_REGIONS; i++)
        {
            if (ktab[i].key[0] != 0 && ktab[i].length > 0)
            {
                ...
                printf(fmt, ktab[i].key, fsm_type_name[ktab[i].type], ktab[i].length, (pointer)FSM.Pointer[i],
...
static void InitProcess(void)
{
    ...
    for (int i = 0; i < FSM_MAX_REGIONS; i++) FSM.Pointer[i] = NULL;
...
static pointer InsertKey(KeyTable* ktab, char* name, int length, int type, int eltsize, pointer ptr)
{
    ...
    {
        if (ktab[i].key[0] == 0 || ktab[i].key[0] == (char)-1)
        { // free slot
            // free slot
    ...
    if (n == FSM_MAX_REGIONS) return NULL;
