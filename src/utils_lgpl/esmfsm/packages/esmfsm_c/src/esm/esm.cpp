...
return_label: {
    ESM_DEBUG((output, "Returning %ld from ESM_Alloc (contextid=%d, name=\"%s\", size=%d)", (long)return_value,
               contextid, name, size))
    UNLOCK(NULL)
    return return_value;
}
...
const char* ESM_Error(void)
{
    ...
    if ((thidp = (int*)pthread_getspecific(Self)) == NULL) return "Cannot get thread ID in ESM_Error";
