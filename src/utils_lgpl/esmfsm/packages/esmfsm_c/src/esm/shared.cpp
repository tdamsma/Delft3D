...
//  given a page base address (B) in the local address space
    ...
    #define LOCALADDRESS(B, A) ((void*)((char*)(B) + (long)(A)))

//------------------------------------------------------------------------------
//  Internal function declarations

static int AttachContext(int thid, int contextid);

static void PrintContextSegmentInfo(struct shmid_ds* segment, CD* cd, FILE* output);

static int GetRegionDescriptor(RD regions[], int* freehead);

static void PutRegionDescriptor(RD regions[], int* freehead, int index);

static int SearchForRegion(RD regions[], int* head, const char* name);

static int SearchForHole(RD regions[], int* head, int size);

static void InsertRegion(RD regions[], int* listhead, int newreg);

static void InsertHole(RD regions[], int* listhead, int newreg);

static void UnlinkElement(RD regions[], int* head, int index);
...
int ESM_Shared_Create(int thid, int pagesize)
{
    ...
    if (contextid == 0)
    {
        ...
        if (newcontextid < 0)
        {
            SetError(thid, "Cannot create second non-zero context descriptor segment in ESM_Create: %s",
                     strerror(errno));
            return 0;
        }
...
int ESM_Shared_Delete(int thid, int ci, int contextid)
{
    ...
    {
        SetError(thid, "Cannot remove semaphore (%d) associated with context descriptor (%d): %s", cd->mutex, contextid,
                 strerror(errno));
        return ESM_ERROR;
    }
...
void* ESM_Shared_Alloc(int thid, int ci, int contextid, const char* name, size_t size)
{
    ...
    void* addr = LOCALADDRESS(Contexts[ci].pageaddr[cd->region[newreg].pagenum], cd->region[newreg].offset);
...
int ESM_Shared_Free(int thid, int ci, int contextid, const char* name)
{
    ...
    {
        if (cd->region[r].pagenum != cd->region[reg].pagenum) continue;
        if (cd->region[reg].offset + cd->region[reg].size != cd->region[r].offset) continue;
    ...
    {
        if (cd->region[r].pagenum != cd->region[reg].pagenum) continue;
        if (cd->region[r].offset + cd->region[r].size != cd->region[reg].offset) continue;
...
static int GetRegionDescriptor(RD regions[], int* freehead)
{
    ...
    int reg = *freehead;
    if (reg == GND) return GND; // no more empty descriptors

    *freehead = regions[reg].next;
    if (*freehead != GND) regions[*freehead].prev = GND;
...
static int SearchForRegion(RD regions[], int* head, const char* name)
{
    ...
    while (reg != GND)
    {
        if (strcmp(regions[reg].name, name) == 0) break;
...
static int SearchForHole(RD regions[], int* head, int size)
{
    ...
    while (reg != GND)
    {
        if (regions[reg].size >= size) break;
...
static void InsertRegion(RD regions[], int* listhead, int newreg)
{
    ...
    while (reg != GND)
    {
        if (strcmp(regions[reg].name, regions[newreg].name) >= 0)
        { // found insertion place
            regions[newreg].next = reg;
            if (regions[reg].prev == GND)
            { // insert before 1st element of list
                regions[newreg].prev = GND;
                *listhead = newreg;
            }
            else
...
static void InsertHole(RD regions[], int* listhead, int newreg)
{
    ...
    while (reg != GND)
    {
        if (regions[reg].size >= regions[newreg].size)
        { // found insertion place
            regions[newreg].next = reg;
            if (regions[reg].prev == GND)
            { // insert before 1st element of list
...
static void UnlinkElement(RD regions[], int* head, int index)
{
    ...
    if (regions[index].next != GND) regions[regions[index].next].prev = regions[index].prev;
...
int ESM_Shared_Info(FILE* output)
{
    ...
    for (int id = 0; id <= maxid; id++)
    {
        ...
        if (cd->magic == ESM_MAGIC) PrintContextSegmentInfo(&segment, cd, output);
...
static void PrintContextSegmentInfo(struct shmid_ds* segment, CD* cd, FILE* output)
{
    ...
    {
        fprintf(output, "\t\tname=\"%s\", size=%d, pagenum=%d, offset=%d\n", cd->region[reg].name, cd->region[reg].size,
                cd->region[reg].pagenum, cd->region[reg].offset);
    }
