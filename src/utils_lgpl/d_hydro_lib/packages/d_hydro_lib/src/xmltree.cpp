...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/utils_lgpl/d_hydro_lib/packages/d_hydro_lib/src/xmltree.cpp
// $
//------------------------------------------------------------------------------
...
XmlTree::XmlTree(XmlTree* parent, const char* name)
{
    ...
    if (pathlen > this->maxPathname) throw new Exception("XML pathname for node \"%s\" is too long", name);
...
{
    if (this->numAttrib >= this->maxAttrib) throw new Exception("XML node %s has too many attributes", this->name);
...
void XmlTree::AddChild(XmlTree* child)
{
    if (this->numChildren >= this->maxChildren) throw new Exception("XML node %s has too many children", this->name);
...
void XmlTree::print(int level)
{
    ...
    for (int i = 0; i < this->numAttrib; i++) printf("%s=%s ", this->attribNames[i], this->attribValues[i]);

    printf("]\n");

    for (int i = 0; i < this->numChildren; i++) this->children[i]->print(level + 1);
}
