...
int main(int argc, char* argv[], char* envp[])
{
    ...
    if (provided != required) Abort("MPI does not support threading the way we need it to");
...
void* Leader(void* argument)
{
    ...
    MPI_Send(Global.leader[id].lochand, strlen(Global.leader[id].lochand), MPI_CHAR, Global.otherside, LEADER_HANDLE,
             MPI_COMM_WORLD);
...
void* Follower(void* argument)
{
    ...
    MPI_Send(Global.follower[id].lochand, strlen(Global.follower[id].lochand), MPI_CHAR, Global.otherside,
             CONNECT_TAG + seqn, MPI_COMM_WORLD);
...
void Log(int level, char* reason, ...)
{
