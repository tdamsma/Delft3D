...
int main()
{
    ...
    if (error == 0)
    {
        ...
        four = 4;
        error = Defelm(&fd_nefis, elm_name, elm_type, four, elm_quantity, elm_unity, elm_desc, elm_num_dim, elm_dimens);
    }
    ...
    if (error == 0)
    {
        error = Inqfel(&fd_nefis, elm_name, elm_type, elm_quantity, elm_unity, elm_desc, &elm_single_bytes, &elm_bytes,
    ...
    {
        i++;
        error = Inqnel(&fd_nefis, elm_name, elm_type, elm_quantity, elm_unity, elm_desc, &elm_single_bytes, &elm_bytes,
                       &elm_num_dim, elm_dimens);
        if (error == 0)
        {
    ...
    if (error == 0)
    {
        error = Inqfcl2(&fd_nefis, cel_name, &cel_num_dim, &cel_num_bytes, &elm_names);
    ...
    while (error == 0)
    {
        i++;
        cel_num_dim = 100;
        error = Inqncl2(&fd_nefis, cel_name, &cel_num_dim, &cel_num_bytes, &elm_names);
        if (error == 0)
        {
    ...
    {
        grp_num_dim = 5;
        error = Inqfgr(&fd_nefis, grp_name, cel_name, &grp_num_dim, grp_dimens, grp_order);
        if (error == 0)
        {
    ...
    while (error == 0)
    {
        i++;
        grp_num_dim = 5;
        error = Inqngr(&fd_nefis, grp_name, cel_name, &grp_num_dim, grp_dimens, grp_order);
        if (error == 0)
        {
