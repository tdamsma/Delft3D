...
int main()
{
    ...
    if (error == 0)
    {
        ...
        elm_name = strdup("S1");
        error = Getelt(&fd_nefis, grp_name, elm_name, (BUInt4*)usr_index, usr_order, &buf_len, (BData)zeta);
    ...
    for (i = 0; i < 6; i++)
    {
        if (error == 0)
        {
            ...
            error = Putelt(&fd_nefis, grp_name, elm_name, (BInt4*)usr_index, usr_order, (BData)getal);
        }
    }
    ...
    for (i = 0; i < 2; i++)
    {
        if (error == 0)
        {
                        ...
                        for (jj = 0; jj < 4; jj++)
                        {
                            for (kk = 0; kk < 6; kk++)
                            {
                                zeta[kk + 6 * jj + 6 * 4 * ll + 6 * 4 * 3 * mm + 6 * 4 * 3 * 5 * nn] =
                                    (BRea4)(100 * (nn + 1) + 1000 * (mm + 1) + 10000 * (ll + 1) + 10 * (jj + 1) + kk +
                                            1);
                            }
                        }
                    }
            ...
            */
            ...
            error = Putelt(&fd_nefis, grp_name, elm_name, (BInt4*)usr_index, usr_order, (BData)zeta);
        }
    }
    ...
    for (i = 0; i < 13; i++)
    {
        if (error == 0)
        {
            ...
            error = Putelt(&fd_nefis, grp_name, elm_name, (BInt4*)usr_index, usr_order, (BData)namst);
        }
    }
    ...
    for (i = 0; i < 2; i++)
    {
        if (error == 0)
        {
            ...
            error = Getelt(&fd_nefis, grp_name, elm_name, (BInt4*)usr_index, usr_order, &buf_len, (BData)getallen);
    ...
    for (i = 0; i < 2; i++)
    {
        if (error == 0)
        {
            ...
            error = Getelt(&fd_nefis, grp_name, elm_name, (BInt4*)usr_index, usr_order, &buf_len, (BData)zetaa);
    ...
    for (i = 5; i < 7; i++)
    {
        if (error == 0)
        {
            ...
            error = Getelt(&fd_nefis, grp_name, elm_name, (BInt4*)usr_index, usr_order, &buf_len, (BData)namstat);
