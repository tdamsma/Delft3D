...
int main()
{
    ...
    if (error == 0)
    {
        ...
        error = Defelm(&fd_nefisa, elm_name, elm_type, elm_single_byte, elm_quantity, elm_unity, elm_desc, elm_num_dim,
                       elm_dimens);
        error = Defelm(&fd_nefisb, elm_name, elm_type, elm_single_byte, elm_quantity, elm_unity, elm_desc, elm_num_dim,
                       elm_dimens);
    }
    ...
    if (error == 0)
    {
        ...
        error = Defgrp(&fd_nefisa, grp_name, cel_names, grp_num_dim, grp_dimens, grp_order);
        error = Defgrp(&fd_nefisb, grp_name, cel_names, grp_num_dim, grp_dimens, grp_order);
    }
    ...
    {
        printf(" Define first element \n");
        error = Defelm(&fd_nefisc, elm_name, elm_type, elm_single_byte, elm_quantity, elm_unity, elm_desc, elm_num_dim,
                       elm_dimens);
    }
    ...
    {
        printf(" Define first group   \n");
        error = Defgrp(&fd_nefisc, grp_name, cel_names, grp_num_dim, grp_dimens, grp_order);
    }
    ...
    for (cyclus = 0; cyclus < cyclus_max; cyclus++)
    {
        if (error == 0)
        {
                        ...
                        for (jj = 0; jj < 4; jj++)
                        {
                            for (kk = 0; kk < 6; kk++)
                            {
                                zeta[kk + 6 * jj + 6 * 4 * ll + 6 * 4 * 3 * mm + 6 * 4 * 3 * 5 * nn] =
                                    (BRea4)(100000 * (nn + 1) + 1000 * (mm + 1) + 100 * (ll + 1) + 10 * (jj + 1) + kk +
                                            1);
                            }
                        }
                    }
            ...
            error = Putelt(&fd_nefisb, grp_name, elm_name, (BInt4*)usr_index, usr_order, (BVoid*)zeta);
            ...
            error = Getelt(&fd_nefisb, grp_name, elm_name, (BUInt4*)usr_index, usr_order, &buf_len, (BData)zetaa);
