...
int main()
{
    ...
    for (i = 0; i < 1601; i++)
    {
        ...
        error = Putelt(&fd_nefis, grp_name, elm_name, (BInt4*)usr_index, usr_order, getal);
    }
    if (error != 0)
    {
        error = Neferr(1, error_string);
    ...
    for (i = 0; i < 250; i++)
    {
        ...
        usr_order[0] = 1;
        j = 60;
        error = Getelt(&fd_nefis, grp_name, elm_name, (BInt4*)usr_index, usr_order, &j, (BData)getal);
        for (j = 0; j < 15; j++)
        {
