...
BInt4 create_nefis_files(BInt4* fd_nefis, BText dat_file, BText def_file, BChar coding, BChar access_type)
{
    ...
    *fd_nefis = -1;
    ...
    for (i = 0; i < nr_nefis_files; i++)
    {
        if (nefis[i].exist != -1)
        {
            if ((strcmp(nefis[i].dat_name, dat_file) == 0) && (strcmp(nefis[i].def_name, "") != 0))
            {
                nefis_errcnt += 1;
                nefis_errno = 8001;
                sprintf(error_text, "Data file \'%s\' has already been opened", nefis[i].dat_name);
                return nefis_errno;
            }

            if ((strcmp(nefis[i].dat_name, "") != 0) && (strcmp(nefis[i].def_name, def_file) == 0))
            {
                nefis_errcnt += 1;
                nefis_errno = 8002;
                sprintf(error_text, "Definition file \'%s\' has already been opened", nefis[i].def_name);
                return nefis_errno;
            }
        }
    }
    ...
    for (i = 0; i < nr_nefis_files; i++)
    {
        ...
        if ((strcmp(nefis[i].dat_name, "") == 0) && (strcmp(nefis[i].def_name, "") != 0))
        {
            set = i;
            break;
        }
    ...
    if (set >= MAX_NEFIS_FILES)
    {
        nefis_errcnt += 1;
        nefis_errno = 8003;
        sprintf(error_text, "Maximum number (=%d) of open files has been achieved", MAX_NEFIS_FILES);
        return nefis_errno;
    }
    ...
    if ((BInt4)strlen(dat_file) != 0 && nefis[set].one_file == FALSE)
    {
        ...
        if (new_files == TRUE)
        {
            ...
            if (n_written != LHDRDT)
            {
                nefis_errcnt += 1;
                nefis_errno = 8004;
                sprintf(error_text, "Header was not written to data file \'%s\'", nefis[set].dat_name);
                return nefis_errno;
            }
            /*
            ...
            if (n_written != SIZE_BINT8 * (LHSHDT + 1))
            {
                nefis_errcnt += 1;
                nefis_errno = 8005;
                sprintf(error_text, "Hashtable was not written to data file \'%s\'", nefis[set].dat_name);
            }
            if (nefis[set].dat_neutral == TRUE)
            {
                free((void*)cp);
        ...
        else
        {
            ...
            if (n_read == -1)
            {
                nefis_errcnt += 1;
                nefis_errno = 8007;
                sprintf(error_text, "Unable to access data file \'%s\'", nefis[set].dat_name);
                return nefis_errno;
            }
            if (n_read > LHDRDT)
            {
                nefis_errcnt += 1;
                nefis_errno = 8008;
                sprintf(error_text, "During reading header of data file \'%s\'", nefis[set].dat_name);
                return nefis_errno;
            }
            dathdr[LHDRDT] = '\0';
            /*
            ...
            if (nefis[set].one_file == FALSE)
            {
                if (p_str == NULL)
                {
                    nefis_errcnt += 1;
                    nefis_errno = 8009;
                    sprintf(error_text, "File \'%s\' is not a NEFIS data file", nefis[set].dat_name);
                ...
                if (nefis_errno != 0)
                {
                    nefis_errcnt += 1;
                    nefis_errno = 8032;
                    sprintf(error_text, "Data File \'%s\' has unregcognized version number", nefis[set].dat_name);
                    return nefis_errno;
                }
                 ...
                 */
                n_read = GP_read_file_pointers(dat_fds, &nefis[set].dat.str[SIZE_BINT8], LHDRDT,
                                               SIZE_BINT8 * (LHSHDT + 1), set);

                if (n_read > SIZE_BINT8 * (LHSHDT + 1))
                {
                    nefis_errcnt += 1;
                    nefis_errno = 8010;
                    sprintf(error_text, "During reading hashtable of data file  \'%s\'", nefis[set].dat_name);
                    return nefis_errno;
                }
    ...
    if ((BInt4)strlen(def_file) != 0 && nefis[set].one_file == FALSE)
    {
        ...
        if (new_files == TRUE)
        {
            ...
            if (n_written != LHDRDF)
            {
                nefis_errcnt += 1;
                nefis_errno = 8011;
                sprintf(error_text, "Unable to write header of definition file \'%s\'", nefis[set].def_name);
                return nefis_errno;
            }
            ...
            if (n_written != SIZE_BINT8 * (LHSHDF + 1))
            {
                nefis_errcnt += 1;
                nefis_errno = 8012;
                sprintf(error_text, "Unable to write hashtable of definition file \'%s\'", nefis[set].def_name);
            }
            if (nefis[set].def_neutral == TRUE)
            {
                free((void*)cp);
        ...
        else
        {
            ...
            if (n_read == -1)
            {
                nefis_errcnt += 1;
                nefis_errno = 8014;
                sprintf(error_text, "Unable to access definition file \'%s\'", nefis[set].def_name);
                return nefis_errno;
            }
            if (n_read > LHDRDF)
            {
                nefis_errcnt += 1;
                nefis_errno = 8015;
                sprintf(error_text, "During reading header of definition file \'%s\'", nefis[set].def_name);
                return nefis_errno;
            }
            defhdr[LHDRDF] = '\0';
            /*
            ...
            if (nefis[set].one_file == FALSE)
            {
                if (p_str == NULL)
                {
                    nefis_errcnt += 1;
                    nefis_errno = 8016;
                    sprintf(error_text, "File \'%s\' is not a NEFIS definition file", nefis[set].def_name);
                ...
                if (nefis_errno != 0)
                {
                    nefis_errcnt += 1;
                    nefis_errno = 8033;
                    sprintf(error_text, "Definition File \'%s\' has unregcognized version number", nefis[set].def_name);
                    return nefis_errno;
                }
                 ...
                 */
                n_read = GP_read_file_pointers(def_fds, &nefis[set].def.str[SIZE_BINT8], LHDRDF,
                                               SIZE_BINT8 * (LHSHDF + 1), set);

                if (n_read > SIZE_BINT8 * (LHSHDF + 1))
                {
                    nefis_errcnt += 1;
                    nefis_errno = 8017;
                    sprintf(error_text, "On reading hashtable of definition file \'%s\'", nefis[set].def_name);
                    return nefis_errno;
                }
    ...
    if (nefis[set].one_file == TRUE)
    {
        ...
        if (new_files == TRUE)
        {
            ...
            if (n_written != LHDRDAF)
            {
                nefis_errcnt += 1;
                nefis_errno = 8018;
                sprintf(error_text, "Unable to write header of DefinitionData file \'%s\'", nefis[set].daf_name);
                return nefis_errno;
            }
            ...
            if (n_written != SIZE_BINT8 * (LHSHDAF + 1))
            {
                nefis_errcnt += 1;
                nefis_errno = 8019;
                sprintf(error_text, "Unable to write hashtable of DefinitionData file \'%s\'", nefis[set].daf_name);
            }
            if (nefis[set].daf_neutral == TRUE)
            {
                free((void*)cp);
        ...
        else
        {
            ...
            if (n_read == -1)
            {
                nefis_errcnt += 1;
                nefis_errno = 8021;
                sprintf(error_text, "Unable to access DefinitionData file \'%s\'", nefis[set].daf_name);
                return nefis_errno;
            }
            if (n_read > LHDRDAF)
            {
                nefis_errcnt += 1;
                nefis_errno = 8022;
                sprintf(error_text, "During reading header of DefinitionData file \'%s\'", nefis[set].daf_name);
                return nefis_errno;
            }
            ...
            if (p_str == NULL)
            {
                nefis_errcnt += 1;
                nefis_errno = 8023;
                sprintf(error_text, "File \'%s\' is not a NEFIS DefinitionData file", nefis[set].daf_name);
            ...
            if (nefis_errno != 0)
            {
                nefis_errcnt += 1;
                nefis_errno = 8034;
                sprintf(error_text, "DefinitionData File \'%s\' has unregcognized version number", nefis[set].daf_name);
                strcpy(nefis[set].daf_name, "");
                return nefis_errno;
            }
             ...
             */
            n_read = GP_read_file_pointers(daf_fds, &nefis[set].daf.str[SIZE_BINT8], LHDRDAF,
            ...
            if (n_read > SIZE_BINT8 * (LHSHDAF + 1))
            {
                nefis_errcnt += 1;
                nefis_errno = 8024;
                sprintf(error_text, "On reading hashtable of DefinitionData file \'%s\'", nefis[set].daf_name);
                return nefis_errno;
            }
...
BInt4 close_nefis_files(BInt4* fd_nefis)
{
    ...
    if ((nefis[set].dat_fds == -1) && (nefis[set].def_fds == -1) && (nefis[set].daf_fds == -1))
    {
        *fd_nefis = -1;
        nefis[set].exist = -1;
    ...
    if (nefis[set].one_file == FALSE)
    {
        if (nefis[set].dat_fds != -1)
        {
            ...
            if (error != 0)
            {
                nefis_errcnt += 1;
                nefis_errno = 8025;
                sprintf(error_text, "Unable to close data file \'%s\'", nefis[set].dat_name);
                return nefis_errno;
            }
        }
        ...
        if (nefis[set].def_fds != -1)
        {
            ...
            if (error != 0)
            {
                nefis_errcnt += 1;
                nefis_errno = 8026;
                sprintf(error_text, "Unable to close definition file \'%s\'", nefis[set].def_name);
                return nefis_errno;
            }
        }
    }
    else if (nefis[set].one_file == TRUE && nefis[set].daf_fds != -1)
    {
        ...
        if (error != 0)
        {
            nefis_errcnt += 1;
            nefis_errno = 8027;
            sprintf(error_text, "Unable to close DefinitionData file \'%s\'", nefis[set].def_name);
            return nefis_errno;
        }
    }
    /*
    ...
    if (clean_up == TRUE && (nefis[set].dat_fds != -1 || nefis[set].daf_fds != -1))
    {
        for (i = 0; i < cap_nefis_files; i++)
        {
            for (j = 0; j < MAX_VAR_GROUPS; j++)
...
BInt4 open_nefis_file(BText file_name, BChar access_type, BInt4* p_fds, BInt4* new_files)
/*
 ...
 */
{
     ...
     */
    if ((access_type != 'c') && (access_type != 'u') && (access_type != 'r'))
    {
        nefis_errcnt += 1;
        nefis_errno = 8029;
        sprintf(error_text, "File \'%s\' can not be opened with unsupported NEFIS access type \'%c\'", file_name,
                access_type);
        return nefis_errno;
    }
    ...
    if (fds == -1)
    {
        ...
        else
        {
            ...
            *new_files = FALSE;
            nefis_errcnt += 1;
            nefis_errno = 8031;
            sprintf(error_text, "File \'%s\' can not be opened as read only\n --- %s", file_name, strerror(errno));
        }
    }
    else
    {
        ...
        else
        {
            nefis_errcnt += 1;
            nefis_errno = 8029;
            sprintf(error_text, "File \'%s\' cann't be opened with unsupported NEFIS access type \'%c\'\n --- %s",
    ...
    if (nefis_errno == 0)
    {
        ...
        if (fds == -1)
        {
            nefis_errcnt += 1;
            nefis_errno = 8030;
            sprintf(error_text, "Cannot open file \'%s\' for access type \'%c\'\n --- %s", file_name, access_type,
                    strerror(errno));
        }
    }
 ...
 */
BInt4 DBG_nefis_tree(BInt4 in_out, BText func_name)
{
    BInt4 i;
...
BInt4 setConversionNewFile(BChar coding, BText hdr, BUInt4 length)
{
    ...
    {
        hdr[length - 1] = coding;
        if (endianness() == 0 && coding == 'B' || endianness() == 1 && coding == 'L')
        {
            retval = TRUE; /* convert */
        }
        else if (endianness() == 0 && coding == 'L' || endianness() == 1 && coding == 'B')
        {
            retval = FALSE;
        }
    }
...
BInt4 setConversionExistingFile(BInt4 set, BChar coding, BText hdr, BUInt4 length)
{
    ...
    if (nefis[set].file_version == Version_5)
    {
        ...
        {
            if (endianness() == 0 &&
                hdr[length - 1] == 'L') /* Neutral == Little endian architecture and Little endian file */
            {
                retval = FALSE;
            }
            else if (endianness() == 1 &&
                     hdr[length - 1] == 'L') /* Neutral == Big endian architecture and Little endian file */
            {
                retval = TRUE; /* convert */
            }
            if (endianness() == 0 &&
                hdr[length - 1] == 'B') /* Neutral == Little endian architecture and Big endian file */
            {
                retval = TRUE;
            }
            else if (endianness() == 1 &&
                     hdr[length - 1] == 'B') /* Neutral == Big endian architecture and Big endian file */
            {
                retval = FALSE;
            }
        }
        ...
        {
            if (endianness() == 0 && hdr[length - 1] == 'L' || endianness() == 1 && hdr[length - 1] == 'B')
            {
                retval = FALSE;
            }
            else if (endianness() == 0 && hdr[length - 1] == 'B' || endianness() == 1 && hdr[length - 1] == 'L')
            {
                retval = TRUE; /* convert */
            }
        }
    ...
    else if (nefis[set].file_version == Version_1)
    {
        ...
        {
            if (endianness() == 1 &&
                hdr[length - 1] == 'N') /* Neutral == Big endian architecture and Big endian file */
            {
                retval = FALSE; /* conversion */
            }
            else if (endianness() == 0 &&
                     hdr[length - 1] == 'N') /* Neutral == Little endian architecture and Big endian file*/
            {
        ...
        else
        {
            if (endianness() == 1 && coding == 'N') /* Neutral == Big endian architecture and convert to Big endian*/
            {
                retval = FALSE;
                     ...
                     coding == 'N') /* Neutral == Little endian architectur and convert to Big endian */
            {
                retval = TRUE;
            }
