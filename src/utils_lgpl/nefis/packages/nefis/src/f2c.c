...
#endif
{
    ...
    for (i = 0; i < *cl_num_dim; i++)
    {
        ...
        for (j = i * (MAX_NAME + 1) + max_copy; j < i * (MAX_NAME + 1) + MAX_NAME; j++)
        {
            elm_names[j] = ' ';
        }
...
#if defined(MIXED_STR_LEN_ARG)
    nefis_errno = GET_ELEMENT(fd, gr_name, gr_name_length, el_name, el_name_length, user_index, user_order,
                              buffer_length, buffer);
#else
    nefis_errno = GET_ELEMENT(fd, gr_name, el_name, user_index, user_order, buffer_length, buffer, gr_name_length,
                              el_name_length);
#endif
...
#endif
{
    ...
    if (at_value_length < MAX_NAME)
    {
        nefis_errcnt += 1;
        nefis_errno = 2011;
        sprintf(error_text, "Getsat: User supplied attribute string too small %ld<%ld\n", at_value_length, MAX_NAME);
        return nefis_errno;
    }
...
#endif
{
    ...
    if (el_names_length < MAX_NAME)
    {
        nefis_errcnt += 1;
        nefis_errno = 2012;
        sprintf(error_text, "Inqcel: Supplied array too small to contain all element names: \'%s\' %ld<%d\n", cel_name,
                el_names_length, MAX_NAME);
        return nefis_errno;
    }
...
#endif
{
    ...
    if (gr_defined_length < MAX_NAME)
    {
        nefis_errcnt += 1;
        nefis_errno = 2014;
        sprintf(error_text, "Inqdat: User supplied array to store group definition too small: %ld<%ld \n",
...
#endif
{
    ...
    if ((el_type_length < MAX_TYPE) || (el_quantity_length < MAX_NAME) || (el_unity_length < MAX_NAME) ||
        (el_desc_length < MAX_DESC))
    {
        ...
        sprintf(error_text,
                ...
                "%ld<%ld, %ld<%ld\n",
                el_name, el_name_length, el_type_length, MAX_TYPE, el_quantity_length, MAX_NAME, el_unity_length,
                MAX_NAME, el_desc_length, MAX_DESC);
        return nefis_errno;
    }
    if (el_name_length > MAX_NAME)
    {
        ...
        nefis_errcnt += 1;
        nefis_errno = 2016;
        sprintf(error_text, "Inqelm: Element name \'%s\' too long", elm_name_tmp);
        free(elm_name_tmp);
        return nefis_errno;
    }
    ...
    nefis_errno = GP_inquire_elm(*fd, elm_name, elm_type, elm_quantity, elm_unity, elm_desc, &elm_num_dim,
                                 (BUInt4*)elm_dimens, &elm_single_bytes, &elm_num_bytes);

    if (nefis_errno == 0)
    {
        strncpy(el_type, elm_type, MAX_TYPE);
        for (i = MAX_TYPE; i < el_type_length; i++)
        {
            el_type[i] = ' ';
        }

        strncpy(el_quantity, elm_quantity, MAX_NAME);
        for (i = MAX_NAME; i < el_quantity_length; i++)
        {
            el_quantity[i] = ' ';
        }

        strncpy(el_unity, elm_unity, MAX_NAME);
        for (i = MAX_NAME; i < el_unity_length; i++)
        {
            el_unity[i] = ' ';
        }

        strncpy(el_desc, elm_desc, MAX_DESC);
        for (i = MAX_DESC; i < el_desc_length; i++)
        {
            el_desc[i] = ' ';
        }
        ...
        if ((BUInt4)*el_num_dim < elm_num_dim)
        {
            ...
            nefis_errno = 2017;
            sprintf(error_text,
                    "Inqelm: User supplied array to contain element dimensions too small (check value of elmndm): "
                    "%ld<%ld \n",
...
#endif
{
    ...
    if ((gr_name_length < MAX_NAME) || (gr_defined_length < MAX_NAME))
    {
        nefis_errcnt += 1;
        nefis_errno = 2018;
        sprintf(error_text, "Inqfst: User supplied array to contain names too small: %ld<%ld, %ld<%ld\n",
...
#endif
{
    ...
    if ((el_name_length < MAX_NAME) || (el_type_length < MAX_TYPE) || (el_quantity_length < MAX_NAME) ||
        (el_unity_length < MAX_NAME) || (el_desc_length < MAX_DESC))
    {
        nefis_errcnt += 1;
        nefis_errno = 2019;
        sprintf(error_text,
                "Inqelm: User supplied array's to store element definition too small: %ld<%ld, %ld<%ld, %ld<%ld, "
                "%ld<%ld, %ld<%ld \n",
                el_name_length, MAX_NAME, el_type_length, MAX_TYPE, el_quantity_length, MAX_NAME, el_unity_length,
                MAX_NAME, el_desc_length, MAX_DESC);
        return nefis_errno;
    }
    /*
    ...
    if (nefis_errno == 0)
    {
        ...
        if (*el_num_dim < elm_num_dim)
        {
            nefis_errcnt += 1;
            nefis_errno = 2020;
            sprintf(error_text, "Inqfel: User supplied array to contain element names too small: %ld<%ld \n",
                    *el_num_dim, elm_num_dim);
            return nefis_errno;
        }
...
#endif
{
    ...
    if (el_names_length < MAX_NAME)
    {
        nefis_errcnt += 1;
        nefis_errno = 2023;
        sprintf(error_text, "Inqfcl: Supplied array too small to contain all element names: \'%s\' %ld>%d \n", cl_name,
                el_names_length, MAX_NAME);
        return nefis_errno;
    }
    /*
...
#endif
{
    ...
    if ((gr_name_length < MAX_NAME) || (cl_name_length < MAX_NAME))
    {
        nefis_errcnt += 1;
        nefis_errno = 2027;
        sprintf(error_text, "Inqfgr: User supplied array to contain names too small: %ld<%ld, %ld<%ld \n",
...
#endif
{
    ...
    if (at_name_length < MAX_NAME)
    {
        nefis_errcnt += 1;
        nefis_errno = 2031;
        sprintf(error_text, "Inqfia: User supplied array to contain integer attribute names too small: %ld<%ld\n",
...
#endif
{
    ...
    if (at_name_length < MAX_NAME)
    {
        nefis_errcnt += 1;
        nefis_errno = 2032;
        sprintf(error_text, "Inqfra: User supplied array to contain real attribute names too small: %ld<%ld\n",
                at_name_length, MAX_NAME);
        return nefis_errno;
    }
    F_Copy_text(grp_name, gr_name, gr_name_length, MAX_NAME);
...
#endif
{
    ...
    if ((at_name_length < MAX_NAME) || (at_value_length < MAX_NAME))
    {
        nefis_errcnt += 1;
        nefis_errno = 2033;
        sprintf(error_text,
    ...
    nefis_errno = GP_get_attribute(*fd, 1, grp_name, att_name, att_value, att_type);
...
#endif
{
    ...
    if (gr_defined_length > MAX_NAME)
    {
        ...
        nefis_errcnt += 1;
        nefis_errno = 2034;
        sprintf(error_text, "Inqgrp: Group name too long \'%s\'", grp_defined_tmp);
    ...
    if (nefis_errno == 0)
    {
        ...
        if (*gr_num_dim < (BInt4)grp_num_dim)
        {
            nefis_errcnt += 1;
            nefis_errno = 2035;
            sprintf(error_text, "Inqgrp: User supplied array to contain group dimensions too small: %ld<%ld\n",
                    *gr_num_dim, grp_num_dim);
            return nefis_errno;
        }
        *gr_num_dim = grp_num_dim;
...
#endif
{
    ...
    if ((gr_name_length < MAX_NAME) || (gr_defined_length < MAX_NAME))
    {
        nefis_errcnt += 1;
        nefis_errno = 2037;
        sprintf(error_text, "Inqnxt: User supplied array to contain names too small: %ld<%ld, %ld<%ld\n",
                gr_name_length, MAX_NAME, gr_defined_length, MAX_NAME);
        return nefis_errno;
    }
...
#endif
{
    ...
    if (at_name_length < MAX_NAME)
    {
        nefis_errcnt += 1;
        nefis_errno = 2038;
        sprintf(error_text, "Inqnia: User supplied array to contain integer attribute names too small: %ld<%ld\n",
                at_name_length, MAX_NAME);
        return nefis_errno;
    }
...
#endif
{
    ...
    if (at_name_length < MAX_NAME)
    {
        nefis_errcnt += 1;
        nefis_errno = 2039;
        sprintf(error_text, "Inqnra: User supplied array to contain real attribute names too small: %ld<%ld\n",
                at_name_length, MAX_NAME);
        return nefis_errno;
    }
...
#endif
{
    ...
    nefis_errno = GP_get_attribute(*fd, 2, grp_name, att_name, att_value, att_type);
...
#if defined(MIXED_STR_LEN_ARG)
    nefis_errno = PUT_ELEMENT_STRING(fd, gr_name, gr_name_length, el_name, el_name_length, user_index, user_order,
                                     getal, getal_length);
#else
    nefis_errno = PUT_ELEMENT_STRING(fd, gr_name, el_name, user_index, user_order, getal, gr_name_length,
                                     el_name_length, getal_length);
#endif
...
#endif
{
    ...
    if ((gr_name_length > MAX_NAME) || (at_name_length > MAX_NAME))
    {
        ...
        nefis_errcnt += 1;
        nefis_errno = 2042;
        sprintf(error_text, "Putiat: Groupname \'%s\' or\n        integer attribute name \'%s\' too long", grp_name_tmp,
                att_name_tmp);
        free(grp_name_tmp);
        free(att_name_tmp);
...
#endif
{
    ...
    if ((gr_name_length > MAX_NAME) || (at_name_length > MAX_NAME))
    {
        ...
        nefis_errcnt += 1;
        nefis_errno = 2043;
        sprintf(error_text, "Putrat: Groupname \'%s\' or\n        real attribute name \'%s\' too long", grp_name_tmp,
                att_name_tmp);
        free(grp_name_tmp);
        free(att_name_tmp);
        return nefis_errno;
...
#endif
{
    ...
    if ((gr_name_length > MAX_NAME) || (at_name_length > MAX_NAME) || (at_value_length > MAX_NAME))
    {
        grp_name_tmp = strFcpy(gr_name, gr_name_length);
        att_name_tmp = strFcpy(at_name, at_name_length);
