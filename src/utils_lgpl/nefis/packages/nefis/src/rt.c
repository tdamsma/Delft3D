...
static BInt4 RT_update_var_array(BInt4, BInt4, BUInt8*);
BInt4 RT_update_var_index_array(BInt4, BInt4, BUInt8, BInt4, BInt4, BUInt4*);
static retrieve* Find(retrieve*, BText, BText);
static retrieve* Tree(retrieve*, BText, BText, BUInt8*, BUInt4*, BUInt4*, BUInt4*, BUInt4*, BText, BUInt4*, BUInt4*,
                      BUInt4*, BUInt8*, BUInt8*);
static BInt4 RT_retrieve_struc(BInt4, BText, BText, BUInt8*, BUInt4*, BUInt4*, BUInt4*, BUInt4*, BText, BUInt4*,
                               BUInt4*, BUInt4*, BUInt8*, BUInt8*);
static BInt4 RT_retrieve_file(BInt4, BText, BText, BUInt8*, BUInt4*, BUInt4*, BUInt4*, BUInt4*, BText, BUInt4*, BUInt4*,
                              BUInt4*, BUInt8*, BUInt8*);
static BInt4 RT_update_struc(BInt4, BText, BText, BUInt8*, BUInt4*, BUInt4*, BUInt4*, BUInt4*, BText, BUInt4*, BUInt4*,
                             BUInt4*, BUInt8*, BUInt8*);
 ...
 */
/*==========================================================================*/
BText RT_streql(BText cs, BText ct, BInt4 max)
{
    cs[max] = '\0';
    ct[max] = '\0';
    if (strcmp(cs, ct) < 0)
 ...
 */
/*==========================================================================*/
BInt4 RT_retrieve(BInt4 set, BText grp_name, BText elm_name, BUInt8* cel_num_bytes, BUInt4* elm_dimens,
                  BUInt4* elm_num_dim, BUInt4* elm_offset, BUInt4* elm_single_bytes, BText elm_type, BUInt4* grp_dimens,
                  BUInt4* grp_num_dim, BUInt4* grp_order, BUInt8* grp_pointer, BUInt8* write_bytes)
{
    BInt4 error = 0;

    error = RT_retrieve_struc(set, grp_name, elm_name, cel_num_bytes, elm_dimens, elm_num_dim, elm_offset,
                              elm_single_bytes, elm_type, grp_dimens, grp_num_dim, grp_order, grp_pointer, write_bytes);
    /*
    ...
    {
        nefis_errno =
            RT_retrieve_file(set, grp_name, elm_name, cel_num_bytes, elm_dimens, elm_num_dim, elm_offset,
                             elm_single_bytes, elm_type, grp_dimens, grp_num_dim, grp_order, grp_pointer, write_bytes);

        if (nefis_errno == 0)
        {
            nefis_errno = RT_update_struc(set, grp_name, elm_name, cel_num_bytes, elm_dimens, elm_num_dim, elm_offset,
                                          elm_single_bytes, elm_type, grp_dimens, grp_num_dim, grp_order, grp_pointer,
                                          write_bytes);
        }
 ...
 */
/*==========================================================================*/
static BInt4 RT_retrieve_struc(BInt4 set, BText grp_name, BText elm_name, BUInt8* cel_num_bytes, BUInt4* elm_dimens,
                               BUInt4* elm_num_dim, BUInt4* elm_offset, BUInt4* elm_single_bytes, BText elm_type,
                               BUInt4* grp_dimens, BUInt4* grp_num_dim, BUInt4* grp_order, BUInt8* grp_pointer,
                               BUInt8* write_bytes)
{
    BInt4 error = 1;
...
/*==========================================================================*/
static retrieve* Find(retrieve* p, BText grp_name, BText elm_name)
{
    BInt4 a = 0;
 ...
 */
/*==========================================================================*/
static BInt4 RT_update_struc(BInt4 set, BText grp_name, BText elm_name, BUInt8* cel_num_bytes, BUInt4* elm_dimens,
                             BUInt4* elm_num_dim, BUInt4* elm_offset, BUInt4* elm_single_bytes, BText elm_type,
                             BUInt4* grp_dimens, BUInt4* grp_num_dim, BUInt4* grp_order, BUInt8* grp_pointer,
                             BUInt8* write_bytes)
{
    ...
    array_retrieve_ptrs[set] =
        Tree(array_retrieve_ptrs[set], grp_name, elm_name, cel_num_bytes, elm_dimens, elm_num_dim, elm_offset,
             elm_single_bytes, elm_type, grp_dimens, grp_num_dim, grp_order, grp_pointer, write_bytes);
...
/*==========================================================================*/
static retrieve* Tree(retrieve* p, BText grp_name, BText elm_name, BUInt8* cel_num_bytes, BUInt4* elm_dimens,
                      BUInt4* elm_num_dim, BUInt4* elm_offset, BUInt4* elm_single_bytes, BText elm_type,
                      BUInt4* grp_dimens, BUInt4* grp_num_dim, BUInt4* grp_order, BUInt8* grp_pointer,
                      BUInt8* write_bytes)
{
    ...
    else
    {
        if (strncmp(elm_name, p->elm_name, MAX_NAME) < 0)
        { /* to the left?     */
            p->left = Tree(p->left, grp_name, elm_name, cel_num_bytes, elm_dimens, elm_num_dim, elm_offset,
                           elm_single_bytes, elm_type, grp_dimens, grp_num_dim, grp_order, grp_pointer, write_bytes);
        }
        else
        { /* to the right?    */
            p->right = Tree(p->right, grp_name, elm_name, cel_num_bytes, elm_dimens, elm_num_dim, elm_offset,
                            elm_single_bytes, elm_type, grp_dimens, grp_num_dim, grp_order, grp_pointer, write_bytes);
        }
    }
    return (p);
 ...
 */
/*==========================================================================*/
BInt4 RT_retrieve_var(BInt4 set, BUInt8* grp_pointer, BInt4 var_index, BUInt8* var_pointer)
{
    BInt4 error = 1;
    ...
    *var_pointer = (BUInt8)BUINT8_MAX;
    ...
    for (i = 0; i < MAX_VAR_GROUPS; i++)
    {
        ...
        if ((fgd == -1) && (retrieve_var[set][i][0] == BUINT8_MAX))
        {
            fgd = i;
        }
    }
    ...
    if ((gd == -1) && (fgd == -1))
    {
        nefis_errno = 9001;
        nefis_errcnt += 1;
        sprintf(error_text, "The variable MAX_VAR_GROUPS needs to be increased.\nContact Deltares\n");
        return nefis_errno;
 ...
 */
/*==========================================================================*/
static BInt4 RT_update_var_array(BInt4 set, BInt4 gd, BUInt8* grp_pointer)
{
    BInt4 error = 1;
    BInt4 level = 0;
    BInt4 index = 0;
    ...
    error = RT_update_var_index_array(set, gd, start_table, level, index, &max_index);
    free(index_copy);
    if (error == 0)
    {
...
/*==========================================================================*/
BInt4 RT_update_var_index_array(BInt4 set, BInt4 gd, BUInt8 table_pointer, BInt4 level, BInt4 index, BUInt4* max_index)
{
    ...
    (BVoid) GP_read_file_pointers(dat_fds, data.st, table_pointer, SIZE_BINT8 * 256, set);
    ...
    for (k = 255; k > -1; k--)
    {
         ...
         */
        if ((data.ptr[k] != BUINT8_MAX) && (found[gd] == 0))
        {
            index += k;
            if (level != NR_VAR_TABLES - 1) /*TODO */
            {
                error = RT_update_var_index_array(set, gd, data.ptr[k], level + 1, index, max_index);
            }
            else
            {
                /*
                ...
                *index_copy = index;
                ...
                {
                    *max_index = index;
                    retrieve_var[set][gd] = (BUInt8*)realloc(retrieve_var[set][gd], (index + 3) * sizeof(BUInt8));
                }
                /*
                 * When using the Intel compilers version 11.0, 64-bit,
 ...
 */
/*==========================================================================*/
static BInt4 RT_retrieve_file(BInt4 set, BText grp_name, BText look_elm_name, BUInt8* cel_num_bytes, BUInt4* elm_dimens,
                              BUInt4* elm_num_dim, BUInt4* elm_offset, BUInt4* elm_single_bytes, BText elm_type,
                              BUInt4* grp_dimens, BUInt4* grp_num_dim, BUInt4* grp_order, BUInt8* grp_pointer,
                              BUInt8* write_bytes)
{
     ...
     */
    elm_names = NULL;
    nefis_errno = GP_inquire_cel(set, cel_name, &cel_num_dim, &elm_names, &numbytes);
    ...
    *elm_offset = 0;
    if (strncmp("*", look_elm_name, 1) == 0)
    {
        ...
        nefis_errno = GP_inquire_elm(set, elm_name, elm_type, elm_quantity, elm_unity, elm_desc, elm_num_dim,
    ...
    if (i == cel_num_dim)
    {
        nefis_errno = 9002;
        nefis_errcnt += 1;
        sprintf(error_text, "Element \"%s\" of group \"%s\" not found on file \"%s\"\n", look_elm_name, grp_name,
                nefis[set].dat_name);
        return nefis_errno;
    }
