...
extern BInt4 HS_check_ecg(BInt4, BInt4, BText, BUInt8, BUInt8*, BInt4, BUInt8*, BUInt4*, BUInt4*);
extern BInt4 HS_get_cont_dat(BInt4, BUInt8, BUInt8*, BText, BText);
extern BInt4 HS_get_cont_cel(BInt4, BUInt8, BUInt8*, BText, BText*, BUInt4*, BUInt8*);
extern BInt4 HS_get_cont_elm(BInt4, BUInt8, BUInt8*, BText, BText, BText, BText, BText, BUInt4*, BUInt4*, BUInt4*,
                             BUInt8*);
extern BInt4 HS_get_cont_grp(BInt4, BUInt8, BUInt8*, BText, BText, BUInt4*, BUInt4*, BUInt4*);
extern BInt4 convert_ieee(BData*, BText*, BUInt8, BInt4, BText, BInt4);
/*
 * Static prototypes
 */
 ...
 */
/*==========================================================================*/
static void FileSeek(BInt4 fds, BUInt8 startPos)
{
    BInt8 retVal;
    BUInt8 rest;
 ...
 */
/*==========================================================================*/
BUInt8 GP_read_file(BInt4 fds, BText string, BUInt8 start, BUInt8 n_bytes)
{
    BUInt8 n_read = 0;
    BUInt4 tmp_bytes;
 ...
 */
/*==========================================================================*/
BUInt8 GP_read_file_pointers(BInt4 fds, BText string, BUInt8 start, BUInt8 n_bytes, BInt4 set)
{
    BUInt8 n_read = 0;
    ...
    if (nefis[set].file_version == Version_1)
    {
        ...
        n_read = 2 * n_read;
        if (nefis[set].daf_neutral == TRUE || nefis[set].dat_neutral == TRUE || nefis[set].def_neutral == TRUE)
        {
            BUInt4 tmp_bytes;
 ...
 */
/*==========================================================================*/
BUInt8 GP_write_file(BInt4 fds, BText string, BUInt8 start, BUInt8 n_bytes)
{
    BUInt8 n_written;
    BUInt8 rest;
    BUInt4 tmp_rest;
 ...
 */
/*==========================================================================*/
BUInt8 GP_write_file_pointers(BInt4 fds, BText string, BUInt8 start, BUInt8 n_bytes, BInt4 set)
{
    BUInt8 n_written = 0;

    if (nefis[set].file_version == Version_1)
    {
        if (nefis[set].daf_neutral == TRUE || nefis[set].dat_neutral == TRUE)
        {
 ...
 */
/*==========================================================================*/
BInt4 GP_flush_hash(BInt4 fds, BInt4 set)
{
    BText cp;
    BInt4 daf_fds;
    BInt4 dat_fds;
    ...
    if (fds == dat_fds && nefis[set].one_file == FALSE)
    {
        ...
        {
            n_written =
                GP_write_file_pointers(dat_fds, (BText)&nefis[set].dat.fds[1], LHDRDT, SIZE_BINT8 * (LHSHDT + 1), set);
        }
        if (n_written != SIZE_BINT8 * (LHSHDT + 1))
        {
            nefis_errcnt += 1;
            nefis_errno = 6001;
            sprintf(error_text, "Hashtable not written to data file \'%s\'", nefis[set].dat_name);
            return nefis_errno;
        }
    }
    ...
    if ((fds == def_fds) && (nefis[set].one_file == FALSE))
    {
        if (nefis[set].def_neutral == TRUE)
        {
            ...
            nefis_errno = convert_ieee(&vp, &cp, SIZE_BINT8 * (LHSHDF + 1), SIZE_BINT8, "INTEGER", from_xdr);
        ...
        {
            n_written =
                GP_write_file_pointers(def_fds, (BText)&nefis[set].def.fds[1], LHDRDF, SIZE_BINT8 * (LHSHDF + 1), set);
        }
        if (n_written != SIZE_BINT8 * (LHSHDF + 1))
        {
            nefis_errcnt += 1;
            nefis_errno = 6002;
            sprintf(error_text, "Hashtable not written to definition file \'%s\'", nefis[set].def_name);
            return nefis_errno;
        }
    }
    ...
    if ((fds == daf_fds) && (nefis[set].one_file == TRUE))
    {
        if (nefis[set].daf_neutral == TRUE)
        {
            ...
            nefis_errno = convert_ieee(&vp, &cp, SIZE_BINT8 * (LHSHDAF + 1), SIZE_BINT8, "INTEGER", from_xdr);
        ...
        {
            n_written = GP_write_file_pointers(daf_fds, (BText)&nefis[set].daf.fds[1], LHDRDAF,
                                               SIZE_BINT8 * (LHSHDAF + 1), set);
        }
        if (n_written != SIZE_BINT8 * (LHSHDAF + 1))
        {
            nefis_errcnt += 1;
            nefis_errno = 6002;
            sprintf(error_text, "Hashtable not written to DefinitionData file \'%s\'", nefis[set].daf_name);
            return nefis_errno;
        }
    }
 ...
 */
/*==========================================================================*/
BInt4 GP_inquire_cel(BInt4 set, BText cel_name, BUInt4* cel_num_dim, BText* elm_names, BUInt8* cel_num_bytes)
{
    BInt4 fds_file;
    BUInt4 new_entry;
    ...
    if (nefis[set].one_file == TRUE)
    {
        ...
        nefis_errno = HS_check_ecg(set, fds_file, cel_name, NIL, &nefis[set].daf.fds[2 + LHSHEL], LHSHCL, &pnt_hash,
                                   &pnt_bucket, &new_entry);
    }
    else
    {
        fds_file = nefis[set].def_fds;
        nefis_errno = HS_check_ecg(set, fds_file, cel_name, NIL, &nefis[set].def.fds[2 + LHSHEL], LHSHCL, &pnt_hash,
                                   &pnt_bucket, &new_entry);
    }
    ...
    if (new_entry == 1)
    {
        nefis_errcnt += 1;
        nefis_errno = 6003;
        sprintf(error_text, "Cell \'%s\' does not exist in definition file", cel_name);
    }

    if (nefis_errno == 0)
    {
        nefis_errno = HS_get_cont_cel(set, pnt_hash, &next_pointer, cel_name, elm_names, cel_num_dim, cel_num_bytes);
    }
 ...
 */
/*==========================================================================*/
BInt4 GP_inquire_dat(BInt4 set, BUInt8* grp_pointer, BText grp_name, BText grp_defined)
{
    BInt4 fds_file;
    ...
    {
        fds_file = nefis[set].daf_fds;
        nefis_errno = HS_check_ecg(set, fds_file, grp_name, NIL, &nefis[set].daf.fds[2 + LHSHEL + LHSHCL + LHSHGR],
                                   LHSHDT, &pnt_hash, &pnt_bucket, &new_entry);
    }
    else
    {
        fds_file = nefis[set].dat_fds;
        nefis_errno = HS_check_ecg(set, fds_file, grp_name, NIL, &nefis[set].dat.fds[2], LHSHDT, &pnt_hash, &pnt_bucket,
                                   &new_entry);
    }
    ...
    *grp_pointer = pnt_hash;
    ...
    if (new_entry == 1)
    {
        nefis_errcnt += 1;
        nefis_errno = 6004;
        sprintf(error_text, "Group \'%s\' does not exist in data file\n", grp_name);
        return nefis_errno;
    }

    if (nefis_errno == 0)
    {
        nefis_errno = HS_get_cont_dat(set, pnt_hash, &next_pointer, grp_name, grp_defined);
    }
 ...
 */
/*==========================================================================*/
BInt4 GP_inquire_elm(BInt4 set, BText elm_name, BText elm_type, BText elm_quantity, BText elm_unity, BText elm_desc,
                     BUInt4* elm_num_dim, BUInt4* elm_dimens, BUInt4* elm_single_bytes, BUInt8* elm_num_bytes)
{
    BInt4 fds_file;
    BUInt4 new_entry;
    BUInt8 next_pointer = 0;
    ...
    {
        fds_file = nefis[set].daf_fds;
        nefis_errno = HS_check_ecg(set, fds_file, elm_name, NIL, &nefis[set].daf.fds[2], LHSHEL, &pnt_hash, &pnt_bucket,
                                   &new_entry);
    }
    else
    {
        fds_file = nefis[set].def_fds;
        nefis_errno = HS_check_ecg(set, fds_file, elm_name, NIL, &nefis[set].def.fds[2], LHSHEL, &pnt_hash, &pnt_bucket,
                                   &new_entry);
    }
    ...
    if (new_entry == 1)
    {
        nefis_errcnt += 1;
        nefis_errno = 6005;
        sprintf(error_text, "Element \'%s\' does not exist in definition file\n", elm_name);
    }

    if (nefis_errno == 0)
    {
        nefis_errno = HS_get_cont_elm(set, pnt_hash, &next_pointer, elm_name, elm_type, elm_quantity, elm_unity,
                                      elm_desc, elm_num_dim, elm_dimens, elm_single_bytes, elm_num_bytes);
    }
 ...
 */
/*==========================================================================*/
BInt4 GP_inquire_grp_def(BInt4 set, BText grp_defined, BText cel_name, BUInt4* grp_num_dim, BUInt4* grp_dimens,
                         BUInt4* grp_order)
{
    BInt4 def_fds;
    BUInt4 new_entry;
    ...
    if (nefis[set].one_file == TRUE)
    {
        ...
        nefis_errno = HS_check_ecg(set, def_fds, grp_defined, NIL, &nefis[set].daf.fds[2 + LHSHEL + LHSHCL], LHSHGR,
                                   &pnt_hash, &pnt_bucket, &new_entry);
    }
    else
    {
        def_fds = nefis[set].def_fds;
        nefis_errno = HS_check_ecg(set, def_fds, grp_defined, NIL, &nefis[set].def.fds[2 + LHSHEL + LHSHCL], LHSHGR,
                                   &pnt_hash, &pnt_bucket, &new_entry);
    }
    ...
    if (new_entry == 1)
    {
        nefis_errcnt += 1;
        nefis_errno = 6007;
        sprintf(error_text, "Group \'%s\' does not exist in definition file\n", grp_defined);
    }
    if (nefis_errno == 0)
    {
        nefis_errno =
            HS_get_cont_grp(set, pnt_hash, &next_pointer, grp_defined, cel_name, grp_num_dim, grp_dimens, grp_order);
    }
 ...
 */
/*==========================================================================*/
BInt4 GP_put_attribute(BInt4 set, BText grp_name, BText att_name, BData att_value, BText att_type)
{
    union data_put
    {
    ...
    if (n_read > (BUInt8)SIZE_DAT_BUF)
    {
        nefis_errcnt += 1;
        nefis_errno = 6009;
        sprintf(error_text, "On reading attribute of group \'%s\'", grp_name);
        return nefis_errno;
    }
    ...
    if (strncmp(att_type, "INTEGER", 7) == 0)
    {
        ...
        if (j == -1)
        {
            nefis_errcnt += 1;
            nefis_errno = 6010;
            sprintf(error_text, "No space left in data file for integer attribute of group \'%s\'", grp_name);
            return nefis_errno;
        }
    ...
    if (strncmp(att_type, "REAL", 4) == 0)
    {
        ...
        if (j == -1)
        {
            nefis_errcnt += 1;
            nefis_errno = 6011;
            sprintf(error_text, "No space left in data file for real attribute of group \'%s\'", grp_name);
            return nefis_errno;
        }
    ...
    if (strncmp(att_type, "CHARACTE", 8) == 0)
    {
        ...
        if (j == -1)
        {
            nefis_errcnt += 1;
            nefis_errno = 6012;
            sprintf(error_text, "No space left in data file for string attribute of group \'%s\'", grp_name);
            return nefis_errno;
        }
        strncpy(dat_buf.st + shift_to_name + MAX_NAME * j, att_name, MAX_NAME);
 ...
 *          1           error occured
 */
BInt4 GP_get_attribute(BInt4 set, BInt4 find, BText grp_name, BText att_name, BData att_value, BText att_type)
{
    ...
    {
        fds = nefis[set].daf_fds;
        nefis_errno = HS_check_ecg(set, fds, grp_name, NIL, &nefis[set].daf.fds[2 + LHSHEL + LHSHCL + LHSHGR], LHSHDT,
                                   &grp_pointer, &pnt_bucket, &new_entry);
    }
    else
    {
        fds = nefis[set].dat_fds;
        nefis_errno = HS_check_ecg(set, fds, grp_name, NIL, &nefis[set].dat.fds[2], LHSHDT, &grp_pointer, &pnt_bucket,
                                   &new_entry);
    }
    ...
    if (strncmp(att_type, "INTEGER", 7) == 0)
    {
        ...
        if (j == -1)
        {
            if (find == given)
            {
                nefis_errcnt += 1;
                nefis_errno = 6015;
                sprintf(error_text, "Integer attribute \'%s\' of group \'%s\'not found", att_name, grp_name);
            }
            else
            {
                nefis_errcnt += 1;
                nefis_errno = 6016;
                sprintf(error_text, "No valid attribute name found");
            }
        }
        else
        {
    ...
    if (strncmp(att_type, "REAL", 4) == 0)
    {
        ...
        if (j == -1)
        {
            if (find == given)
            {
                nefis_errcnt += 1;
                nefis_errno = 6017;
                sprintf(error_text, "Real attribute \'%s\' of group \'%s\'not found", att_name, grp_name);
            }
            else
            {
                nefis_errcnt += 1;
    ...
    if (strncmp(att_type, "CHARACTE", 8) == 0)
    {
        ...
        if (j == -1)
        {
            if (find == given)
            {
                nefis_errcnt += 1;
                nefis_errno = 6019;
                sprintf(error_text, "String attribute \'%s\' of group \'%s\'not found", att_name, grp_name);
            }
            else
            {
                nefis_errcnt += 1;
                      ...
                      BText grp_defined) /* O */
{
    ...
    if (nefis[set].one_file == TRUE)
    {
        if (next == 0)
        {
            ...
            if (num_hash == -1)
            {
                nefis_errcnt += 1;
                nefis_errno = -6021;
                sprintf(error_text, "No data groups available in DefinitonData file \'%s\'\n", nefis[set].dat_name);
                return nefis_errno;
            }
        ...
        else if (hash_pointer > num_hash)
        {
            nefis_errcnt += 1;
            nefis_errno = -6021;
            sprintf(error_text, "No more data groups available in DefinitonData file \'%s\'\n", nefis[set].daf_name);
            return nefis_errno;
        }
    }
    else
     ...
     */
    {
        if (next == 0)
        {
            ...
            if (num_hash == -1)
            {
                nefis_errcnt += 1;
                nefis_errno = -6022;
                sprintf(error_text, "No data groups available in data file \'%s\'\n", nefis[set].dat_name);
                return nefis_errno;
            }
        ...
        else if (hash_pointer > num_hash)
        {
            nefis_errcnt += 1;
            nefis_errno = -6022;
            sprintf(error_text, "No more data groups available in data file \'%s\'\n", nefis[set].dat_name);
            return nefis_errno;
        }
    }
     ...
     */
    nefis_errno = HS_get_cont_dat(set, pointer, &next_pointer, grp_name, grp_defined);
    if (nefis_errno != 0)
    {
        return nefis_errno;
    }
                      ...
                      BUInt8* elm_num_bytes)    /* O */
{
    ...
    if (nefis[set].one_file == TRUE)
    {
        ...
        else if (hash_pointer > num_hash)
        {
            nefis_errcnt += 1;
            nefis_errno = -6023;
            sprintf(error_text, "No more elements available in DefinitionData file \'%s\'\n", nefis[set].daf_name);
            return nefis_errno;
        }
    }
    else
     ...
     */
    {
        ...
        else if (hash_pointer > num_hash)
        {
            nefis_errcnt += 1;
            nefis_errno = -6024;
            sprintf(error_text, "No more elements available in definition file \'%s\'\n", nefis[set].def_name);
            return nefis_errno;
        }
    }
    ...
    nefis_errno = HS_get_cont_elm(set, pointer, &next_pointer, elm_name, elm_type, elm_quantity, elm_unity, elm_desc,
                                  elm_num_dim, elm_dimens, elm_single_bytes, elm_num_bytes);
    if (nefis_errno != 0)
    {
        return nefis_errno;
    }
                       ...
                       BUInt8* cel_num_bytes) /* O */
{
    ...
    if (nefis[set].one_file == TRUE)
    {
        ...
        else if (hash_pointer > num_hash)
        {
            nefis_errcnt += 1;
            nefis_errno = -6025;
            sprintf(error_text, "No more cells available in DefinitionData file \'%s\'\n", nefis[set].daf_name);
            return nefis_errno;
        }
    }
    else
     ...
     */
    {
        ...
        else if (hash_pointer > num_hash)
        {
            nefis_errcnt += 1;
            nefis_errno = -6026;
            sprintf(error_text, "No more cells available in definition file \'%s\'\n", nefis[set].def_name);
            return nefis_errno;
        }
    }
    ...
    nefis_errno = HS_get_cont_cel(set, pointer, &next_pointer, cel_name, elm_names, cel_num_dim, cel_num_bytes);
    if (nefis_errno != 0)
    {
        return nefis_errno;
    }
                          ...
                          BUInt4* grp_order)   /* O */
{
    ...
    if (nefis[set].one_file == TRUE)
    {
        ...
        else if (hash_pointer > num_hash)
        {
            nefis_errcnt += 1;
            nefis_errno = -6027;
            sprintf(error_text, "No more defined groups available in DefinitionData file \'%s\'\n",
                    nefis[set].daf_name);
            return nefis_errno;
        }
    }
     ...
     */
    {
        ...
        else if (hash_pointer > num_hash)
        {
            nefis_errcnt += 1;
            nefis_errno = -6028;
            sprintf(error_text, "No more defined groups available in definition file \'%s\'\n", nefis[set].def_name);
            return nefis_errno;
        }
    }
    ...
    nefis_errno = HS_get_cont_grp(set, pointer, &next_pointer, grp_name, cel_name, grp_num_dim, grp_dimens, grp_order);
...
BInt4 GP_variable_pointer(BInt4 set, BUInt8* grp_pointer, BUInt4 var_index, BUInt8* var_pointer)
{
    union data_var
    {
        BChar st[8 * 257];
    ...
    if (n_read > (BUInt8)SIZE_BINT8 * 256)
    {
        nefis_errcnt += 1;
        nefis_errno = 6029;
        sprintf(error_text, "On reading first variable pointer table");
        return nefis_errno;
    }
    ...
    for (j = 24; j >= 0; j = j - 8)
    {
        ...
        if (pointer_buf.ptr[where] == BUINT8_MAX)
        {
            ...
            if (n_written != (BUInt8)SIZE_BINT8 * 256)
            {
                nefis_errcnt += 1;
                nefis_errno = 6030;
                sprintf(error_text, "On writing variable pointer table, table %ld", j);
                return nefis_errno;
            }
            ...
            else
            {
                if (nefis[set].one_file == TRUE)
                {
                    if ((end_file == BUINT8_MAX) || (end_file < nefis[set].daf.fds[1]))
                    {
                        nefis_errcnt += 1;
                        nefis_errno = 6033;
                        sprintf(error_text, "Maximum size reached in DataDefinition file\n");
                        return nefis_errno;
                    }
                ...
                {
                    if ((end_file == BUINT8_MAX) || (end_file < nefis[set].dat.fds[1]))
                    {
                        nefis_errcnt += 1;
                        nefis_errno = 6034;
                        sprintf(error_text, "Maximum size reached in data file\n");
                        return nefis_errno;
                    }
        ...
        else
        {
            ...
            if (j != 0)
            {
                ...
                if (n_read > (BUInt8)SIZE_BINT8 * 256)
                {
                    nefis_errcnt += 1;
                    nefis_errno = 6031;
                    sprintf(error_text, "On reading variable pointer table, table %ld", j);
                    return nefis_errno;
                }
 ...
 */
static BVoid QuickSort(BUInt8* v, BInt4 left, BInt4 right)
{
    BInt4 i;
    BInt4 last;
 ...
 */
/*==========================================================================*/
BInt4 GP_inquire_max(BInt4 set, BText grp_name, BUInt4* max_index)
{
    union dat_var
    {
        BChar st[8 * 256];
    ...
    nefis_errno = GP_inquire_grp_def(set, grp_defined, cel_name, &grp_num_dim, grp_dimens, grp_order);
    if (nefis_errno != 0)
    {
        return nefis_errno;
    }
