 ...
 */
/*==========================================================================*/
BInt4 Get_element(BInt4 set, BText grp_name, BText elm_name, BInt4* usr_index, BInt4* usr_order, BUInt4 buffer_length,
                  BData buffer)
{
    ...
    nefis_errno = RT_retrieve(set, grp_name, elm_name, &cel_num_bytes, (BUInt4*)elm_dimens, &elm_num_dim,
                              ...
                              (BUInt4*)grp_order, &grp_pointer, &read_bytes);
    ...
    for (i = 0; i < grp_num_dim; i++)
    {
        ...
        if (grp_dimens[usr_order[i] - 1] > 0)
        {
            if (usr_index[A2D(i, 1)] > grp_dimens[usr_order[i] - 1])
            {
                nefis_errcnt += 1;
                nefis_errno = 3004;
                sprintf(error_text, "Stop value %ld should be smaller than %ld\n", usr_index[A2D(i, 1)],
                        grp_dimens[usr_order[i] - 1]);
            }
        }
    }
    ...
    conv_bytes = read_bytes * (j2 - j1 + j3) / j3 * (k2 - k1 + k3) / k3 * (l2 - l1 + l3) / l3 * (m2 - m1 + m3) / m3 *
    ...
    if (buffer_length < conv_bytes)
    {
        nefis_errcnt += 1;
        nefis_errno = 3005;
        sprintf(error_text, "Buffer length too small, should be %llu instead of %ld\nGroup \"%s\", element \"%s\"\n",
                ...
                for (l = l1 - 1; l < l2; l = l + l3)
                {
                    ...
                    for (k = k1 - 1; k < k2; k = k + k3)
                    {
                        ...
                        for (j = j1 - 1; j < j2; j = j + j3)
                        {
                            ...
                            (BVoid) GP_read_file(dat_fds, ch_buffer + (BUInt4)(j_array_offset * read_bytes), l_pointer,
                                                 read_bytes);
