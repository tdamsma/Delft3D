 ...
 */
/*==========================================================================*/
BInt4 Define_element(BInt4 set, BText elm_name, BText elm_type, BText elm_quantity, BText elm_unity, BText elm_desc,
                     BUInt4 elm_single_bytes, BUInt4 elm_num_dim, BUInt4* elm_dimens)
{
    BText cp;
    BUInt8* eof_ptr;
    BInt4 fds;
     ...
     */
    if ((elm_num_dim < 1) || (elm_num_dim > MAX_DIM))
    {
        nefis_errcnt += 1;
        nefis_errno = 5001;
        sprintf(error_text, "Number of dimensions not within the range [1,%d]\n      Element \'%s\' has dimension %ld",
     ...
     */
    nefis_errno = HS_check_ecg(set, fds, elm_name, NIL, hashtb, LHSHEL, &pnt_hash, &pnt_bucket, &new_entry);
    if (new_entry == 0)
    {
        nefis_errcnt += 1;
        nefis_errno = 5007;
        sprintf(error_text, "Element \'%s\' already exists\n", elm_name);
    }
    if (nefis_errno != 0)
    {
        return nefis_errno;
    ...
    if (pnt_hash == NIL)
    {
         ...
         */
        if (nefis[set].daf_neutral == TRUE || nefis[set].def_neutral == TRUE)
        {
            from_xdr = 0;
        ...
        if (n_written != size_elm_buf)
        {
            nefis_errcnt += 1;
            nefis_errno = 5008;
            sprintf(error_text, "Error writing element \'%s\' to definition file\n", elm_name);
            return nefis_errno;
        }
        ...
        if (next_pnt_hash < prv_pnt_hash)
        {
            ...
            if (nefis[set].one_file == TRUE)
            {
                sprintf(error_text, "Maximum size reached in DataDefinition file \"%s\"\n", nefis[set].daf_name);
            }
            else
            {
                sprintf(error_text, "Maximum size reached in definition file\"%s\"\n", nefis[set].def_name);
            }
            return nefis_errno;
        }
...
BInt4 Define_cel(BInt4 set, BText cel_name, BUInt4 cel_dimens, BText elm_names)
{
    BText cp;
    ...
    for (i = 0; i < cel_dimens; i++)
    {
        ...
        else
        {
            nefis_errcnt += 1;
            nefis_errno = 5009;
            sprintf(error_text, "Element \'%s\' does not exist\n", elm_name);
            return nefis_errno;
        }
    }
    /*
     ...
     */
    nefis_errno = HS_check_ecg(set, fds, cel_name, NIL, hashtb + LHSHEL, LHSHCL, &pnt_hash, &pnt_bucket, &new_entry);
    if (new_entry == 0)
    {
        nefis_errcnt += 1;
        nefis_errno = 5010;
        sprintf(error_text, "Cel \'%s\' already exists\n", cel_name);
    }
    if (nefis_errno != 0)
    {
        return nefis_errno;
    ...
    if (pnt_hash == NIL)
    {
         ...
         *    Set pointer to end-of-file and write buffer
         */
        if (nefis[set].def_neutral == TRUE || nefis[set].daf_neutral == TRUE)
        {
            from_xdr = 0;
        ...
        if (n_written != size_cel_buf)
        {
            nefis_errcnt += 1;
            nefis_errno = 5011;
            sprintf(error_text, "Error writing cel \'%s\' to definition file.\n", cel_name);
            return nefis_errno;
        }
 ...
 */
/*==========================================================================*/
BInt4 Define_group(BInt4 set, BText grp_name, BText cel_name, BUInt4 grp_num_dim, BInt4* grp_dimens, BInt4* grp_order)
{
    BText cp;
    BUInt8* eof_ptr;
    BInt4 fds;
     ...
     */
    if ((grp_num_dim < 1) && (grp_num_dim > MAX_DIM))
    {
        nefis_errcnt += 1;
        nefis_errno = 5016;
        sprintf(error_text, "Number of dimensions not within the range [1,MAX_DIM]\n Group \'%s\'", grp_name);
        return nefis_errno;
    }
    /*
    ...
    if (new_entry == 1)
    {
        nefis_errcnt += 1;
        nefis_errno = 5013;
        sprintf(error_text, "Cel \'%s\' does not exist on file \'%s\'.\n", cel_name, nefis[set].def_name);
    }
    if (nefis_errno != 0)
    {
        return nefis_errno;
     ...
     */
    nefis_errno =
        HS_check_ecg(set, fds, grp_name, NIL, hashtb + LHSHEL + LHSHCL, LHSHGR, &pnt_hash, &pnt_bucket, &new_entry);
    if (new_entry == 0)
    {
        nefis_errcnt += 1;
        nefis_errno = 5014;
        sprintf(error_text, "Group \'%s\' already exists on file \'%s\'.\n", grp_name, nefis[set].def_name);
    }
    if (nefis_errno != 0)
    {
        return nefis_errno;
    ...
    if (pnt_hash == NIL)
    {
        ...
        if (n_written != (BUInt8)size_grp_buf)
        {
            nefis_errcnt += 1;
            nefis_errno = 5015;
            sprintf(error_text, "Error on writing group \'%s\' to definition file.\n", grp_name);
            return nefis_errno;
        }
 ...
 */
/*==========================================================================*/
BInt4 Define_data(BInt4 set, BText grp_name, BText grp_defined)
{
     ...
     */
    nefis_errno = HS_check_ecg(set, def_fds, grp_defined, NIL, def_hashtb + LHSHEL + LHSHCL, LHSHGR, &pnt_hash,
    ...
    if (new_entry == 1)
    {
        nefis_errcnt += 1;
        nefis_errno = 5017;
        sprintf(error_text, "Group definition \'%s\' does not exist on file \'%s\'.\n", grp_defined,
                nefis[set].def_name);
    }
    if (nefis_errno != 0)
    {
     ...
     */
    nefis_errno =
        HS_check_ecg(set, def_fds, cel_name, NIL, def_hashtb + LHSHEL, LHSHCL, &pnt_hash, &pnt_bucket, &new_entry);
    if (new_entry == 1)
    {
        nefis_errcnt += 1;
        nefis_errno = 5018;
     ...
     */
    nefis_errno = HS_check_ecg(set, dat_fds, grp_name, NIL, dat_hashtb, LHSHDT, &pnt_hash, &pnt_bucket, &new_entry);
    if (new_entry == 0)
    {
        pnt_hash = 0;
        nefis_errcnt += 1;
        nefis_errno = 5019;
        sprintf(error_text, "Data group \'%s\' already exists in data file \'%s\'\n", grp_name, nefis[set].dat_name);
    }
    if (nefis_errno != 0)
    {
        return nefis_errno;
    ...
    if (pnt_hash == NIL)
    {
        ...
        if (nefis[set].dat_neutral == TRUE || nefis[set].daf_neutral == TRUE)
        {
            from_xdr = 0;
        ...
        if (n_written != size_dat_buf)
        {
            nefis_errcnt += 1;
            nefis_errno = 5023;
            sprintf(error_text, "Error on writing fixed part of group \'%s\' \n", grp_name);
            return nefis_errno;
        }
        if (var_dim == TRUE)
        {
            n_written = GP_write_file_pointers(dat_fds, dat_buf.st + size_dat_buf, *dat_eof_ptr + size_dat_buf,
                                               grp_num_bytes, set);
            if (n_written != grp_num_bytes)
            {
                nefis_errcnt += 1;
                nefis_errno = 5022;
                sprintf(error_text, "Error on writing variable part of group \'%s\' (first pointer table).\n",
                        grp_name);
                return nefis_errno;
            }
            if (nefis[set].file_version == Version_1)
 ...
 */
/*==========================================================================*/
BInt4 DF_check_bytes_elm(BUInt8* total_bytes, BText elm_type, BUInt4 elm_single_bytes, BUInt4 elm_num_dim)
{
    BUInt8 num_bytes;
    ...
    if (strncmp(elm_type, "REAL", 4) == 0)
    {
        ...
        if (num_bytes != elm_single_bytes)
        {
            ...
            if (num_bytes != elm_single_bytes)
            {
                nefis_errcnt += 1;
                nefis_errno = 5002;
                sprintf(error_text, "This size of real (%ld) is not supported\n", elm_single_bytes);
                return nefis_errno;
            }
        }
    }
        ...
        if (num_bytes != elm_single_bytes)
        {
            ...
            if (num_bytes != elm_single_bytes)
            {
                ...
                if (num_bytes != elm_single_bytes)
                {
                    nefis_errcnt += 1;
                    nefis_errno = 5003;
                    sprintf(error_text, "This size of integer (%ld) is not supported\n", elm_single_bytes);
                    return nefis_errno;
                }
            }
        }
    ...
    else if (strncmp(elm_type, "COMPLEX", 7) == 0)
    {
        ...
        if (num_bytes != elm_single_bytes)
        {
            ...
            if (num_bytes != elm_single_bytes)
            {
                nefis_errcnt += 1;
                nefis_errno = 5004;
                sprintf(error_text, "This size of complex (%ld) is not supported\n", elm_single_bytes);
                return nefis_errno;
            }
        }
    }
    else if (strncmp(elm_type, "LOGICAL", 7) == 0)
    {
        ...
        if (num_bytes != elm_single_bytes)
        {
            ...
            if (num_bytes != elm_single_bytes)
            {
                nefis_errcnt += 1;
                nefis_errno = 5005;
                sprintf(error_text, "This size of logical (%ld) is not supported\n", elm_single_bytes);
                return nefis_errno;
            }
        }
    }
