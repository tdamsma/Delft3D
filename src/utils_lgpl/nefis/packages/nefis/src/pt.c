 ...
 */
/*==========================================================================*/
BInt4 Put_element(BInt4 set, BText grp_name, BText elm_name, BInt4* usr_index, BInt4* usr_order, BData getal)
{
     ...
     */
    nefis_errno = RT_retrieve(set, grp_name, elm_name, &cel_num_bytes, (BUInt4*)elm_dimens, &elm_num_dim,
                              &element_offset, &elm_single_bytes, elm_type, (BUInt4*)grp_dimens, &grp_num_dim,
                              (BUInt4*)grp_order, &grp_pointer, &write_bytes);
    ...
    for (i = 0; i < grp_num_dim; i++)
    {
        if (usr_index[A2D(i, 0)] > usr_index[A2D(i, 1)])
        {
            nefis_errcnt += 1;
            nefis_errno = 4001;
            sprintf(error_text, "Start value user index [%ld,2]=%ld should be smaller than [%ld,2]=%ld\n", i,
                    usr_index[A2D(i, 0)], i, usr_index[A2D(i, 1)]);
        }
        if (usr_index[A2D(i, 2)] < 1)
        {
            nefis_errcnt += 1;
            nefis_errno = 4002;
            sprintf(error_text, "Increment value user index [%ld,2]=%ld should be greater than 0\n", i,
                    usr_index[A2D(i, 2)]);
        }
        if (usr_index[A2D(i, 0)] < 1)
        {
            nefis_errcnt += 1;
            nefis_errno = 4003;
            sprintf(error_text, "Start value user index [%ld,0]=%ld should be greater than zero\n", i,
                    usr_index[A2D(i, 0)]);
        }
        if (grp_dimens[usr_order[i] - 1] > 0)
        {
            if (usr_index[A2D(i, 1)] > grp_dimens[usr_order[i] - 1])
            {
                nefis_errcnt += 1;
                nefis_errno = 4004;
                sprintf(error_text, "Stop value user index [%ld,1]=%ld should be smaller than %ld\n", i,
    ...
    for (i = 1; i < MAX_DIM; i++)
    {
        usr_offset[i] = (BUInt4)((usr_index[A2D(i - 1, 1)] - usr_index[A2D(i - 1, 0)] + usr_index[A2D(i - 1, 2)]) /
                                 usr_index[A2D(i - 1, 2)]) *
                        usr_offset[i - 1];
    }
    ...
    conv_bytes = write_bytes * (j2 - j1 + j3) / j3 * (k2 - k1 + k3) / k3 * (l2 - l1 + l3) / l3 * (m2 - m1 + m3) / m3 *
                 (n2 - n1 + n3) / n3 * (v2 - v1 + v3) / v3;
    ...
    if (nefis[set].daf_neutral == TRUE || nefis[set].dat_neutral == TRUE)
    {
        ...
        nefis_errno = convert_ieee(&vp, &cp, conv_bytes, elm_single_bytes, elm_type, from_xdr);
    }
    else
    {
        cp = (BText)getal;
    ...
    for (v = v1 - 1; v < v2; v = v + v3)
    {
        if (var_dim == TRUE)
        {
            nefis_errno = GP_variable_pointer(set, &grp_pointer, v, &var_file_offset);
            ...
            {
                next_eof = prev_eof + cel_num_bytes * (BUInt8)grp_dimens[0] * (BUInt8)grp_dimens[1] *
                                          (BUInt8)grp_dimens[2] * (BUInt8)grp_dimens[3] * (BUInt8)grp_dimens[4];
                if (nefis[set].one_file == TRUE)
                {
                ...
                for (l = l1 - 1; l < l2; l = l + l3)
                {
                    ...
                    for (k = k1 - 1; k < k2; k = k + k3)
                    {
                        ...
                        for (j = j1 - 1; j < j2; j = j + j3)
                        {
                            ...
                            if (var_dim == TRUE)
                            {
                                cel_pointer = var_file_offset + cel_num_bytes * j_file_offset;
                            }
                            else
                            {
                                cel_pointer =
                                    grp_pointer + (BUInt8)size_dat_buf_on_file + cel_num_bytes * j_file_offset;
                            }
                            ...
                            if (cel_num_bytes == write_bytes && j3 == 1 && element_offset == 0 && array_offset[0] == 1)
                            {
                                ...
                                (BVoid) GP_write_file(dat_fds, cp + j_array_offset * write_bytes, l_pointer,
                                                      write_bytes_seq);
                                j += (j2 - j1 + 1);
                            }
                            ...
                            else if (((BRea4)n_elems_j) / step_size_elm > 5.f)
                            {
                                ...
                                while (elems_to_write > 0)
                                {
                                    ...
                                    for (i = 0; i < elems_per_loop && elems_to_write > 0; i++, elems_to_write--)
                                    {
                                        ...
                                        {
                                            cp_cont[i * cel_num_bytes * j3 + zz] =
                                                ((char*)cp)[j_array_offset * write_bytes + zz];
                                        }
                                        j_array_offset += array_offset[0];
                                        elems_written_loop += 1;
                                    }

                                    GP_write_file(dat_fds, cp_cont, l_pointer,
                                                  ((elems_written_loop - 1) * j3 + 1) * cel_num_bytes);
                            ...
                            {
                                (BVoid)
                                    GP_write_file(dat_fds, cp + j_array_offset * write_bytes, l_pointer, write_bytes);
                                j_array_offset += array_offset[0];
                            }
                        }
