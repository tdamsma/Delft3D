 ...
 *                 != 0  Error occured
 */
BInt4 HS_check_ecg(BInt4 set, BInt4 fds, BText key, BUInt8 nil, BUInt8* hashtb, BInt4 lhshtb, BUInt8* pnt_hash,
                   BUInt4* pnt_bucket, BUInt4* new_entry)
{
    union pntr
    {
    ...
    *pnt_bucket = (BUInt4)BUINT8_MAX;
    ...
    if (bucket == -1)
    {
        nefis_errcnt += 1;
        nefis_errno = 7001;
        sprintf(error_text, "No entry found in hash table for key \'%s\'", key);
        return nefis_errno;
    }
    ...
    else
    {
        ...
        while (found == 0)
        {
            ...
            if (n_read == -1)
            {
                nefis_errcnt += 1;
                nefis_errno = 7002;
                sprintf(error_text, "Unable to read next pointer (file write only?)");
                return nefis_errno;
            }
            if (n_read > (BInt4)sizeof(nxt_pnt.st))
            {
                nefis_errcnt += 1;
                nefis_errno = 7003;
                sprintf(error_text, "During reading of next pointer");
                return nefis_errno;
            }
...
/*==========================================================================*/
BInt4 HS_get_cont_cel(BInt4 set, BUInt8 pointer, BUInt8* next_pointer, BText cel_name, BText* elm_names,
                      BUInt4* cel_num_dim, BUInt8* cel_num_bytes)
{
    union cel
    {
        BChar* st;
    ...
    *cel_num_dim = cel_buf->in[index];
    ...
    if (n_read > (BInt4)size_cel_buf)
    {
        nefis_errcnt += 1;
        nefis_errno = 7004;
        sprintf(error_text, "During reading of cell structure.");
        return nefis_errno;
    }
...
/*==========================================================================*/
BInt4 HS_get_cont_elm(BInt4 set, BUInt8 pointer, BUInt8* next_pointer, BText elm_name, BText elm_type,
                      BText elm_quantity, BText elm_unity, BText elm_desc, BUInt4* elm_num_dim, BUInt4* elm_dimens,
                      BUInt4* elm_single_bytes, BUInt8* elm_num_bytes)
{
    union element
    {
    ...
    if (n_read > (BInt4)sizeof(elm.st))
    {
        nefis_errcnt += 1;
        nefis_errno = 7005;
        sprintf(error_text, "During reading of element structure");
        return nefis_errno;
    }
...
/*==========================================================================*/
BInt4 HS_get_cont_grp(BInt4 set, BUInt8 pointer, BUInt8* next_pointer, BText grp_name, BText cel_name,
                      BUInt4* grp_num_dim, BUInt4* grp_dimens, BUInt4* grp_order)
{
    union group
    {
    ...
    if (n_read > (BInt4)size)
    {
        nefis_errcnt += 1;
        nefis_errno = 7006;
        sprintf(error_text, "During reading of group structure");
        return nefis_errno;
    }
 ...
 *        1       Not succesful
 */
BInt4 HS_get_cont_dat(BInt4 set, BUInt8 pointer, BUInt8* next_pointer, BText grp_name, BText grp_defined)
{
    union dat_union
    {
    ...
    if (n_read > (BInt4)sizeof(data.st))
    {
        nefis_errcnt += 1;
        nefis_errno = 7007;
        sprintf(error_text, "During reading of data structure");
        return nefis_errno;
    }
...
/*==========================================================================*/
static BInt4 HS_hash_key(BText key, BUInt4 len_hash_table)
{
    /*
