...
/*==========================================================================*/
static BVoid C_Copy_text(BText name1, BText name2, BInt4 max_length)
{
    BInt4 i;
    BInt4 max_copy;
 ...
 *           !=0                Error occured
 */
DLLEXPORT BInt4 DEFINE_CELL(BInt4* fd, BText cl_name, BInt4 cl_num_dim, BChar el_names[][MAX_NAME + 1])
{
 ...
 *           !=0                Error occured
 */
DLLEXPORT BInt4 DEFINE_CELL2(BInt4* fd, BText cl_name, BInt4 cl_num_dim, BText* el_names)
{
    BChar cel_name[MAX_NAME + 1];
    BInt4 cel_num_dim;
    BInt4 i;
 ...
 *           !=0                Error occured
 */
DLLEXPORT BInt4 DEFINE_CELL3(BInt4* fd, BText cl_name, BInt4 cl_num_dim, BText el_names)
{
    BChar cel_name[MAX_NAME + 1];
    BInt4 cel_num_dim;
    BInt4 i;
...
DLLEXPORT BInt4 DEFINE_DATA(BInt4* fd, BText gr_name, BText gr_defined)
{
...
DLLEXPORT BInt4 DEFINE_ELEMENT(BInt4* fd, BText el_name, BText el_type, BInt4 el_single_byte, BText el_quantity,
                               BText el_unity, BText el_desc, BInt4 el_num_dim, BInt4* el_dimens)
{
    BInt4 elm_num_dim;
    ...
    nefis_errno = Define_element(*fd, elm_name, elm_type, elm_quantity, elm_unity, elm_desc, elm_single_byte,
...
DLLEXPORT BInt4 DEFINE_GROUP(BInt4* fd, BText gr_name, BText cl_name, BInt4 gr_num_dim, BInt4* gr_dimens,
                             BInt4* gr_order)
{
    BChar cel_name[MAX_NAME + 1];
    BChar grp_name[MAX_NAME + 1];
    ...
    nefis_errno = Define_group(*fd, grp_name, cel_name, grp_num_dim, grp_dimens, grp_order);
...
DLLEXPORT BInt4 CREATE_NEFIS(BInt4* fd, BText dat_file, BText def_file, BChar coding, BChar ac_type)
{
    nefis_errno = 0;
    ...
    if ((BInt4)strlen(dat_file) > (BInt4)(FILENAME_MAX - 1))
    {
        nefis_errcnt += 1;
        nefis_errno = 1002;
        sprintf(error_text, "Crenef: Data filename too long ( length < %d )", FILENAME_MAX - 1);
    }
    if ((BInt4)strlen(def_file) > (BInt4)(FILENAME_MAX - 1))
    {
        nefis_errcnt += 1;
        nefis_errno = 1003;
        sprintf(error_text, "Crenef: Definition filename too long ( length < %d )", FILENAME_MAX - 1);
    }

    if (nefis_errno == 0)
    {
        nefis_errno = create_nefis_files(fd, dat_file, def_file, coding, ac_type);
    }
...
DLLEXPORT BInt4 GET_DAT_HEADER(BInt4* fd, BText header)
{
    BUInt8 n_read = 0;
    ...
    if (((BInt4)strlen(header)) < LHDRDT)
    {
        nefis_errcnt += 1;
        nefis_errno = 1004;
        sprintf(error_text, "Gethdt: Supplied character string too small for header: %ld<%ld\n", strlen(header),
                LHDRDT);
        return nefis_errno;
    }
    ...
    if (nefis_errno == 0)
    {
        ...
        if (n_read == -1)
        {
            nefis_errcnt += 1;
            nefis_errno = 1005;
            sprintf(error_text, "Gethdt: Unable to read data file header (file write only?)");
        }
        if (n_read > LHDRDT)
        {
            nefis_errcnt += 1;
            nefis_errno = 1006;
            sprintf(error_text, "Gethdt: During reading of data file header");
        }
    }
...
DLLEXPORT BInt4 GET_DEF_HEADER(BInt4* fd, BText header)
{
    BUInt8 n_read = 0;
    ...
    if (((BInt4)strlen(header)) < LHDRDT)
    {
        nefis_errcnt += 1;
        nefis_errno = 1007;
        sprintf(error_text, "Gethdf: Supplied character string too small for header %ld<%ld\n", strlen(header), LHDRDF);
        return nefis_errno;
    }
    ...
    if (nefis_errno == 0)
    {
        ...
        if (n_read == -1)
        {
            nefis_errcnt += 1;
            nefis_errno = 1008;
            sprintf(error_text, "Gethdf: Unable to read definition file header (file write only?)");
        }
        if (n_read > LHDRDF)
        {
            nefis_errcnt += 1;
            nefis_errno = 1009;
            sprintf(error_text, "Gethdf: During reading of definition file header");
        }
    }
...
DLLEXPORT BInt4 GET_ELEMENT(BInt4* fd, BText gr_name, BText el_name, BInt4* user_index, BInt4* user_order,
                            BInt4* buffer_length, BData buffer)
{
    BChar elm_name[MAX_NAME + 1];
    BChar grp_name[MAX_NAME + 1];
    BInt4 i;
    ...
    nefis_errno = Get_element(*fd, grp_name, elm_name, usr_index, usr_order, *buffer_length, buffer);
...
DLLEXPORT BInt4 GET_ELEMENT_STRING(BInt4* fd, BText gr_name, BText el_name, BInt4* user_index, BInt4* user_order,
                                   BInt4* buffer_length, BData buffer)
{
    nefis_errno = 0;

    nefis_errno = GET_ELEMENT(fd, gr_name, el_name, user_index, user_order, buffer_length, buffer);
 ...
 *            !=0               Error occured
 */
DLLEXPORT BInt4 GET_INT_ATTRIBUTE(BInt4* fd, BText gr_name, BText at_name, BInt4* at_value)
{
    ...
    nefis_errno = GP_get_attribute(*fd, next, grp_name, att_name, at_value, att_type);
 ...
 *            !=0               Error occured
 */
DLLEXPORT BInt4 GET_REAL_ATTRIBUTE(BInt4* fd, BText gr_name, BText at_name, BRea4* at_value)
{
    BChar att_name[MAX_NAME + 1];
    BChar att_type[MAX_TYPE + 1];
    BChar grp_name[MAX_NAME + 1];
 ...
 *            !=0               Error occured
 */
DLLEXPORT BInt4 GET_STRING_ATTRIBUTE(BInt4* fd, BText gr_name, BText at_name, BText at_value)
{
    BChar att_name[MAX_NAME + 1];
    BChar att_type[MAX_TYPE + 1];
    BChar grp_name[MAX_NAME + 1];
...
DLLEXPORT BInt4 INQUIRE_CELL(BInt4* fd, BText cl_name, BInt4* cl_num_dim, BChar el_names[][MAX_NAME + 1])
{
    ...
    nefis_errno = GP_inquire_cel(*fd, cel_name, &cel_num_dim, &elm_names, &cel_num_bytes);
    ...
    if (nefis_errno == 0)
    {
        if (cel_num_dim > (BUInt4)*cl_num_dim)
        {
            nefis_errcnt += 1;
            nefis_errno = 1011;
            sprintf(error_text, "Inqcel: User supplied array too small to contain Cell properties: \'%s\' %ld>%ld \n",
...
DLLEXPORT BInt4 INQUIRE_CELL2(BInt4* fd, BText cl_name, BInt4* cl_num_dim, BText** el_names)
{
    BUInt8 cel_num_bytes = 0;
    BUInt4 cel_num_dim = 0;
    BChar cel_name[MAX_NAME + 1];
...
DLLEXPORT BInt4 INQUIRE_CELL3(BInt4* fd, BText cl_name, BInt4* cl_num_dim, BText el_names)
{
    BUInt8 cel_num_bytes = 0;
    BUInt4 cel_num_dim = 0;
    BChar cel_name[MAX_NAME + 1];
...
DLLEXPORT BInt4 INQUIRE_DATA_GROUP(BInt4* fd, BText gr_name, BText gr_defined)
{
    ...
    nefis_errno = GP_inquire_dat(*fd, &grp_pointer, grp_name, grp_defined);
...
DLLEXPORT BInt4 INQUIRE_ELEMENT(BInt4* fd, BText el_name, BText el_type, BInt4* el_single_bytes, BText el_quantity,
                                BText el_unity, BText el_desc, BInt4* el_num_dim, BInt4* el_dimens)
{
    ...
    nefis_errno = GP_inquire_elm(*fd, elm_name, elm_type, elm_quantity, elm_unity, elm_desc, &elm_num_dim, elm_dimens,
 ...
 *           !=0  Error occured
 */
DLLEXPORT BInt4 INQUIRE_FIRST_DAT_GROUP(BInt4* fd, BText gr_name, BText gr_defined)
{
    BChar grp_name[MAX_NAME + 1];
    BChar grp_defined[MAX_NAME + 1];
    BInt4 i;
                                      ...
                                      BInt4* el_dimens)      /* O */
{
    ...
    nefis_errno = GP_get_next_elm(*fd, 0, elm_name, elm_type, elm_quantity, elm_unity, elm_desc, &elm_num_dim,
                                  elm_dimens, &elm_single_byte, &elm_num_byte);
                                     ...
                                     BInt4* el_dimens)      /* O */
{
    ...
    nefis_errno = GP_get_next_elm(*fd, 1, elm_name, elm_type, elm_quantity, elm_unity, elm_desc, &elm_num_dim,
                                  elm_dimens, &elm_single_byte, &elm_num_byte);
                                   ...
                                   BChar el_names[][MAX_NAME + 1]) /* O */
{
    ...
    nefis_errno = GP_get_next_cell(*fd, 0, cel_name, &elm_names, &cel_num_dim, &cel_num_bytes);
    if (nefis_errno == 0)
    {
        if (cel_num_dim > (BUInt4)*cl_num_dim)
        {
            nefis_errcnt += 1;
            nefis_errno = 1013;
            sprintf(error_text, "Inqfcl: User supplied array too small to contain Cell properties: \'%s\' %ld>%ld \n",
                    cel_name, cel_num_dim, *cl_num_dim);
            return nefis_errno;
        }
                                  ...
                                  BChar el_names[][MAX_NAME + 1]) /* O */
{
    ...
    nefis_errno = GP_get_next_cell(*fd, 1, cel_name, &elm_names, &cel_num_dim, &cel_num_bytes);
    if (nefis_errno == 0)
    {
        if (cel_num_dim > (BUInt4)*cl_num_dim)
        {
            nefis_errcnt += 1;
            nefis_errno = 1013;
            sprintf(error_text, "Inqncl: User supplied array too small to contain Cell properties: \'%s\' %ld>%ld \n",
                    cel_name, cel_num_dim, *cl_num_dim);
            return nefis_errno;
        }
                                        ...
                                        BInt4* gr_order)   /* O */
{
    ...
    nefis_errno = GP_get_next_def_grp(*fd, 0, grp_name, cel_name, &grp_num_dim, grp_dimens, grp_order);
    if (nefis_errno == 0)
    {
        if (grp_num_dim > *gr_num_dim)
        {
            nefis_errcnt += 1;
            nefis_errno = 1015;
            sprintf(error_text, "Inqfgr: User supplied array too small to contain group properties: \'%s\' %ld>%ld \n",
                                       ...
                                       BInt4* gr_order)   /* O */
{
    ...
    nefis_errno = GP_get_next_def_grp(*fd, 1, grp_name, cel_name, &grp_num_dim, grp_dimens, grp_order);
    if (nefis_errno == 0)
    {
        if (grp_num_dim > *gr_num_dim)
        {
            nefis_errcnt += 1;
            nefis_errno = 1016;
            sprintf(error_text, "Inqngr: User supplied array too small to contain group properties: \'%s\' %ld>%ld \n",
                    grp_name, grp_num_dim, *gr_num_dim);
            return nefis_errno;
        }
 ...
 *           !=0  Error occured
 */
DLLEXPORT BInt4 INQUIRE_FIRST_INTEGER(BInt4* fd, BText gr_name, BText at_name, BInt4* at_value)
{
    BChar att_name[MAX_NAME + 1];
    BChar att_type[MAX_TYPE + 1];
    BChar grp_name[MAX_NAME + 1];
    ...
    nefis_errno = GP_get_attribute(*fd, 1, grp_name, att_name, at_value, att_type);
 ...
 *           !=0  Error occured
 */
DLLEXPORT BInt4 INQUIRE_FIRST_REAL(BInt4* fd, BText gr_name, BText at_name, BRea4* at_value)
{
    BChar att_name[MAX_NAME + 1];
    BChar att_type[MAX_TYPE + 1];
    BChar grp_name[MAX_NAME + 1];
 ...
 *           !=0  Error occured
 */
DLLEXPORT BInt4 INQUIRE_FIRST_STRING(BInt4* fd, BText gr_name, BText at_name, BText at_value)
{
    BChar att_name[MAX_NAME + 1];
    BChar att_type[MAX_TYPE + 1];
    BChar grp_name[MAX_NAME + 1];
...
DLLEXPORT BInt4 INQUIRE_GROUP(BInt4* fd, BText gr_defined, BText cl_name, BInt4* gr_num_dim, BInt4* grp_dimens,
                              BInt4* grp_order)
{
    BChar cel_name[MAX_NAME + 1];
    BChar grp_defined[MAX_NAME + 1];
    ...
    nefis_errno = GP_inquire_grp_def(*fd, grp_defined, cel_name, &grp_num_dim, (BUInt4*)grp_dimens, (BUInt4*)grp_order);
...
DLLEXPORT BInt4 INQUIRE_MAX_INDEX(BInt4* fd, BText gr_name, BInt4* max_index)
{
    BChar grp_name[MAX_NAME + 1];
 ...
 *           !=0  Error occured
 */
DLLEXPORT BInt4 INQUIRE_NEXT_DAT_GROUP(BInt4* fd, BText gr_name, BText gr_defined)
{
    BChar grp_name[MAX_NAME + 1];
    BChar grp_defined[MAX_NAME + 1];
    BInt4 i;
 ...
 *           !=0  Error occured
 */
DLLEXPORT BInt4 INQUIRE_NEXT_INTEGER(BInt4* fd, BText gr_name, BText at_name, BInt4* at_value)
{
    BChar att_name[MAX_NAME + 1];
    BChar att_type[MAX_TYPE + 1];
    BChar grp_name[MAX_NAME + 1];
    ...
    nefis_errno = GP_get_attribute(*fd, 2, grp_name, att_name, at_value, att_type);
 ...
 *           !=0         Error occured
 */
DLLEXPORT BInt4 INQUIRE_NEXT_REAL(BInt4* fd, BText gr_name, BText at_name, BRea4* at_value)
{
    BChar att_name[MAX_NAME + 1];
    BChar att_type[MAX_TYPE + 1];
    BChar grp_name[MAX_NAME + 1];
 ...
 *           !=0         Error occured
 */
DLLEXPORT BInt4 INQUIRE_NEXT_STRING(BInt4* fd, BText gr_name, BText at_name, BText at_value)
{
    BChar att_name[MAX_NAME + 1];
    BChar att_type[MAX_TYPE + 1];
    BChar grp_name[MAX_NAME + 1];
...
DLLEXPORT BInt4 OPEN_DAT_FILE(BInt4* fd, BText dt_file, BChar coding)
{
    BText dat_file;
    BUInt4 file_length;
 ...
 *            !=0               Error occured
 */
DLLEXPORT BInt4 OPEN_DEF_FILE(BInt4* fd, BText df_file, BChar coding)
{
    BText def_file;
    BUInt4 file_length;
...
DLLEXPORT BInt4 PUT_ELEMENT(BInt4* fd, BText gr_name, BText el_name, BInt4* user_index, BInt4* user_order, BData getal)
{
    BChar elm_name[MAX_NAME + 1];
    BChar grp_name[MAX_NAME + 1];
    BInt4 i;
    ...
    nefis_errno = Put_element(*fd, grp_name, elm_name, usr_index, usr_order, getal);
...
DLLEXPORT BInt4 PUT_ELEMENT_STRING(BInt4* fd, BText gr_name, BText el_name, BInt4* user_index, BInt4* user_order,
                                   BData getal)
{
    nefis_errno = 0;
    nefis_errno = PUT_ELEMENT(fd, gr_name, el_name, user_index, user_order, getal);
    return nefis_errno;
}
/*==========================================================================*/
/*
 ...
 *            !=0               Error occured
 */
DLLEXPORT BInt4 PUT_INT_ATTRIBUTE(BInt4* fd, BText gr_name, BText at_name, BInt4* at_value)
{
    BChar att_name[MAX_NAME + 1];
    BChar att_type[MAX_TYPE + 1];
    BData att_value;
    ...
    if (((BInt4)strlen(gr_name) > (BInt4)MAX_NAME) || ((BInt4)strlen(at_name) > (BInt4)MAX_NAME))
    {
        nefis_errcnt += 1;
        nefis_errno = 1019;
        sprintf(error_text, "Putiat: Groupname \'%s\' or\n        integer attribute name \'%s\' too long", gr_name,
                at_name);
        return nefis_errno;
    }
 ...
 *            !=0               Error occured
 */
DLLEXPORT BInt4 PUT_REAL_ATTRIBUTE(BInt4* fd, BText gr_name, BText at_name, BRea4* at_value)
{
    BChar att_name[MAX_NAME + 1];
    BChar att_type[MAX_TYPE + 1];
    BData att_value;
    ...
    if (((BInt4)strlen(gr_name) > (BInt4)MAX_NAME) || ((BInt4)strlen(at_name) > (BInt4)MAX_NAME))
    {
        nefis_errcnt += 1;
        nefis_errno = 1020;
        sprintf(error_text, "Putrat: Groupname \'%s\' or\n        real attribute name \'%s\' too long", gr_name,
                at_name);
        return nefis_errno;
    }
 ...
 *            !=0               Error occured
 */
DLLEXPORT BInt4 PUT_STRING_ATTRIBUTE(BInt4* fd, BText gr_name, BText at_name, BText at_value)
{
    BChar att_name[MAX_NAME + 1];
    BChar att_type[MAX_TYPE + 1];
