...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/d_hydro/packages/d_hydro/src/d_hydro.cpp
// $
//------------------------------------------------------------------------------
...
bool FLOW2D3D_MonolithicInit(DeltaresHydro* DH);
#endif
...
DeltaresHydro::DeltaresHydro(int argc, char* argv[], char* envp[])
{
    ...
    while ((c = getopt(argc, argv, (char*)"d:l:S:v?")) != -1)
    {
        ...
        {
            case 'd': {
                if (sscanf(optarg, "%i", &logMask) != 1) throw new Exception("Invalid log mask (-d option)");
                break;
            }

            case 'l': {
                logFile = fopen(optarg, "w");
                if (logFile == NULL) throw new Exception("Cannot create log file \"%s\"", optarg);
            ...
            default: {
                throw new Exception("Invalid command-line argument.  Execute \"%s -?\" for command-line syntax",
                                    this->exeName);
            }
        }
    }
    ...
    else
    {
        ...
        if (conf == NULL) throw new Exception("Cannot open configuration file \"%s\"", this->configfile);
    }
    ...
    if (handle == NULL) throw new Exception("Cannot load component library \"%s\": %s", library, dlerror());
    ...
    if (entryPoint == NULL)
        throw new Exception("Cannot find function \"%s\" in library \"%s\". Return code: %d", this->startEntry, library,
                            GetLastError());
