...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/varinfocoll.cpp
// $
//------------------------------------------------------------------------------
...
VarInfoCollection::VarInfoCollection(int maxBlockInfo, int maxVarInfo, int maxVarGroup)
{
...
BlockInfo* VarInfoCollection::AddBlockInfo(int offset, int size, int amount_D1, int stride_D1)
{
    BlockInfo* newBlockInfo = NULL;
...
BlockInfo* VarInfoCollection::AddBlockInfo(int offset, int size, int amount_D1, int stride_D1, int amount_D2,
                                           int stride_D2)
{
    BlockInfo* newBlockInfo = NULL;
...
BlockInfo* VarInfoCollection::AddBlockInfo(int offset, int size, int amount_D1, int stride_D1, int amount_D2,
                                           int stride_D2, int amount_D3, int stride_D3)
{
    BlockInfo* newBlockInfo = NULL;
...
BlockInfo* VarInfoCollection::AddBlockInfo(int offset, int size, int amount_D1, int stride_D1, int amount_D2,
                                           int stride_D2, int amount_D3, int stride_D3, int amount_D4, int stride_D4)
{
    BlockInfo* newBlockInfo = NULL;
...
void VarInfoCollection::AddVarInfo(int varId, int* varAddress, VarType varType, BlockInfo* blockInfo)
{
...
void VarInfoCollection::AddVarInfo(int varId, float* varAddress, VarType varType, BlockInfo* blockInfo)
{
    VarInfo* newVarInfo = NULL;
...
void VarInfoCollection::AddVarInfo(int varId, double* varAddress, VarType varType, BlockInfo* blockInfo)
{
    VarInfo* newVarInfo = NULL;
...
void VarInfoCollection::AddGroup(int groupId // group be added
)
{
...
int VarInfoCollection::GetNumBytes(int groupId // group for which #bytes must be determined
)
{
    ...
    else
    {
        ...
        {
            varGroup->numBytes = this->BufferGroup(groupId, BufferAction_GetSize, NULL, 0);
        }
...
void VarInfoCollection::AddVarToGroup(int groupId, // group to add var.to
...
VarInfo* VarInfoCollection::FindVar(int varId // var to be found
)
{
    VarInfo* retVal = NULL;
    int varIndex;
                                ...
                                if (VAR_TO_BE_PRINTED(varId)) {
                                    if (varInfo->varType == FloatType)
                                    {
                                        FLOW2D3D->dd->log->Write(
                                            Log::DDMAPPER_MINOR, " %s idx%5d, ofs=%5d, m%3d, n%3d (vId%3d, %12.8f)",
                                            actionStr, index, blockInfo->Offset, m, n, varId, *((float*)varAddress));
                                    }
                                    else if (varInfo->varType == DoubleType)
                                    {
                                        FLOW2D3D->dd->log->Write(
                                            Log::DDMAPPER_MINOR, " %s idx%5d, ofs=%5d, m%3d, n%3d (vId%3d, %12.8g)",
                                            actionStr, index, blockInfo->Offset, m, n, varId, *((double*)varAddress));
                                    }
                                    else if (varInfo->varType == IntegerType)
                                    {
                                        FLOW2D3D->dd->log->Write(
                                            Log::DDMAPPER_MINOR, " %s idx%5d, ofs=%5d, m%3d, n%3d (vId%3d, %12d)",
                                            actionStr, index, blockInfo->Offset, m, n, varId, *((int*)varAddress));
                                    }
                                    fflush(stdout);
                                })
                        }
...
)
{
    ...
    if (varGroup != NULL)
    {
        ...
        {
            nAddedOrReadBytes += this->BufferVar(varGroup->varId[v], bufferAction, buffer + nAddedOrReadBytes,
                                                 maxBytes - nAddedOrReadBytes);
            ON_LOG_EXCHANGE(if (VAR_TO_BE_PRINTED(varGroup->varId[v])) {
                FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, " Processed var: %d, #bytes %d", varGroup->varId[v],
                                         nAddedOrReadBytes);
            })
        }
...
void VarInfoCollection::PrintVarInfo(int varId // var to be printed
)
{
    VarInfo* varInfo = FindVar(varId);
    if (varInfo != NULL)
        FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "VarInfo: varId=%d varAddress=0x%x varType=%d blockInfo=%x",
                                 varInfo->varId, varInfo->varAddress, varInfo->varType, varInfo->blockInfo);
}
...
void VarInfoCollection::InitBlockInfo(BlockInfo* blockInfo // block admin. info to be inited
)
{
    int dim;
...
void VarInfoCollection::InitVarInfo(VarInfo* varInfo // var-info to be inited
)
{
    varInfo->varId = -1;
...
void VarInfoCollection::InitVarGroup(VarGroup* varGroup // group to be inited
)
{
    varGroup->groupId = -1;
...
VarGroup* VarInfoCollection::FindGroup(int groupId // group to be found
)
{
...
void VarInfoCollection::PrintGroupInfo(int groupId // group to be printed
)
{
