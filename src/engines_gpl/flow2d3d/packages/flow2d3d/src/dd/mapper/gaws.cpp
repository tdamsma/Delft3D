...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/gaws.cpp
// $
//------------------------------------------------------------------------------
...
int Gaws::Setup(Iterator* gaws,                  // GAWS iterator
...
)
{
...

    {
        ...
        {
            FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "GAWS \"%s\" fills mapper cells for mapper %d", gaws->name,
                                     m);

            retVal = this->FillMapperCells(&(this->mapCells[m]), this->mapInfo[m]);

            ON_LOG_VERBOSE(printf("%d Ncb %d Nce %d Mcb %d Mce %d Nnb %d Nne %d Mnb %d Mne %d \n",
                                  mapCells[m].leftToRight, mapCells[m].nCurrBegin, mapCells[m].nCurrEnd,
                                  mapCells[m].mCurrBegin, mapCells[m].mCurrEnd, mapCells[m].nNbrBegin,
                                  mapCells[m].nNbrEnd, mapCells[m].mNbrBegin, mapCells[m].mNbrEnd);)
        }
    }
...

    {
        for (int p = 0; (p < this->nProc) && (retVal == HY_OK); p++)
        {
            ...
            this->contexts[p]->Setup(procRelations[p].gawsBarrier, procRelations[p].flowIterator,
    ...
    while (!finished)
    {
...

        {
            for (int p = 0; p < this->nProc; p++)
            {
                ...
                this->contexts[p]->flowIterator->Receive(leftToRightBlob,
                                                         &incomingBlobID); // DDD-COMM: FLOW->GAWS, left2right message
                delete leftToRightBlob;
                if (incomingBlobID != F2G_Blob_Left2RightMessage)
                {
                    throw new Exception("Gaws::Setup: DoSolving blobID (%d /= %d)", incomingBlobID,
                                        F2G_Blob_Left2RightMessage);
                }
        ...
        else
        {
...

            {
                for (int p = 0; p < this->nProc; p++)
                {
                    ...
                    this->contexts[p]->flowIterator->Send(okMesgBlob,
                                                          G2F_Blob_OKMessage); // DDD-COMM: GAWS->FLOW, ok message
...
void Gaws::Solver(bool leftToRight // boolean L->R or Top->Bottom
)
{
    ...
    if (leftToRight)
    {
        ON_LOG_WANG(printf("Start Wang-X (%x) solver\n", this->wangEquations_X); fflush(stdout);)

        for (rLev = 0; rLev <= maxRefineLevel[Wang_XDir]; rLev++)
        {
            ON_LOG_WANG(printf("Refinement level: %d\n", rLev);)
        ...
        {
            ON_LOG_WANG(printf("X-Tridiag-Refinement level: %d\n", rLev);)
    ...
    {
        ON_LOG_WANG(printf("Start Wang-Y (%x) solver\n", this->wangEquations_Y); fflush(stdout);)
        ...
        {
            ON_LOG_WANG(printf("Y-Tridiag-Refinement level: %d\n", rLev);)
...
int Gaws::FillMapperCells(MapperCells* cells, // mapper cells info
...
)
{
    ...
    if (ParseMapperConfigString(mapper.configStr, edgeType, firstCell, lastCell, normalCell, refine, 0) != HY_OK)
    {
    ...
    else
    {
        for (ctx = 0; ctx < NR_CNTXTS; ctx++)
        {
            if (firstCell[ctx] == YET_TO_INIT || firstCell[ctx] == 0 || lastCell[ctx] == YET_TO_INIT ||
                lastCell[ctx] == 0 || normalCell[ctx] == YET_TO_INIT || normalCell[ctx] == 0)
            {
        ...
        ON_LOG_VERBOSE(printf("edgeType[C_0] %d edgeType[C_1] %d\n", edgeType[C_0], edgeType[C_1]);
                       printf("Ncb %d Nce %d Mcb %d Mce %d Nnb %d Nne %d Mnb %d Mne %d \n", cells->nCurrBegin,
                              cells->nCurrEnd, cells->mCurrBegin, cells->mCurrEnd,

                              cells->nNbrBegin, cells->nNbrEnd, cells->mNbrBegin, cells->mNbrEnd);)
    }
    return retVal;
}

int Gaws::FindNeighborPoint(bool leftToRight,                 // boolean L->R or Top->Bottom
                            D3dFlowContextGawsSide* curCntxt, // context of current domain
                            ...
                            D3dFlowContextGawsSide** nbrCntxt // context of neighbor domain
)
{
    ...
    *nbrCntxt = NULL;
    ...
    for (map = 0; map < this->numMappers; map++)
    {
        ...
        if (mapCells[map].leftToRight == leftToRight)
        {
            if (curCntxt->flowIterator->id == mapCells[map].mapInfo.flowIterator[C_0]->id)
            {
                ...
                if ((cM >= mapCells[map].mCurrBegin && cM <= mapCells[map].mCurrEnd) &&
                    (cN >= mapCells[map].nCurrBegin && cN <= mapCells[map].nCurrEnd))
                {
                    if (leftToRight)
                    {
                        *nbrCell = mapCells[map].mNbrEnd;
                        if (mapCells[map].curRefine >= mapCells[map].nbrRefine)
                        {
                            *nbrLine = mapCells[map].nNbrBegin +
                                       (curLine - mapCells[map].nCurrBegin) / mapCells[map].curRefine;
                        }
                        else
                        {
                            *nbrLine = mapCells[map].nNbrBegin +
                                       (curLine - mapCells[map].nCurrBegin) * mapCells[map].nbrRefine;
                        }
                    }
                    else
                    {
                        *nbrCell = mapCells[map].nNbrEnd;
                        if (mapCells[map].curRefine >= mapCells[map].nbrRefine)
                        {
                            *nbrLine = mapCells[map].mNbrBegin +
                                       (curLine - mapCells[map].mCurrBegin) / mapCells[map].curRefine;
                        }
                        else
                        {
                            *nbrLine = mapCells[map].mNbrBegin +
                                       (curLine - mapCells[map].mCurrBegin) * mapCells[map].nbrRefine;
                        }
                    }
    ...
    ON_LOG_VERBOSE(int nbrCID = (*nbrCntxt != NULL) ? (*nbrCntxt)->GetContextId() : -999;
                   printf("FN(%3d): %6d(%3d,%3d) -> %6d(%3d,%3d)\n", retVal, curCntxt->GetContextId(), curCell, curLine,
                          nbrCID, *nbrCell, *nbrLine);)

    return retVal;
}

int Gaws::CreateWangAdministration(WangDir wangDir // Wang direction (X or Y)
)
{
    int retVal = HY_OK;               // return value
    ...
    ON_LOG_WANG(printf("Begin routine create wang-administration\n"); fflush(stdout);)
    ...
    for (p = 0; (p < this->nProc) && (retVal == HY_OK); p++)
    {
        ...
        ON_LOG_VERBOSE(printf("Max[%d]=%d,%d\n", p, cellMax, lineMax); fflush(stdout);)
                ...
                if ((kcs == 1 || kcs == 2) && ((kcsDown == 0) || (kcsDown == 2)))
                {
                    for (cell = cell + 1; (cell <= cellMax) && (kcs == 1 || kcs == 2); cell++)
                    {
                        ...
                        if (kcs == 3)
                        {
                            ...
                            ON_LOG_VERBOSE(printf("newSerie (kcs=3): %d\n", eqSerie);)

                            retVal = ProcessCouplePoint(curCntxt, leftToRight, eqSerie, cell, line);
                        }
                    }
                    cell--;
                }
    ...
    ON_LOG_WANG(printf("CreateWangAdministration(%d) RetVal: %d\n", wangDir, retVal);)

    return retVal;
}

int Gaws::ContinueInNextContext(int eqSerie,                     // equation serie number
                                int curCell,                     // cell-index in this next domain
                                int curLine,                     // line-index  "  "    "      "
                                D3dFlowContextGawsSide* curCntxt // context of  "    "      "
)
{
    bool leftToRight;   // boolean L->R or Top->Bottom
    int retVal = HY_OK; // return value
    ...
    ON_LOG_VERBOSE2(printf("KCS: %d at line=%d,cell=%d\n", kcs, curLine, curCell); fflush(stdout);)
        ...
        for (cell = curCell; (cell <= cellMax) && (kcs != 0); cell++)
        {
            ...
            if (kcs == 1 || kcs == 2)
            {
                for (cell = cell + 1; (cell <= cellMax) && (kcs == 1 || kcs == 2); cell++)
                {
                    ...
                    if (kcs == 3)
                    {
                        retVal = ProcessCouplePoint(curCntxt, leftToRight, eqSerie, cell, curLine);
                    }
                    else if (kcs == 0 || kcs == 2)
                    {
                        AddEqPoint(eqSerie, curCntxt, (kcs == 0) ? cell - 1 : cell, curLine, NULL, 0, 0, Pnt_EndLine,
                                   0);
                    }
                }
                cell--;
...
int Gaws::CreateWangEquations(WangEquations* Wang_Eq_XorY, // pointer Wang Equations context
...
)
{
    ...
    ON_LOG_WANG(printf("Begin routine create wang-equations \n"); fflush(stdout);)

    leftToRight = (wangDir == Wang_XDir) ? true : false;

    for (s = 0; s < nSeries; s++)
    {
        if ((eqSeries[s].leftToRight == leftToRight) && (eqSeries[s].level == level))
        {
            for (p = 0; p < eqSeries[s].nPnt; p++)
            {
                ...
                //
                BeginOfLine = false;
                Wang_Eq_XorY->AddEquation(leftToRight, curM, curN, coupCurM, coupCurN, curCntxt, BeginOfLine);
                ...
                {
                    BeginOfLine = true;
                    Wang_Eq_XorY->AddEquation(leftToRight, nbrM, nbrN, coupNbrM, coupNbrN, nbrCntxt, BeginOfLine);
                }
...
void Gaws::GawsLog(char* format, // I: 'fprintf-format' for print of log
...
int Gaws::CheckDependents(D3dFlowContextGawsSide* curCntxt, D3dFlowContextGawsSide* nbrCntxt, bool leftToRight,
                          int myLine, int* centreLine)
{
    int retVal = -999;
    ...
    ON_LOG_VERBOSE(printf("mapCells[%2d].refines for line %d: %d,%d -> %d\n", map, myLine, mapCells[map].curRefine,
                          mapCells[map].nbrRefine, retVal);)

    return retVal;
}

int Gaws::GetCentreLine(int map, int myLine, int ctx)
{
    int line, refine, start[NR_CNTXTS];

    refine = (ctx == C_0) ? mapCells[map].curRefine : mapCells[map].nbrRefine;
    ...
    ON_LOG_VERBOSE2(printf("mapCells[%2d].centreLine for line %d = %d (ctx=%d)\n", map, myLine, line, ctx);)

    return line;
}

int Gaws::NewEquationSerie(bool leftToRight, int level)
{
...
void Gaws::AddEqPoint(int eqSerie, D3dFlowContextGawsSide* curCntxt, int curCell, int curLine,
                      D3dFlowContextGawsSide* nbrCntxt, int nbrCell, int nbrLine, PntType type, int numDependents)
{
    //
    ...
    if (eqSeries[eqSerie].nPnt % EQ_PNT_BLOCK_SIZE == 0)
    {
        ...
        eqSeries[eqSerie].pnt = (EqPoint*)realloc(eqSeries[eqSerie].pnt, sizeof(EqPoint) * nNewPoints);
    ...
    ON_LOG_VERBOSE(printf("\nEqPnt added(%3d):  Cid %5d, C=%2d, L=%2d, #dep=%d\n", eqSerie, curCntxt->GetContextId(),
                          curCell, curLine, numDependents);
                   printf("EqPnt added     : nCid %5d,nC=%2d,nL=%2d, type=%d\n\n",
                          (nbrCntxt != NULL) ? nbrCntxt->GetContextId() : -999, nbrCell, nbrLine, type);)
    ...
    if (type != Pnt_EndLine) curCntxt->AddCommPointAdmin(mVirtIndex, nVirtIndex, eqSeries[eqSerie].leftToRight);
    ...
    if (nbrCntxt != NULL)
    {
        ...
        if (type != Pnt_EndLine) nbrCntxt->AddCommPointAdmin(mVirtIndex, nVirtIndex, eqSeries[eqSerie].leftToRight);
    }
...
void Gaws::SetDependentLevels(int curSer // Current equation Series
)
{
    ...
    for (p = 0; (p < eqSeries[curSer].nPnt); p++)
    {
        ...
        ON_LOG_VERBOSE(printf("serie[%d], #dep[%d]=%d\n", curSer, p, numDep);)
        ...
        if (numDep != 0)
        {
            ...
            ON_LOG_VERBOSE(
                printf("%d(%d,%d) %s\n", curCntxt->GetContextId(), curCell, centreLine, (numDep > 0) ? "F2C" : "C2F");)

            ref = (numDep >= 0) ? numDep : -numDep;

            for (curLine = centreLine - ref / 2; curLine <= centreLine + (ref - 1) / 2; curLine++)
            {
                if (curLine != centreLine)
                {
                    ...
                    {
                        ON_LOG_VERBOSE(printf("   OS1: %d\n", othSer);)

                        if ((othSer != curSer) && (eqSeries[othSer].level != 0) &&
                            (eqSeries[othSer].leftToRight == eqSeries[curSer].leftToRight))
                        {
                            ON_LOG_VERBOSE(printf("   OS2: %d\n", othSer);)

                            for (oP = 0; (oP < eqSeries[othSer].nPnt) && !found; oP++)
                            {
                                if (numDep > 0)
                                {
                                    oCell = eqSeries[othSer].pnt[oP].cell;
                                    oLine = eqSeries[othSer].pnt[oP].line;
                                    oCntxt = eqSeries[othSer].pnt[oP].curCntxt;
                                }
                                else
                                {
                                    oCell = eqSeries[othSer].pnt[oP].nbrCell;
                                    oLine = eqSeries[othSer].pnt[oP].nbrLine;
                                    oCntxt = eqSeries[othSer].pnt[oP].nbrCntxt;
                                }
                                if (oCell == curCell && oLine == curLine && oCntxt == curCntxt)
                                {
                                    found = 1;
                                    refSerie = othSer;

                                    ON_LOG_VERBOSE(printf("         %d(%d,%d)\n",
                                                          (oCntxt != NULL) ? oCntxt->GetContextId() : -999, oCell,
                                                          oLine);)
                                }
                            }
                        }
                    ...
                    if (found)
                    {
                        ...
                        maxRefineLevel[eqSeries[refSerie].leftToRight] =
                            B_MAX(maxRefineLevel[eqSeries[refSerie].leftToRight], eqSeries[refSerie].level);

                        ON_LOG_VERBOSE(printf("OtherS to refine: %d (level %d)\n", refSerie, eqSeries[refSerie].level);)

                        SetDependentLevels(refSerie);
                    }
                    else
                    {
                        fprintf(stderr, "ERROR in SetDependentLevels (serie: %d)\n", curSer);
                        success = 0;
                    }
                }
            }
...
void Gaws::RefineSeries(void)
{
    ...
    {
        ON_LOG_VERBOSE(
            printf("Refine Serie[%3d], level %d (has dep: %d)\n", s, eqSeries[s].level, eqSeries[s].hasDependents);)
    ...
    ON_LOG_WANG(printf("maxRefineLevels: LTR %d B2T %d \n", maxRefineLevel[1], maxRefineLevel[0]);)

    ON_LOG_VERBOSE2(for (l = 0; l < B_MAX(maxRefineLevel[1], maxRefineLevel[0]); l++) {
        for (s = 0; (s < nSeries); s++)
        {
            if (eqSeries[s].level == l)
            {
                printf("Ready Serie[%3d], level %d (%s)\n", s, l, eqSeries[s].leftToRight ? "L2R" : "B2T");
                for (p = 0; (p < eqSeries[s].nPnt); p++)
                {
                    printf("           %5d(%3d,%3d) -> %5d(%3d,%3d)\n",
                           (eqSeries[s].pnt[p].curCntxt != NULL) ? eqSeries[s].pnt[p].curCntxt->GetContextId() : -999,
                           eqSeries[s].pnt[p].cell, eqSeries[s].pnt[p].line,
                           (eqSeries[s].pnt[p].nbrCntxt != NULL) ? eqSeries[s].pnt[p].nbrCntxt->GetContextId() : -999,
                           eqSeries[s].pnt[p].nbrCell, eqSeries[s].pnt[p].nbrLine);
                }
            }
        }
    })
...
int Gaws::ProcessCouplePoint(D3dFlowContextGawsSide* curCntxt, bool leftToRight, int eqSerie, int curCell, int curLine)
{
    ...
    retVal = this->FindNeighborPoint(leftToRight, curCntxt, curCell, curLine, &nbrCell, &nbrLine, &nbrCntxt);
    ...
    numDep = CheckDependents(curCntxt, nbrCntxt, leftToRight, curLine, &centreLine);
    ...
    if (numDep >= 0)
    {
        ...
        if (curLine == centreLine)
        {
            AddEqPoint(eqSerie, curCntxt, curCell - 1, curLine, nbrCntxt, nbrCell, nbrLine, Pnt_Regular, numDep);

            retVal = this->ContinueInNextContext(eqSerie, nbrCell, nbrLine, nbrCntxt);
            if (numDep > 0)
            {
        ...
        {
            AddEqPoint(eqSerie, curCntxt, curCell - 1, curLine, nbrCntxt, nbrCell, nbrLine, Pnt_EndRefine, 0);
    ...
    else
    {
        ...
        //
        for (ll = centreLine + numDep / 2; ll <= centreLine - (numDep + 1) / 2; ll++)
        {
            if (ll == centreLine)
            {
                AddEqPoint(eqSerie, curCntxt, curCell - 1, curLine, nbrCntxt, nbrCell, ll, Pnt_Regular, numDep);
            ...
            else
            {
                ...
                ON_LOG_VERBOSE(printf("newSerie (nbrFine): %d\n", newSerie);)

                AddEqPoint(newSerie, curCntxt, curCell - 1, curLine, nbrCntxt, nbrCell, ll, Pnt_StartRefine, 0);

                retVal = ContinueInNextContext(newSerie, nbrCell, ll, nbrCntxt);
            }
        }
    }
    return retVal;
...
void Gaws::InitializeMapperInfo(void)
{
            ...
            for (int m = 0; m < numFlowMappers; m++)
            {
...

                {
                    for (int map = 0; (map < this->numMappers) && (!allReadyInMapInfo); map++)
                        ...
                        {
                            FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "GAWS \"%s\": mapper \"%s\" already added",
                ...
                if (!allReadyInMapInfo)
                {
                    ...
                    else
                    {
                        ...
                        FLOW2D3D->dd->log->Write(
                            Log::DDMAPPER_MINOR,
                            "GAWS \"%s\" added mapper \"%s\" (flow \"%s\" -> \"%s\"), configString \"%s\"",
                            this->gawsIterator->name, mapper->name, this->mapInfo[curM].flowIterator[C_0]->name,
                            this->mapInfo[curM].flowIterator[C_1]->name, this->mapInfo[curM].configStr);
    ...
    if (this->numMappers < 1)
        ...
        FLOW2D3D->dd->log->Write(Log::DDMAPPER_MAJOR, "GAWS \"%s\" found %d mappers", this->gawsIterator->name,
                                 this->numMappers);
}
