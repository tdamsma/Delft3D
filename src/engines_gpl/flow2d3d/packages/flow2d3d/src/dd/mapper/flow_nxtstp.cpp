...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/flow_nxtstp.cpp
// $
//------------------------------------------------------------------------------
...
void FlowDD_Process(Iterator* self, const char* name, Blob* configblob)
{
    ...
    if (mappers == NULL) throw new Exception("Mapper category does not exist for D3dFlow \"%s\"", name);
        ...
        for (int m = 0; m < numNeighborMappers; m++)
        {
            ...
            else
            {
                ...
                FLOW2D3D->dd->log->Write(Log::DDMAPPER_MAJOR, "FLOW \"%s\" stored contextID %d for mapper \"%s\"",
                                         mapperRelation[numMappers].self->name, mapperRelation[numMappers].esmContextID,
    ...
    if (gawsBarrier != NULL)
        ...
        throw new Exception("FLOW, gawsBarrier NOT found for FLOW \"%s\"", IteratorSelf()->name);
    ...
    else
    {
        ...
        FLOW2D3D->dd->log->Write(Log::DDMAPPER_MAJOR, "FLOW \"%s\" stored contextID %d for gaws \"%s\"",
                                 gawsRelation[nGawsRelations].self->name, gawsRelation[nGawsRelations].esmContextID,
    ...
    int initOnly = 0;
    TRISIM(&FLOW2D3D->dd->numSubdomains, &numNeighborMappers, &esmContextID, &esmfsm_flags, name, &initOnly,
           &FLOW2D3D->gdp, strlen(name));
}
...
void STDCALL GWSSLV(int* leftToRight // 1 = L2R (X-dir), 0 = B2T (Y-dir), -1 = finish
)
{
    ...
    if (gawsBarrier == NULL)
    {
        throw new Exception("FLOW: GWSSLV, gawsBarrier found for flow \"%s\"", IteratorSelf()->name);
    }
...

    {
        for (int g = 0; g < nGawsRelations; g++)
        {
            if (gawsRelation[g].self == IteratorSelf())
            {
                ...
                gawsRelation[g].gawsBarrier->Send(
                    mOutBlob, F2G_Blob_Left2RightMessage); // DDD-COMM: FLOW->GAWS, left2right message
                delete mOutBlob;
                ...
                else
                {
                    ...
                    //
                    gawsRelation[g].contextFlowSide->UpdateFlowToGaws(
                        GawsDistrib_AllVars); // DDD-COMM: FLOW->GAWS, Solver Update
                }
            }
        }
    }
...

    {
        for (int g = 0; g < nGawsRelations; g++)
        {
            if (gawsRelation[g].self == IteratorSelf())
            {
                ...
                {
                    throw new Exception("FLOW: GWSSLV: unexpected blobID (%d /= %d)", incomingBlobID,
                                        G2F_Blob_OKMessage);
                }
    ...
    ON_LOG_STEPS(printf("GWSSLV: Returned from barrier(%d)\n", *leftToRight); fflush(stdout);)
...
int STDCALL COMMUNICATENEXTSTEPWITHMAPPER(int* ptr_myStep, /* Completed DELFT3D-FLOW step  */
...
)
{
...

        {
            for (int m = 0; m < numMappers; m++)
                if (mapperRelation[m].self == IteratorSelf())
                    mapperRelation[m].contextFlowSide->Setup(mapperRelation[m].self, mapperRelation[m].mapper,
            ...
            for (int m = 0; m < numMappers; m++)
            {
                if (mapperRelation[m].self == IteratorSelf())
                {
                    ...
                    FLOW2D3D->dd->log->Write(
                        Log::DDMAPPER_MINOR,
                        "FLOW NEXTSTEP to Mapper \"%s\": myStep %s, neighborStep %s, nOutMessages %d",
                        mapperRelation[m].contextFlowSide->mapperIterator->name, PrintNextStepName(myStep),
                        PrintNextStepName(neighborStep), nOutMessages);
                    ...
                    updateHeader.intValue = -1;
                    if (nOutMessages == 1) updateHeader.intValue = outMessage.value;
        ...
        for (int m = 0; m < numMappers; m++)
        {
            if (mapperRelation[m].self == IteratorSelf())
            {
                ...
                FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR,
                                         "FLOW NEXTSTEP from Mapper \"%s\": nextStep %s, nInMessages %d",
    ...
    if (myStep == D3dFlow_Init)
    {
...

        {
            for (int g = 0; g < nGawsRelations; g++)
                if (gawsRelation[g].self == IteratorSelf())
                    gawsRelation[g].contextFlowSide->SetupForGaws(gawsRelation[g].self, gawsRelation[g].gawsBarrier,
                                                                  gawsRelation[g].esmContextID,
                                                                  gawsRelation[g].memType);
        }
    ...
    ON_LOG_STEPS(FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOW NEXTSTEP RETURNS: cur %s nn %s nxt %s",
                                          PrintNextStepName(myStep), PrintNextStepName(neighborStep),
                                          PrintNextStepName(nextStep));)

    if (nextStep == -1) throw new Exception("FLOW: CommunicateNextStepWithMapper: nextStep undefined");
...
static int D3dFlowDetermineMapperStep(int curStep /* Just completed DELFT3D-FLOW step */
)
{
...
char* PrintNextStepName(int step)
{
    ...
    else
    {
        ...
        else
        {
            if (!initialized)
            {
                for (int i = 0; i < D3DFLOW_NR_STEPS; i++) stepName[i] = (char*)"Step Not Known";
...
MapDistribGroup DetermineDistribGroup(int step)
{
    ...
    else
    {
        if (step >= 0 && step < D3DFLOW_NR_STEPS)
        {
            if (!initialized)
            {
                for (int i = 0; i < D3DFLOW_NR_STEPS; i++) groups[i] = MapDistrib_NoGroup;
