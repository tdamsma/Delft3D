...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/context_general_functs.cpp
// $
//------------------------------------------------------------------------------
...
void D3dFlowContext::AttachVars(int cntxtId // context Id
)
{
...
int D3dFlowContext::CreateContextVar(void** var, char* name, int size)
{
    int retVal = HY_ERR;
    void* ptr = NULL;
...
void D3dFlowContext::SetOtherContext(D3dFlowContext* otherC // pointer to other context
)
{
    ...
    if (kMax > oC->kMax)
    {
        ...
        ON_DEBUG(DBLEV7, CMapLog("Refined context, this->refUp: %d\n", this->refUp);)
    }
}

int D3dFlowContext::AttachContextVar(int aContextId, void** var, char* name)
{
    int retVal = HY_ERR;
    void* ptr = NULL;
