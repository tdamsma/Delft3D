...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/context-flowside.cpp
// $
//------------------------------------------------------------------------------
...
int m;
    #define DUMP_2DARR(arr, arrName, text)                                                                        \
        FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "DUMP_2DARR flow \"%s\", mOffSet %d, nOffset %d",           \
                                 this->flowIterator->name, this->mArrayOffset, this->nArrayOffset);               \
        FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "DUMP_2DARR \"%s\" flow \"%s\"", text,                      \
...
D3dFlowContextFlowSide::D3dFlowContextFlowSide(void) { MAPDBG_FUN2("D3dFlowContextFlowSide::D3dFlowContextFlowSide"); }
...
int D3dFlowContextFlowSide::Setup(Iterator* flow,   // flow process
...
)
{
    ...
    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOW \"%s\" SENDS contextID %d to mapper \"%s\"",
                             this->flowIterator->name, this->contextID, this->mapperIterator->name);
...
int D3dFlowContextFlowSide::SetupForGaws(Iterator* flow,  // flow process
...
)
{
    ...
    if (aMemType == Mem_Distributed)
    {
        ...
        FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOW \"%s\" SENDS KCS to gaws \"%s\"", this->flowIterator->name,
        ...
        FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOW \"%s\" HAS SENT KCS (%d*%d)to gaws \"%s\"",
                                 this->flowIterator->name, this->mArraySize, this->nArraySize,
                                 this->gawsIterator->name);
    }
...
void D3dFlowContextFlowSide::ReceiveAndCreateCommPoints(void)
{
    ...
    if (this->memType == Mem_Distributed)
    {
        ...
        {
            throw new Exception("D3dFlowContextFlowSide::SetupForGaws: unexpected blobID (%d /= %d)", incomingBlobID,
                                G2F_Blob_NumCommPoints);
        }
        ...
        if (this->numCommPoints > 0)
        {
            ...
            int incomingBlobID = 0;
            Blob* commPointsBlob = new Blob(this->commPointAdmins, this->numCommPoints * sizeof(GawsCommPointAdmin));
...
void D3dFlowContextFlowSide::UpdateFlowToMapper(UpdateHeader& updateHeader)
{
    ...
    if (this->memType == Mem_Distributed)
    {
        ...
        else
        {
            ...
            int numBytes = this->varInfoColl->BufferGroup(distribGroup, BufferAction_Fill, buffer + headerSize,
                                                          numDataBytesToBeSent);

            if (numBytes != numDataBytesToBeSent)
                throw new Exception("FLOW \"%s\" to mapper \"%s\": numBytes(%d) != numDataBytesToBeSent(%d)",
                                    this->flowIterator->name, this->mapperIterator->name, numBytes,
                                    numDataBytesToBeSent);
...
void D3dFlowContextFlowSide::UpdateFlowFromMapper(UpdateHeader& updateHeader)
{
    ...
    if (this->memType == Mem_Distributed)
    {
        ...
        else
        {
            ...
            FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR,
                                     "FLOW \"%s\" WAITS for Update (max %d bytes) from mapper \"%s\"",
            ...
            FLOW2D3D->dd->log->Write(
                Log::DDMAPPER_MINOR,
                "FLOW \"%s\" RECEIVED Update from mapper \"%s\" (step %d, #mess %d, numInBytes %d)",
                this->mapperIterator->name, this->flowIterator->name, updateHeader.nextStep, updateHeader.numMessages,
                numInBytes);
            ...
            int numReadBytes =
                this->varInfoColl->BufferGroup(distribGroup, BufferAction_Read, buffer + headerSize, numInBytes);
            ...
            if (numReadBytes != this->varInfoColl->GetNumBytes(distribGroup))
                throw new Exception("FLOW \"%s\" from mapper \"%s\": numReadBytes(%d) != maxNumInBytes(%d)",
                                    this->flowIterator->name, this->mapperIterator->name, numReadBytes, maxNumInBytes);
...
void D3dFlowContextFlowSide::SendBlobToMapper(DDBlobID blobID, // Identifier of blob to be sent
...
int D3dFlowContextFlowSide::ReceiveBlobFromMapper(DDBlobID blobID, // Identifier of blob to be sent
        ...
        else
        {
            ...
            if ((this->numCommPoints > 0) && (distribGroup == GawsDistrib_AllVars))
            {
                ...
                if (bytes != NULL)
                {
                    ...
                    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Fill \"%10s\" %x wrka1",
                                             this->flowIterator->name, this->wrka1);
                    numBytes += this->BufferVarOnCommPoints(bytes, false, this->wrka1, BufferAction_Fill);
                    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Fill \"%10s\" %x wrka1 end",
                                             this->flowIterator->name, this->wrka1);

                    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Fill \"%10s\" %x wrka2",
                                             this->flowIterator->name, this->wrka2);
                    numBytes += this->BufferVarOnCommPoints(bytes + numBytes, false, this->wrka2, BufferAction_Fill);
                    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Fill \"%10s\" %x wrka2 end",
                                             this->flowIterator->name, this->wrka2);

                    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Fill \"%10s\" %x wrka3",
                                             this->flowIterator->name, this->wrka3);
                    numBytes += this->BufferVarOnCommPoints(bytes + numBytes, false, this->wrka3, BufferAction_Fill);
                    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Fill \"%10s\" %x wrka3 end",
                                             this->flowIterator->name, this->wrka3);

                    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Fill \"%10s\" %x wrka4",
                                             this->flowIterator->name, this->wrka4);
                    numBytes += this->BufferVarOnCommPoints(bytes + numBytes, false, this->wrka4, BufferAction_Fill);
                    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Fill \"%10s\" %x wrka4 end",
                                             this->flowIterator->name, this->wrka4);
    ...
    if (this->memType == Mem_Distributed)
    {
        ...
        else
        {
            ...
            if ((this->numCommPoints > 0) && (distribGroup == GawsDistrib_AllVars))
            {
                ...
                FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Read \"%10s\" %x wrka1",
                                         this->flowIterator->name, this->wrka1);
                numBytes += this->BufferVarOnCommPoints(bytes, left_2_right, this->wrka1, BufferAction_Read);
                FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Read \"%10s\" %x wrka1 end",
                                         this->flowIterator->name, this->wrka1);

                FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Read \"%10s\" %x wrka2",
                                         this->flowIterator->name, this->wrka1);
                numBytes += this->BufferVarOnCommPoints(bytes + numBytes, left_2_right, this->wrka2, BufferAction_Read);
                FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Read \"%10s\" %x wrka2 end",
                                         this->flowIterator->name, this->wrka1);

                FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Read \"%10s\" %x wrka3",
                                         this->flowIterator->name, this->wrka1);
                numBytes += this->BufferVarOnCommPoints(bytes + numBytes, left_2_right, this->wrka3, BufferAction_Read);
                FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Read \"%10s\" %x wrka3 end",
                                         this->flowIterator->name, this->wrka1);

                FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Read \"%10s\" %x wrka4",
                                         this->flowIterator->name, this->wrka1);
                numBytes += this->BufferVarOnCommPoints(bytes + numBytes, left_2_right, this->wrka4, BufferAction_Read);
                FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Read \"%10s\" %x wrka4 end",
                                         this->flowIterator->name, this->wrka1);
...
void D3dFlowContextFlowSide::SendSizesAndFlagsToMapper(void)
{
    ...
    if (this->memType == Mem_Distributed)
    {
        ...
        else
        {
            ...
            this->mapperIterator->Send(sizesFlagsBlob,
                                       F2M_Blob_SubdomainSizesFlags); // ddd-comm: flow->mapper, sizes/Flags
...
void D3dFlowContextFlowSide::ReceiveAndCreateMapperStrips(void)
{
    ...
    if (this->memType == Mem_Distributed)
    {
        ...
        else
        {
            ...
            {
                throw new Exception(
                    "D3dFlowContextFlowSide::ReceiveAndCreateMapperStrips: unexpected blobID (%d /= %d)",
...
int D3dFlowContextFlowSide::BufferVarOnCommPoints(char* buffer,             // buffer to be filled or read
        ...
        if (bufferAction == BufferAction_Read)
        {
            if (this->commPointAdmins[i].l2r == left_to_right)
            {
                ...
                FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "FLOWBufferComm Read \"%10s\" %x (%3d,%3d): %10.4f",
                                         this->flowIterator->name, var, this->commPointAdmins[i].m,
                                         this->commPointAdmins[i].n,
...
int D3dFlowContextFlowSide::BufferVarOnCommPoints_PREC(char* buffer,             // buffer to be filled or read
                                                       bool left_to_right,       // L2R (true) or bottom->top (false)
