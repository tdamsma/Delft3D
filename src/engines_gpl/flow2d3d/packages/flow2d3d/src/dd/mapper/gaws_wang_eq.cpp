...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/gaws_wang_eq.cpp
// $
//------------------------------------------------------------------------------
// Class: WangEquations
...
WangEquations::WangEquations(void)
{
    ...
    this->numEq = 0;
    this->equations = (EquationLine*)malloc(this->maxEq * sizeof(EquationLine));
}

void WangEquations::AddEquation(bool l2r,                      // equation in L2R or B2T line?
...
)
{
    ...
    if (!BeginOfLine)
    {
        ...
        this->equations[numEq].Dnext = cntxt->Get_Adress(l2r, coupM, coupN, GawsTerm_D);
        ...
        ON_LOG_EQUATIONS(
            ...
            FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "ADD-EQ-END(%3d): %6d(%3d,%3d) %10.4f %10.4f %10.4f %10.4f",
                                     numEq, cntxt->GetContextId(), curM, curN, *(this->equations[numEq].A),
                                     *(this->equations[numEq].B), *(this->equations[numEq].C),
...

    {
        ...
        this->equations[numEqP].Hprev = cntxt->Get_Adress(l2r, coupM, coupN, GawsTerm_D);
        ...
        ON_LOG_EQUATIONS(
            ...
            FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "ADD-EQ-BEGIN(%3d): %6d(%3d,%3d) %10.4f %10.4f %10.4f %10.4f",
                                     numEqP, cntxt->GetContextId(), curM, curN, *(this->equations[numEqP].E),
                                     *(this->equations[numEqP].F), *(this->equations[numEqP].G),
...
#if LOG_SOLVE
        FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "solution: (%6d)) %10.5e %10.5e %10.5e %10.5e", n, *A, *B, *C,
                                 *D);
#endif
    }
...
void WangEquations::AddEquationSpace(void)
{
    ...
    newEquations = (EquationLine*)realloc(this->equations, this->maxEq * sizeof(EquationLine));
