...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/context-mapside.cpp
// $
//------------------------------------------------------------------------------
...
D3dFlowContextMapSide::D3dFlowContextMapSide(void) { MAPDBG_FUN2("D3dFlowContextMapSide::D3dFlowContextMapSide"); }

D3dFlowContextMapSide::~D3dFlowContextMapSide(void) { MAPDBG_FUN2("D3dFlowContext::~D3dFlowContextMapSide"); }

int D3dFlowContextMapSide::Setup(Iterator* mapper, // mapper process
                                 Iterator* flow,   // flow processes
                                 MemType aMemType, EdgeType aEdgeType, int mStart[NR_EQ], int nStart[NR_EQ],
                                 int mEnd[NR_EQ], int nEnd[NR_EQ])
{
    int retVal = HY_OK;
    ...
    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "MAPPER \"%s\" WAITS for cID from flow \"%s\"", mapper->name,
                             flow->name);
    ...
    {
        throw new Exception("D3dFlowContextMapSide::Setup: unexpected blobID (%d /= %d)", incomingBlobID,
                            F2M_Blob_ContextID);
    }

    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "MAPPER \"%s\" RECEIVED contextID %d from flow \"%s\"", mapper->name,
                             this->contextID, flow->name);

    if (this->contextID == YET_TO_INIT)
    {
        throw new Exception("MAPPER \"%s\" didn't RECEIVE valid contextID from flow \"%s\"", mapper->name, flow->name);
    }

    if (this->memType == Mem_Shared)
    {
        FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR,
                                 "MAPPER \"%s\", Context %d connected for dd-data from FLOW \"%s\"",
...
void D3dFlowContextMapSide::UpdateMapperToFlow(UpdateHeader& updateHeader)
{
...
void D3dFlowContextMapSide::UpdateMapperFromFlow(UpdateHeader& updateHeader)
{
    ...
    if (this->memType == Mem_Distributed)
    {
        ...
        else
        {
            ...
            FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR,
                                     "MAPPER \"%s\" WAITS for Update (max %d bytes) from flow \"%s\"",
            ...
            FLOW2D3D->dd->log->Write(
                Log::DDMAPPER_MINOR,
                "MAPPER \"%s\" RECEIVED Update from flow \"%s\" (step %d, #mess %d, numInBytes %d)",
...
void D3dFlowContextMapSide::ReceiveSizesAndFlagsFromFlow(void)
{
    ...
    else
    {
        ...
        FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR,
                                 "\"%s\"<-\"%s\": mMx=%d, nMx=%d, nMxus=%d, kMx=%d, lSt=%d, lT=%d, lSd=%d, Zm=%d, "
                                 "dDb=%d, mMxdb=%d, nMxdb=%d",
                                 this->mapperIterator->name, this->flowIterator->name, mMax, nMax, nMaxus, kMax, lStsci,
                                 lTur, lSedtt, Zmodel, dDb, mMaxdb, nMaxdb);
...
void D3dFlowContextMapSide::SendBlobToFlow(DDBlobID blobID, // Identifier of blob to be sent
...
int D3dFlowContextMapSide::ReceiveBlobFromFlow(DDBlobID blobID, // Identifier of blob to be received
