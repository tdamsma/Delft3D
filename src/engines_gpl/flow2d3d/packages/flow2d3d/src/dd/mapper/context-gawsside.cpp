...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/context-gawsside.cpp
// $
//------------------------------------------------------------------------------
...
D3dFlowContextGawsSide::D3dFlowContextGawsSide(void) { MAPDBG_FUN2("D3dFlowContextGawsSide::D3dFlowContextGawsSide"); }

D3dFlowContextGawsSide::~D3dFlowContextGawsSide(void) { MAPDBG_FUN2("D3dFlowContextMapSide::~D3dFlowContextGawsSide"); }

int D3dFlowContextGawsSide::Setup(Iterator* gaws,  // Gaws iterator
...
)
{
    ...
    {
        throw new Exception("D3dFlowContextGawsSide::Setup: unexpected blobID (%d /= %d)", incomingBlobID,
                            F2G_Blob_ContextID);
    }
    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "GAWS \"%s\" RECEIVED cID from flow \"%s\"", this->gawsIterator->name,
                             this->flowIterator->name);

    if (this->contextID == YET_TO_INIT)
    {
        throw new Exception("GAWS didn't receive valid contextID from D3dFlow \"%s\"", this->flowIterator->name);
    }
    ...
    else
    {
        ...
        FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "GAWS \"%s\" WAITS for KCS (%d*%d) from flow \"%s\"",
                                 this->gawsIterator->name, this->mArraySize, this->nArraySize,
                                 this->flowIterator->name);
...
FlowScalar* D3dFlowContextGawsSide::Get_Adress(bool l2r,     // L2R or B2T?
                                               int m,        // M index
                                               int n,        // N index
...
)
{
    ...
    if (var != NULL)
    {
        switch (this->memType)
        {
            ...
            case Mem_Distributed:
                ...
                {
                    if ((this->commPointAdmins[i].m == m) && (this->commPointAdmins[i].n == n) &&
    ...
    FLOW2D3D->dd->log->Write(
        Log::DDMAPPER_MINOR, "D3dFlowContextGawsSide::Get_Adress for %d in \"%s\"(%s %2d,%2d): %x, value %12.5f", term,
        this->flowIterator->name, (l2r ? "L2R" : "T2B"), m, n, retVal, (retVal == NULL) ? 0.0 : *retVal);

    return retVal;
}

int D3dFlowContextGawsSide::Get_KCS_XorY(int cell,        // cell-index on line
                                         int line,        // line-index
...
)
{
    ...
    ON_DEBUG(DBLEV7, CMapLog("Get_KCS_X_OR_Y (line=%2d, cell=%2d), %s: %d\n", line, cell,
                             (leftToRight ? "leftToRight" : "topToBottom"), retVal);)
    ...
    if (this->memType == Mem_Distributed)
    {
        ...
        else
        {
            ...
            if ((this->numCommPoints > 0) && (distribGroup == GawsDistrib_AllVars))
            {
                ...
                numBufferedBytes +=
                    this->BufferVarOnCommPoints(bytes + numBufferedBytes, this->wrka2, BufferAction_Fill);
                numBufferedBytes +=
                    this->BufferVarOnCommPoints(bytes + numBufferedBytes, this->wrka3, BufferAction_Fill);
                numBufferedBytes +=
                    this->BufferVarOnCommPoints(bytes + numBufferedBytes, this->wrka4, BufferAction_Fill);
...
void D3dFlowContextGawsSide::AddCommPointAdmin(int m,   // m-index of point
...
)
{
    if (this->memType != Mem_Shared)
    {
        ...
        for (int cp = 0; cp < this->numCommPoints; cp++)
        {
            if (this->commPointAdmins[cp].m == m && this->commPointAdmins[cp].n == n &&
...
void D3dFlowContextGawsSide::ReceiveSizesFromFlow(void)
{
    ...
    else
    {
        ...
        FLOW2D3D->dd->log->Write(
            Log::DDMAPPER_MINOR, "\"%s\"<-\"%s\": mMx=%d, nMx=%d, nMxus=%d, kMx=%d, dDb=%d, mMxdb=%d, nMxdb=%d",
            this->gawsIterator->name, this->flowIterator->name, mMax, nMax, nMaxus, kMax, dDb, mMaxdb, nMaxdb);

        delete sizes;
    }
}

int D3dFlowContextGawsSide::BufferVarOnCommPoints(char* buffer,             // buffer to be filled or read
...
)
{
    ...
    for (i = 0; i < this->numCommPoints; i++)
    {
        ...
        {
            *(var + i) = *copyTarget++;
            ON_LOG_GAWS_EXCHANGE(FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR,
                                                          "GAWSBufferComm Read from \"%10s\" %x (%3i): %10.4f",
        ...
        else
        {
            *copyTarget++ = *(var + i);
            ON_LOG_GAWS_EXCHANGE(FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR,
                                                          "GAWSBufferComm Fill for  \"%10s\" %x (%3i): %10.4f",
                                                          this->flowIterator->name, var, i, *(var + i));
                                 fflush(stdout);)
        }
