...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/mapper_uvz.cpp
// $
//------------------------------------------------------------------------------
//  Class: D3dFlowMapper
...
void D3dFlowMapper::InitKc(Eq eq // equation type
)
{
    ...
    // don't do anything.
    //
    if ((mEnd[C_0][eq] < mStart[C_0][eq]) || (nEnd[C_0][eq] < nStart[C_0][eq]))
    {
        FLOW2D3D->dd->log->Write(Log::WARN, "Map-length <= 0 for equation %d: m1=%d > m2=%d or n1=%d > n2=%d", eq,
                                 mStart[C_0][eq], mEnd[C_0][eq], nStart[C_0][eq], nEnd[C_0][eq]);
        return;
    }
    ...
    for (ctx = 0; ctx < NR_CNTXTS; ctx++)
    {
        ...
        if ((eq == Eq_U) && (Edge[ctx] == Edge_Right))
        {
            MAP_CELLS_LOOP(ctx, eq)
            {
                // printf("KCU=3: (%2d,%2d) %d\n", m+1, n, ctx);
        ...
        //
        if ((eq == Eq_U) && (Edge[ctx] == Edge_Left) && (Ref[ctx] < Ref[oCtx]))
        {
            MAP_CELLS_LOOP(ctx, eq)
            {
        ...
        if ((eq == Eq_U) && (Edge[ctx] == Edge_Right) && (Ref[ctx] > Ref[oCtx]))
        {
            MAP_CELLS_LOOP(ctx, eq)
            {
        ...
        if ((eq == Eq_V) && (Edge[ctx] == Edge_Bottom))
        {
            MAP_CELLS_LOOP(ctx, eq)
            {
        ...
        //
        if ((eq == Eq_V) && (Edge[ctx] == Edge_Bottom) && (Ref[ctx] < Ref[oCtx]))
        {
            MAP_CELLS_LOOP(ctx, eq)
            {
        ...
        if ((eq == Eq_V) && (Edge[ctx] == Edge_Top) && (Ref[ctx] > Ref[oCtx]))
        {
            MAP_CELLS_LOOP(ctx, eq)
            {
        ...
        //
        if ((eq == Eq_Zeta) && (Edge[ctx] == Edge_Left) && (nStart[ctx][Eq_Zeta] < nEnd[ctx][Eq_Zeta]))
        {
        ...
        if ((eq == Eq_Zeta) && (Edge[ctx] == Edge_Bottom) && (mStart[ctx][Eq_Zeta] < mEnd[ctx][Eq_Zeta]))
        {
...
void D3dFlowMapper::CheckDDbounds(Eq eq // equation type
)
{
            ...
            if (Edge[ctx] == Edge_Left)
            {
                ...
                if ((KcsLeft != 0) || (KcsRight != 1))
                {
                    throw new Exception("%s\"%s\": Wrong coupling point at (%2d,%2d) Kcs=%d KcsInside=%d",
                                        iniErrorMessage, this->C[ctx]->flowIterator->name, m, n, KcsLeft, KcsRight);
                }
            }
...
void D3dFlowMapper::CheckDepthUV(Eq eq // equation type (U or V)
)
{
    ...
    for (ctx = 0; ctx < NR_CNTXTS; ctx++)
    {
        ...
        if ((Edge[ctx] == Edge_Left || Edge[ctx] == Edge_Right) && (eq == Eq_U) && (Ref[ctx] <= Ref[oCtx]))
        {
            MAP_CELLS_LOOP(ctx, eq)
            {
                int m2 = m;
        ...
        if ((Edge[ctx] == Edge_Bottom || Edge[ctx] == Edge_Top) && (eq == Eq_V) && (Ref[ctx] <= Ref[oCtx]))
        {
            MAP_CELLS_LOOP(ctx, eq)
            {
                int n2 = n;
...
void D3dFlowMapper::InitFlowVariables(void)
{
    ...
    for (eq = 0; eq < NR_EQ; eq++)
    {
        for (ctx = 0; ctx < NR_CNTXTS; ctx++)
        {
            ON_DEBUG(DBLEV1, FMapLog((char*)"Exchange FLOW VARS (ctx %d, eqType: %d)\n", ctx, eq);)
            ...
            switch (eq)
            {
                ...
                case Eq_Zeta:
                    ...
                    MAP_CELLS_LOOP(ctx, eq)
                    {
                        ...
                        FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "copy-dps: (%2d,%2d) %10.5f", m, n,
                                                 cI2D(ctx, m, n, dps));
...
void D3dFlowMapper::CopyFlowVariables(Seq seq // Type of variable
)
{
                ...
                if (Edge[ctx] == Edge_Left || Edge[ctx] == Edge_Right)
                {
                    ...
                    MAP_CELLS_AND_LAYERS_LOOP(ctx, eq)
                    {
                        if (cI2D(ctx, m, n, kcu) == 3)
                        {
                            ...
                            MAP_REFINED_LOOP(oCtx, oM, oN)
                            {
                                ON_DEBUG(DBLEV3, FMapLog((char*)"\t\tCopy-U (%d,%d)<-(%d,%d)\n", m, n, fM, fN);)
                ...
                if (Edge[ctx] == Edge_Bottom || Edge[ctx] == Edge_Top)
                {
                    ...
                    MAP_CELLS_AND_LAYERS_LOOP(ctx, eq)
                    {
                        if (cI2D(ctx, m, n, kcv) == 3)
                        {
                            ...
                            MAP_REFINED_LOOP(oCtx, oM, oN)
                            {
                                ON_DEBUG(DBLEV3, FMapLog((char*)"\t\tCopy-V (%d,%d)<-(%d,%d)\n", m, n, fM, fN);)
                    ...
                    MAP_CELLS_LOOP(ctx, eq)
                    {
                        if (cI2D(ctx, m, n, kcv) == 3)
                        {
                            ...
                            // n,cI2D(ctx,m,n,kfv),oM,oN,cI2D(oCtx,oM,oN,kfv));
                            if (C[ctx]->Zmodel && C[ctx]->kMax == C[oCtx]->kMax)
                            {
...
void D3dFlowMapper::CopyOuterNormalVelocities(Seq seq // Type of variable
)
...
void D3dFlowMapper::CopyOuterTangentVelocities(Seq seq // Type of variable
)
...
void D3dFlowMapper::Build(Seq seq, // Variable
...
)
{
    ...
    for (ctx = 0; ctx < NR_CNTXTS; ctx++)
    {
        ...
        switch (seq)
        {
            case Seq_U:
                ...
                if (Edge[ctx] == Edge_Left || Edge[ctx] == Edge_Right)
                {
                    MAP_CELLS_AND_LAYERS_LOOP(ctx, eq)
                    {
                        ...
                        if (cI2D(ctx, m, n, kcu) == 3)
                        {
                            ON_DEBUG(DBLEV2, FMapLog((char*)"Building U for %d,%d,%d (ctx %d)\n", m, n, k, ctx);)

                            horAverage = 0.0;
                            MAP_REFINED_LOOP(oCtx, oM, oN)
                            {
                                ON_DEBUG(DBLEV3, FMapLog((char*)"\t\tRefine (%d,%d)<-(%d,%d)\n", m, n, fM, fN);)
                                V_AVERAGE3D(oCtx, fM, fN, oK, u1, vertAverage);
                                horAverage += vertAverage;
                            }
            ...
            case Seq_V:
                ...
                if (Edge[ctx] == Edge_Bottom || Edge[ctx] == Edge_Top)
                {
                    MAP_CELLS_AND_LAYERS_LOOP(ctx, eq)
                    {
                        ...
                        if (cI2D(ctx, m, n, kcv) == 3)
                        {
                            ON_DEBUG(DBLEV2, FMapLog((char*)"Building V for %d,%d,%d (ctx %d)\n", m, n, k, ctx);)
            ...
            case Seq_Zeta:
            case Seq_Zeta_ADI_St1:
            case Seq_Zeta_ADI_St2:
                printf("THIS CODE LOCATION SHOULD NOT BE REACHED\n");
                break;

            case Seq_Conc:
            case Seq_Conc_ADI_St1:
            case Seq_Conc_ADI_St2:

                MAP_CELLS_AND_LAYERS_LOOP(ctx, eq)
                {
                    ON_DEBUG(DBLEV2, FMapLog((char*)"Building C for %d,%d,%d, c-%d (ctx %d)\n", m, n, k, anL, ctx);)
...
int D3dFlowMapper::Proces(Seq seq, // Variable
                          int anL  // concentration loop counter
)
{
    ...
    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "MAPPER \"%s\" calls minimumbarrier (converged %d)",
                             IteratorSelf()->name, converged);
    allConverged = MinimumBarrier(converged);
    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "MAPPER \"%s\" minimumbarrier returned, conv: %d",
                             IteratorSelf()->name, allConverged);
...
int D3dFlowMapper::CheckConvergence(Seq seq,                  // Variable type
                                    int anL,                  // concentration loop counter
                                    int toBeSolved[NR_CNTXTS] // which context must be resolved?
)
{
    ...
    for (ctx = 0; ctx < NR_CNTXTS; ctx++)
    {
        ...
        switch (seq)
        {
            case Seq_U:
                MAP_COARSE_CELLS_AND_COARSE_LAYERS_LOOP(ctx, eq)
                {
                    ...
                    {
                        ON_DEBUG(DBLEV2, FMapLog((char*)"ChkConv U for %d,%d,%d (ctx %d)\n", m, n, k, ctx);)
                        ...
                        ON_DEBUG(DBLEV6, FMapLog((char*)"2 sides %10.8f,%10.8f\n", otherRHS, currentRHS);
            ...
            case Seq_V:
                MAP_COARSE_CELLS_AND_COARSE_LAYERS_LOOP(ctx, eq)
                {
                    ...
                    {
                        ON_DEBUG(DBLEV2, FMapLog((char*)"ChkConv V for %d,%d,%d (ctx %d)\n", m, n, k, ctx);)
            ...
            case Seq_2DAD:
                ...
                {
                    horAverage = 0.0;
                    MAP_REFINED_LOOP(oCtx, oM, oN) { horAverage += cI3D(oCtx, fM, fN, 1, wrkb17); }
                    otherRHS = horAverage / nHorRef;

                    horAverage = 0.0;
                    MAP_REFINED_LOOP(ctx, m, n) { horAverage += cI3D(ctx, fM, fN, 1, wrkb17); }
                    currentRHS = horAverage / nHorRef;
        ...
        ON_DEBUG(DBLEV2 + DBLEV6, FMapLog((char*)"deltaMax-%s Cntxt-%d: %10.8f\n", seqTxt[seq], ctx, deltaMax[ctx]);)
...
void D3dFlowMapper::Adjust(Seq seq,                  // Equation type
                           int anL,                  // concentration loop counter
                           int toBeSolved[NR_CNTXTS] // which context must be resolved?
)
{
                ...
                case Seq_U:
                    ...
                    if (Edge[ctx] == Edge_Left || Edge[ctx] == Edge_Right)
                    {
                        MAP_CELLS_AND_LAYERS_LOOP(ctx, eq)
                        {
                            ...
                            if (cI2D(ctx, m, n, kcu) == 3)
                            {
                                ON_DEBUG(DBLEV2, FMapLog((char*)"Adjusting U for %d,%d,%d (ctx %d)\n", m, n, k, ctx);)
                ...
                case Seq_V:
                    ...
                    if (Edge[ctx] == Edge_Bottom || Edge[ctx] == Edge_Top)
                    {
                        MAP_CELLS_AND_LAYERS_LOOP(ctx, eq)
                        {
                            ...
                            if (cI2D(ctx, m, n, kcv) == 3)
                            {
                                ON_DEBUG(DBLEV2, FMapLog((char*)"Adjusting V for %d,%d,%d (ctx %d)\n", m, n, k, ctx);)
...
double D3dFlowMapper::InterpolateNormalU1(int fineCtx, int coarseCtx, int fineN, int coarseM, int coarseN, int coarseK)
{
...
double D3dFlowMapper::InterpolateTangentialU1(int fineCtx, int coarseCtx, int fineM, int coarseM, int coarseN,
                                              int coarseK)
{
    // From coarse to fine, interpolate: (1-alpha) * A + alpha * B
    //                         (A: value on n-down, B: value on n-up)
...
double D3dFlowMapper::InterpolateNormalV1(int fineCtx, int coarseCtx, int fineM, int coarseM, int coarseN, int coarseK)
{
    // From coarse to fine, interpolate: (1-alpha) * A + alpha * B
    //                         (A: value on n-down, B: value on n-up)
...
double D3dFlowMapper::InterpolateTangentialV1(int fineCtx, int coarseCtx, int fineN, int coarseM, int coarseN,
                                              int coarseK)
{
    // From coarse to fine, interpolate: (1-alpha) * A + alpha * B
    //                         (A: value on n-down, B: value on n-up)
...
double D3dFlowMapper::InterpolateS1(int fineCtx, int coarseCtx, int fineM, int fineN, int coarseM, int coarseN,
                                    int coarseK)
{
    // From coarse to fine, interpolate: (1-alpha) * A + alpha * B
    //                         (A: value on n-down, B: value on n-up)
    ...
    // Determine alpha
    bool topBottomMapper = (Edge[ctx] == Edge_Bottom || Edge[ctx] == Edge_Top) ? true : false;
    int start = topBottomMapper ? mStart[fineCtx][Eq_Zeta] : nStart[fineCtx][Eq_Zeta];
    int end = topBottomMapper ? mEnd[fineCtx][Eq_Zeta] : nEnd[fineCtx][Eq_Zeta];
    int index = topBottomMapper ? fineM : fineN;
    ...
    for (ctx = 0; ctx < NR_CNTXTS; ctx++)
    {
        ...
        if (Ref[ctx] < Ref[oCtx])
        {
            ...
            MAP_CELLS_LOOP(ctx, eq)
            {
                ...
                if ((Edge[ctx] == Edge_Right || Edge[ctx] == Edge_Left) && (cI2D(ctx, M_onEdge, N_onEdge, kfu) == 0))
                {
                    MAP_LAYERS_LOOP(oCtx)
                    {
                        MAP_REFINED_LOOP(oCtx, otherM_onEdge, otherN_onEdge)
                        {
                            ...
                            if (fabs(otherVertSum) > EPSILON)
                            {
                                ...
                                {
                                    factor = -1.0;
                                    zetaCorrF = C[oCtx]->Hdt * factor * otherVertSum / cI2D(oCtx, fM + 1, fN, gsqs);
                                    cI2D(oCtx, fM + 1, fN, s1) += (float)zetaCorrF;
                                }
                                else
                                {
                                    factor = 1.0;
                                    zetaCorrF = C[oCtx]->Hdt * factor * otherVertSum / cI2D(oCtx, fM, fN, gsqs);
                ...
                if ((Edge[ctx] == Edge_Top || Edge[ctx] == Edge_Bottom) && (cI2D(ctx, M_onEdge, N_onEdge, kfv) == 0))
                {
                    MAP_LAYERS_LOOP(oCtx)
                    {
                        MAP_REFINED_LOOP(oCtx, otherM_onEdge, otherN_onEdge)
                        {
                            ...
                            if (fabs(otherVertSum) > EPSILON)
                            {
                                ...
                                {
                                    factor = -1.0;
                                    zetaCorrF = C[oCtx]->Hdt * factor * otherVertSum / cI2D(oCtx, fM, fN + 1, gsqs);
                ...
                if ((Edge[ctx] == Edge_Right || Edge[ctx] == Edge_Left) && (cI2D(ctx, M_onEdge, N_onEdge, kfu) == 1))
                {
                    MAP_LAYERS_LOOP(ctx)
                    {
                        otherKfuSum = 0;
                        MAP_REFINED_LOOP(oCtx, otherM_onEdge, otherN_onEdge) { otherKfuSum += cI2D(oCtx, fM, fN, kfu); }
                        if (otherKfuSum == 0)
                        {
                            ...
                            {
                                factor = -1.0;
                                zetaCorrC = C[ctx]->Hdt * factor * qxkold / cI2D(ctx, M_onEdge + 1, N_onEdge, gsqs);
                                cI2D(ctx, M_onEdge + 1, N_onEdge, s1) += (float)zetaCorrC;
                                cI2D(oCtx, otherM_onEdge + 1, otherN_onEdge, s1) =
                                    cI2D(ctx, M_onEdge + 1, N_onEdge, s1);
                            }
                            else
                            {
                                factor = 1.0;
                                zetaCorrC = C[ctx]->Hdt * factor * qxkold / cI2D(ctx, M_onEdge, N_onEdge, gsqs);
                ...
                if ((Edge[ctx] == Edge_Top || Edge[ctx] == Edge_Bottom) && (cI2D(ctx, M_onEdge, N_onEdge, kfv) == 1))
                {
                    MAP_LAYERS_LOOP(ctx)
                    {
                        otherKfvSum = 0;
                        MAP_REFINED_LOOP(oCtx, otherM_onEdge, otherN_onEdge) { otherKfvSum += cI2D(oCtx, fM, fN, kfv); }
                        if (otherKfvSum == 0)
                        {
                            // printf("Discharge_KFV_correctie (%2d %2d) (%2d
                            // %2d)\n",M_onEdge,N_onEdge,otherM_onEdge,otherN_onEdge);
                            ...
                            {
                                factor = -1.0;
                                zetaCorrC = C[ctx]->Hdt * factor * qykold / cI2D(ctx, M_onEdge, N_onEdge + 1, gsqs);
                                cI2D(ctx, M_onEdge, N_onEdge + 1, s1) += (float)zetaCorrC;
                                cI2D(oCtx, otherM_onEdge, otherN_onEdge + 1, s1) =
                                    cI2D(ctx, M_onEdge, N_onEdge + 1, s1);
                            }
                            else
                            {
                                factor = 1.0;
                                zetaCorrC = C[ctx]->Hdt * factor * qykold / cI2D(ctx, M_onEdge, N_onEdge, gsqs);
                                cI2D(ctx, M_onEdge, N_onEdge, s1) += (float)zetaCorrC;
                                cI2D(oCtx, otherM_onEdge, otherN_onEdge, s1) = cI2D(ctx, M_onEdge, N_onEdge, s1);
                            }
                        }
                ...
                zetaCorrC = difference * C[ctx]->Hdt / cI2D(ctx, M_onEdge, N_onEdge, gsqs);

                if ((Edge[ctx] == Edge_Right) || (Edge[ctx] == Edge_Top))
                {
                    cI2D(ctx, M_onEdge, N_onEdge, s1) -= (float)zetaCorrC;
                }
                else
        ...
        else if ((Ref[ctx] == Ref[oCtx]) && (C[ctx]->kMax <= C[oCtx]->kMax))
        {
            //
            ...
            MAP_CELLS_LOOP(ctx, eq)
            {
                ...
                zetaCorr = difference * C[ctx]->Hdt / cI2D(ctx, M_onEdge, N_onEdge, gsqs);
                ...
                // printf("FW-cor-NoRef %2d %2d %2d %2d %10.7f %10.7f
...
int D3dFlowMapper::CheckForDry()
{
    ...
    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "MAPPER \"%s\" calls minimumbarrier (myWet %d)", IteratorSelf()->name,
                             myWet);
    allWet = MinimumBarrier(myWet);
    FLOW2D3D->dd->log->Write(Log::DDMAPPER_MINOR, "MAPPER \"%s\" minimumbarrier returned (allWet %d)",
                             IteratorSelf()->name, allWet);

    return (!allWet);
}

void D3dFlowMapper::CopySediment(Seq seq // Type of variable
)
{
    Eq eq = location[seq]; // type of equation location
                        ...
                        MAP_CELLS_LOOP(ctx, eq)
                        {
                            ...
                            for (l = 1; l <= C[ctx]->lSedtt; l++)
                            {
                                ...
                                if (cI3D(ctx, m + 1, n, l, sbuu) == 0.0L)
                                {
                                    ...
                                    // printf("   Sed-SBUU-coarse (%2d %2d %2d)<--
                    ...
                    else
                    {
                        ...
                        MAP_CELLS_LOOP(ctx, eq)
                        {
                            ...
                            for (l = 1; l <= C[ctx]->lSedtt; l++)
                            {
                                ...
                                {
                                    cI3D(ctx, m + 1, n, l, sbuu) = cI3D(oCtx, oM + 1, oN, l, sbuu);
                                    // printf("   Sed-SBUU-norm (%2d %2d %2d)<-- (%2d %2d)
                        ...
                        MAP_CELLS_LOOP(ctx, eq)
                        {
                            ...
                            for (l = 1; l <= C[ctx]->lSedtt; l++)
                            {
                                ...
                                if (cI3D(ctx, m, n + 1, l, sbvv) == 0.0L)
                                {
                                    ...
                                    // printf("   Sed-SBVV-coarse (%2d %2d %2d)<--
                    ...
                    else
                    {
                        ...
                        MAP_CELLS_LOOP(ctx, eq)
                        {
                            ...
                            for (l = 1; l <= C[ctx]->lSedtt; l++)
                            {
                                ...
                                {
                                    cI3D(ctx, m, n + 1, l, sbvv) = cI3D(oCtx, oM, oN + 1, l, sbvv);
                                    // printf("   Sed-SBVV-norm (%2d %2d %2d)<-- (%2d %2d)
