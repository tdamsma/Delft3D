...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/dd.cpp
// $
//------------------------------------------------------------------------------
...
DD::DD(Flow2D3D* flow, XmlTree* config)
{
    this->flow = flow;
    this->config = config;
    ...
    if (this->multiNode == NULL) {
        ...
        if (this->flow->DH->slaveArg == NULL) {
            this->log->Write (Log::MAJOR, "Flow2D3D initializing master for multi-domain multi-node simulation on %d
    nodes", this->numNodes); this->role = ROLE_MASTER;
            }
        else {
...
void DD::CreateFixedEntities(void)
{
...
void DD::ReadConfig(void)
{
    const char* ddbfile = this->config->GetElement("ddbFile");
    if (ddbfile == NULL) throw new Exception("DD-bounds file not specified in configuration file");

    this->log->Write(Log::CONFIG_MAJOR, "Reading DD-bounds file \"%s\"...", ddbfile);

    FILE* input;
    if ((input = fopen(ddbfile, "r")) == NULL) throw new Exception("Cannot open DD-bounds file \"%s\"", ddbfile);

    char line[DD::MAXSTRING];
    if (fgets(line, DD::MAXSTRING, input) == NULL) throw new Exception("Error reading first line of DD-bounds file");
...
void DD::ReadDDBounds(FILE* input, const char* filename)
{
    ...
    while (fgets(&line[pos], MAXSTRING - pos, input) != NULL)
    {
        ...
        sprintf(mapname, "M%d=%s=%s", ++mappercount, process_name(left, leftname), process_name(right, rightname));
...
Iterator* DD::AddProcess(const char* name, const char* config, int nodeID)
{
...
Iterator* DD::AddMapper(const char* name, const char* config, Iterator* leftprocess, Iterator* rightprocess, int nodeID)
{
    ...
    if (nodeID == this->nodeID) this->mappersOnThisNode++;
...
void DD::JoinIterators(Iterator* iter1, Iterator* iter2)
{
    // Validate function arguments

    if (iter1 == NULL || iter2 == NULL) throw new Exception("Null iterator in JoinIterators");
    ...
    if (id1 == Dictionary::NOTFOUND) throw new Exception("Unknown iterator \"%s\" in JoinIterators", name1);

    const char* name2 = iter2->name;
    if (name2 == NULL) name2 = "";
    long id2 = (long)this->iteratorDict->Lookup(name2);
    if (id2 == Dictionary::NOTFOUND) throw new Exception("Unknown iterator \"%s\" in JoinIterators", name2);

    // Make sure iterators are in different categories

    if (this->iterator[id1].catID == this->iterator[id2].catID)
        throw new Exception("Iterators \"%s\" and \"%s\" are in the same category (#%d); cannot join", name1, name2,
                            this->iterator[id2].catID);
...
void DD::PrintJoinTable(void)
{
    for (int jid = 0; jid < this->numJoins; jid++)
        this->log->Write(
            Log::CONFIG_MINOR, "JoinTable #%-2d %-6s iter1=%s:%d, iter2=%s:%d, handle=\"%s\" stream=0x%x", jid,
            (this->join[jid].local      ? "local"
             : this->join[jid].leader   ? "lead"
             : this->join[jid].follower ? "follow"
                                        : ""),
            this->iterator[this->join[jid].iter1].name, this->iterator[this->join[jid].iter1].iterator->node->nodeID,
            this->iterator[this->join[jid].iter2].name, this->iterator[this->join[jid].iter2].iterator->node->nodeID,
            this->join[jid].handle, this->join[jid].stream);
}

//------------------------------------------------------------------------------

static void check_md_file(char* filename)
{
    ...
    if (i >= (int)DD::MAXSTRING) throw new Exception("File name in DD-bound file is too long");

    for (; i >= 0 && filename[i] != '.'; i--); // find last dot

    if (i <= 0) throw new Exception("Improperly formed file name in DD-bound file");
    if (i >= (int)DD::MAXSTRING - 4) throw new Exception("MD-file name derived from DD-bound file will be too long");

    strncpy(filename + i, ".mdf", 4);

    if (!readable_file(filename)) throw new Exception("Cannot find subdomain input file \"%s\"", filename);
}
...
static bool readable_file(const char* filename)
{
    ...
    if ((file = open(filename, O_RDONLY)) == -1) return false; // cannot open
...
static char* process_name(const char* filename, char* processname)
{
...
void DD::InitializeLocalMessageBuffers(void)
{
...
void DD::StartLocalIteratorThreads(void)
{
    ...
    for (int iid = 0; iid < this->numIterators; iid++)
    {
        ...
        {
            localiter++;
            this->log->Write(Log::ITER_MAJOR, "Spawning thread for iterator #%d \"%s\" \"%s\"", iid,
                             this->category[this->iterator[iid].catID].name, this->iterator[iid].name);
...
char* GetHostnamePID(void)
{
...
#else
    if (gethostname(hostnamePID, bufsize) != 0) throw new Exception("Cannot get hostname for distributed DD");
#endif
    ...
    if (len + 16 >= bufsize) throw new Exception("Hostname too long for buffer in distributed DD");
