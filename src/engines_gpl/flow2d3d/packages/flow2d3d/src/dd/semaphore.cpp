...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/semaphore.cpp
// $
//------------------------------------------------------------------------------
...
Semaphore::Semaphore(const char* semname, const int initialValue, Log* log)
{
...
void Semaphore::PSem(void)
{
    //  Get mutex to this semaphore object

    if (pthread_mutex_lock(&this->mutex) != 0)
        throw new Exception("Pthreads error in Semaphore: pthread_mutex_lock fails in PSem \"%s\", errno=%d",
                            this->name, errno);
    ...
    if (this->value == 0)
    {
        ...
        if (pthread_cond_wait(&this->syncv, &this->mutex) != 0)
            throw new Exception("Pthreads error in Semaphore: pthread_cond_wait fails in PSem \"%s\", errno=%d",
                                this->name, errno);
    ...
    if (pthread_mutex_unlock(&this->mutex) != 0)
        throw new Exception("Pthreads error in Semaphore: pthread_mutex_unlock fails in PSem \"%s\", errno=%d",
                            this->name, errno);
}
...
void Semaphore::VSem()
{
    ...
    if (pthread_mutex_lock(&this->mutex) != 0)
        throw new Exception("Pthreads error in Semaphore: pthread_mutex_lock fails in VSem \"%s\", errno=%d",
                            this->name, errno);
    ...
    if (this->waiting > 0)
    {
        ...
        if (pthread_cond_signal(&this->syncv) != 0)
            throw new Exception("Pthreads error in Semaphore: pthread_cond_signal fails in VSem \"%s\", errno=%d",
                                this->name, errno);
    }
    else
    {
    ...
    if (pthread_mutex_unlock(&this->mutex) != 0)
        throw new Exception("Pthreads error in Semaphore: pthread_mutex_unlock fails in VSem \"%s\", errno=%d",
                            this->name, errno);
}
