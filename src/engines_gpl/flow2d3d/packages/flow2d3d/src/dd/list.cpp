...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/list.cpp
// $
//------------------------------------------------------------------------------
...
bool List::Delete(void* value)
{
    ...
    for (n = this->head; n != NULL; n = n->next)
    {
        if (value == n->value)
        {
            ...
            if (n == this->tail) this->tail = lastn;
...
void List::Rewind(void) { this->current = this->head; }
