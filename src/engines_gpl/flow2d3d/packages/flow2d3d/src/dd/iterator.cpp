...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/iterator.cpp
// $
//------------------------------------------------------------------------------
...
Iterator::Iterator(DD* dd, const char* name, Blob* configblob, Category* category, IteratorFunction function)
{
    // Validate constructor arguments

    if (name == NULL || name[0] == '\0') throw new Exception("Iterator does not have a name");
    if (strlen(name) >= DD::MAXSTRING) throw new Exception("Iterator name is too long");
    if (dd->iteratorDict->Lookup(name) != (void*)Dictionary::NOTFOUND)
        throw new Exception("Duplicate iterator name \"%s\"", name);
    if (category == NULL) throw new Exception("No category for iterator \"%s\"", name);
    if (function == NULL) throw new Exception("Null iterator function for \"%s\"", name);
    ...
    if (blobsize > 0) memcpy(dd->iterator[this->id].blob, configblob->Address(), blobsize);
    ...
    dd->log->Write(Log::ITER_MAJOR, "Created iterator #%d \"%s\" in category \"%s\"", this->id, this->name,
                   category->name);
}
...
void Iterator::AddNeigh(Iterator* neighbor)
{
...
unsigned int Iterator::NeighborCount(Category* category)
{
    CatNeigh* catNeigh;
    ...
    else
    {
        ...
        if ((long)catNeigh == Dictionary::NOTFOUND) return 0;
    }

    return catNeigh->count;
}

void Iterator::RewindNeighbors(Category* category)
{
    CatNeigh* catNeigh;
    ...
    else
    {
        ...
        if ((long)catNeigh == Dictionary::NOTFOUND)
        {
            this->dd->log->Write(Log::ALWAYS, "Iterator \"%s\" does not have neighbors in category \"%s\"", this->name,
                                 category->name);
            return;
        }
    }
...
Iterator* Iterator::NextNeighbor(Category* category)
{
    CatNeigh* catNeigh;
    ...
    else
    {
        catNeigh = (CatNeigh*)this->neighDict->Lookup(category->name);
        if ((long)catNeigh == Dictionary::NOTFOUND)
            throw new Exception("Iterator \"%s\" does not have neighbors in category \"%s\"", this->name,
                                category->name);
    }

    if (!catNeigh->rewound)
        throw new Exception("Neighbor list of \"%s\" not rewound before Next in category \"%s\"", this->name,
                            category->name);
...
int Iterator::FindJoin(int iter1, int iter2)
{
...
void Iterator::Send(Blob* message, int tag)
{
    ...
    if (this->id == senderid)
        throw new Exception("Iterator \"%s\" in \"%s\" is trying to send itself a message", this->name,
    ...
    this->dd->log->Write(Log::DD_SENDRECV, "%s Send %s(%d) -> %s(%d) tag=%d, size=%d", lr, sender->name, sender->id,
                         this->name, this->id, tag, message->Size());
}

void Iterator::Receive(Blob* message, int* ptag)
{
    int tag;
    ...
    int senderid = this->dd->DDGetThreadID();
    if (this->id == senderid)
        throw new Exception("Iterator \"%s\" in \"%s\" is trying to receive a message from itself", this->name,
                            this->dd->category[this->dd->iterator[senderid].catID].name);
    ...
    const char* lr = this->dd->join[jid].local ? "local" : "remote";
    Iterator* sender = this->dd->iterator[senderid].iterator;
    this->dd->log->Write(Log::DD_SENDRECV, "%s Receive %s(%d) -> %s(%d) tag=%d, size=%d", lr, this->name, this->id,
                         sender->name, sender->id, tag, message->Size());
}

//-------------------------------------------------------------------------------

void Iterator::SendMessage(Blob* message, int tag, Channel* channel)
{
    ...
    if (channel->head == channel->tail) throw new Exception("Message buffer full (%d slots)", this->MESGBUFSIZE);

    // Release previous buffer
    if (channel->ring[slot].data != NULL) delete[] channel->ring[slot].data;
...
void Iterator::ReceiveMessage(Blob* message, int* ptag, Channel* channel)
{
    if (pthread_mutex_lock(&channel->mutex) != 0)
...
void Iterator::SetValue(void* value) { this->value = value; }

void* Iterator::GetValue(void) { return this->value; }

//-------------------------------------------------------------------------------

void Iterator::Ready(void)
{
...
void* IteratorShell(void* argument)
{
    ...
    try
    {
        if (pthread_setspecific(FLOW2D3D->dd->thiter, (void*)&iid) != 0)
            throw new Exception(
                "Pthreads error in IteratorShell: Cannot set thread-specific key for iterator thread, errno=%d", errno);
        ...
        iter->dd->log->Write(Log::ITER_MAJOR, "IteratorShell \"%s\" \"%s\" is waiting for initialization signal",
                             iter->category->name, iter->name);

        iter->sync->PSem();

        Blob* configblob = new Blob(FLOW2D3D->dd->iterator[iid].blobsize == 0 ? NULL : FLOW2D3D->dd->iterator[iid].blob,
                                    FLOW2D3D->dd->iterator[iid].blobsize);

        iter->dd->log->Write(Log::ITER_MAJOR, "IteratorShell: Invoking function for \"%s\" \"%s\"...",
                             iter->category->name, iter->name);

        FLOW2D3D->dd->iterator[iid].function(iter, iter->name, configblob);
