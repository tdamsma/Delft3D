...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/iterators/minimumbarrier.cpp
// $
//------------------------------------------------------------------------------
...
void MinimumBarrier_Function(Iterator* self, const char* name, Blob* configblob)
{
    ...
    while (true)
    {
        ...
        mesg.value = minvalue;
        for (i = 0; i < npart; i++) part[i]->Send(mblob);
    }
...
unsigned int MinimumBarrier(unsigned int value)
{
    ...
    else
    {
        ...
        if (value < MinimumBarrierState.localmin) MinimumBarrierState.localmin = value;
...
#if defined(WITH_MPI)
    #if defined(NO_CPP_MPI)
                MPI_Allreduce(&MinimumBarrierState.localmin, &MinimumBarrierState.globalmin, 1, MPI_UNSIGNED, MPI_MIN,
                              MinimumBarrierState.intracomm);
    #else
                MinimumBarrierState.intracomm.Allreduce(&MinimumBarrierState.localmin, &MinimumBarrierState.globalmin,
                                                        1, MPI::UNSIGNED, MPI::MIN);
    #endif
#endif
            }
