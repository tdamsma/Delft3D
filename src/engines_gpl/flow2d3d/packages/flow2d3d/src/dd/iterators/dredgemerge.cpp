...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/iterators/dredgemerge.cpp
// $
//------------------------------------------------------------------------------
...
void DredgeMerge_Function(Iterator* self, const char* name, Blob* configblob)
{
        ...
        while (true)
        {
            ...
            for (ipart = 0; ipart < npart; ipart++)
            {
                ...
                if (ipart == 0)
                    ...
                    throw new Exception("Dredge array size (%d) from Flow %s deviates from size (%d) from Flow %s",
            ...
            for (iarr = 0; iarr < numelements; iarr++) mergedvoldred[iarr] = 0.0;
            ...
            for (ipart = 0; ipart < npart; ipart++)
            {
                ...
                if (blobtype != DredgeMerge::F2DM_voldred)
                    throw new Exception("Unexpected message (%d) from Flow %s", blobtype, part[ipart]->name);
                for (iarr = 0; iarr < numelements; iarr++) mergedvoldred[iarr] += mesg[iarr];
            }

            // Send merged values to all participants by reusing mesg

            for (iarr = 0; iarr < numelements; iarr++) mesg[iarr] = mergedvoldred[iarr];
            for (ipart = 0; ipart < npart; ipart++) part[ipart]->Send(mblob, DredgeMerge::DM2F_mergedvoldred);
...
extern "C" {
void STDCALL START_DD_DREDGECOMMUNICATION(int* domainnumber, int* numdomains)
{
    Iterator* dredgemIterator = FLOW2D3D->dd->dredgem;

    Iterator* self = IteratorSelf();
    if (self == NULL) throw new Exception("Cannot get iterator self in DD_DredgeCommunicate");
    ...
    *numdomains = inimesg[1];
    ...
    if (*numdomains <= 1) subdomglobals->dredgecommunicate = false; // No DredgeDump communication
}
}
...
extern "C" {
void STDCALL DD_DREDGECOMMUNICATE(REAL_FP* voldred, int* numelements)
{
    Iterator* dredgemIterator = FLOW2D3D->dd->dredgem;
    ...
    for (int i = 0; i < *numelements; i++) mesg[i] = voldred[i];
    ...
    for (int i = 0; i < *numelements; i++) voldred[i] = mesg[i];
...
extern "C" {
void STDCALL NO_DD_DREDGECOMMUNICATION(void)
{
    Iterator* dredgemIterator = FLOW2D3D->dd->dredgem;
