...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/iterators/rtccomm.cpp
// $
//------------------------------------------------------------------------------
...
void Rtc_Function(Iterator* self, const char* name, Blob* configblob)
{
    ...
    while (eltremoved)
    {
        ...
        {
            if (part[i] == NULL) delelt = i;
            i++;
        }
        if (delelt != -1)
        {
            for (i = delelt; i < npart - 1; i++) part[i] = part[i + 1];
            npart--;
            eltremoved = true;
        }
    }
    ...
    while (true)
    {
        ...
        if (currentblobtype == RTC::F2RC_values)
        {
            ...
            for (iarr = 0; iarr < numelements; iarr++) mergedvalues[iarr] = 0.0;
            ...
            for (ipart = 0; ipart < npart; ipart++)
            {
                ...
                if (blobtype != RTC::F2RC_values)
                    throw new Exception("Unexpected message (%d) from Flow %s", blobtype, part[ipart]->name);
                for (iarr = 0; iarr < numelements; iarr++) mergedvalues[iarr] += mesg[iarr];
            }

            // Send merged values to all participants by reusing mesg

            for (iarr = 0; iarr < numelements; iarr++) mesg[iarr] = mergedvalues[iarr];
            for (ipart = 0; ipart < npart; ipart++) part[ipart]->Send(mblob, RTC::RC2F_values);
        ...
        else if (currentblobtype == RTC::F2RC_strings)
        {
            ...
            for (iarr = 0; iarr < numelements; iarr++) mergedvalues[iarr] = ' ';
            ...
            for (ipart = 0; ipart < npart; ipart++)
            {
                ...
                for (iarr = 0; iarr < numelements; iarr++)
                    if (mesg[iarr] != ' ') mergedvalues[iarr] = mesg[iarr];
            }

            // Send merged values to all participants by reusing mesg

            for (iarr = 0; iarr < numelements; iarr++) mesg[iarr] = mergedvalues[iarr];
            for (ipart = 0; ipart < npart; ipart++) part[ipart]->Send(mblob, RTC::RC2F_strings);
...
extern "C" {
void STDCALL DD_RTCSTARTCOMMUNICATION(int* domainnumber, int* numdomains)
{
    Iterator* self = IteratorSelf();
    if (self == NULL) throw new Exception("Cannot get iterator self in RtcStartCommunicate");
    ...
    if (blobtype != RTC::RC2F_init) throw new Exception("Unexpected message (%d) in %s from Rtc", blobtype, self->name);
    ...
    *numdomains = inimesg[1];
    ...
    if (*numdomains <= 1) subdomglobals->rtccommunicate = false; // No Rtc communication
}
}
...
extern "C" {
void STDCALL DD_RTCCOMMUNICATE(REAL_FP* values, int* numelements)
{
    Iterator* self = IteratorSelf();
    if (self == NULL) throw new Exception("Cannot get iterator self in RtcCommunicate");
    ...
    // Copy values to mesg
    for (int i = 0; i < mesgsize; i++) mesg[i] = values[i];
    ...
    for (int i = 0; i < mesgsize; i++) values[i] = mesg[i];
...
extern "C" {
void STDCALL DD_RTCCHARCOMMUNICATE(char* strings, int* numelements, int numchar)
{
    Iterator* self = IteratorSelf();
    if (self == NULL) throw new Exception("Cannot get iterator self in RtcCharCommunicate");
    ...
    for (int i = 0; i < mesgsize; i++) mesg[i] = strings[i];
    ...
    for (int i = 0; i < mesgsize; i++) strings[i] = mesg[i];
...
extern "C" {
void STDCALL DD_RTCNOCOMMUNICATION(void)
{
    Iterator* rtcIterator = FLOW2D3D->dd->rtc;
