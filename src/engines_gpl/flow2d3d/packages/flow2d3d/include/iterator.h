...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/iterator.h
// $
//------------------------------------------------------------------------------
...
typedef void (*IteratorFunction)(Iterator* self, const char* name, Blob* configblob);

class Iterator
{
public:
    Iterator(DD* dd, const char* name, Blob* configblob, Category* category, IteratorFunction function);
...
private:
    int FindJoin(int iter1, int iter2);

    void SendMessage(Blob*, int, Channel*);
    void ReceiveMessage(Blob*, int*, Channel*);
