...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/gaws.h
// $
//------------------------------------------------------------------------------
...
typedef struct
{                           // Gaws-info on a D3d flow process:
...
public:
    ...
    virtual int Setup(Iterator* gaws,                  // GAWS iterator
    ...
    virtual void GawsLog(char* format, // 'fprintf-format' for log
...
protected:
    ...
    virtual int FillMapperCells(MapperCells* cells, // cell-info to fill
                                GawsMapInfo mapper  // gaws info for a mapper
    );

    virtual int FindNeighborPoint(bool leftToRight,                 // boolean L->R or Top->Bottom
                                  ...
                                  int* nbrN,                        // N-index  "     "      "
                                  D3dFlowContextGawsSide** nbrCntxt // context of current domain
    );

    virtual int CreateWangEquations(WangEquations* Wang_Eq_XorY, // pointer Wang Eqs context
                                    WangDir wangDir,             // X- or Y-direction
                                    int level                    // Refinement level
    );

    virtual int CreateWangAdministration(WangDir wangDir // X- or Y-direction
    );

    virtual int ContinueInNextContext(int eqSerie,                     // current eq. series
                                      int curM,                        // M-index in this next domain
                                      int curN,                        // N-index  "  "    "      "
                                      D3dFlowContextGawsSide* curCntxt // context of current domain
    );

    virtual int CheckDependents(D3dFlowContextGawsSide* curCntxt, // context of current domain
    ...
    virtual int GetCentreLine(int map,    // mapper index
                              int myLine, // line in current domain
                              int ctx     // context index (0/1)
    );

    virtual int NewEquationSerie(bool leftToRight, // boolean L->R or Top->Bottom
                                 int level         // intial ref. level (0 / -1)
    );

    virtual void AddEqPoint(int eqSerie,                      // current eq.series
                            D3dFlowContextGawsSide* curCntxt, // context of current domain
    ...
    virtual void SetDependentLevels(int curSer // current eq.series
    );

    virtual int ProcessCouplePoint(D3dFlowContextGawsSide* curCntxt, // context of current domain
                                   bool leftToRight,                 // boolean L->R or Top->Bottom
    ...
    virtual void Solver(bool leftToRight // #x or y dir
    );
};
