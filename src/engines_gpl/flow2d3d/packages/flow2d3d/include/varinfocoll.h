...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/varinfocoll.h
// $
//------------------------------------------------------------------------------
...
public:
    ...
    VarInfoCollection(int maxBlockInfo, // max #blockAdmin.s to be stored
    ...
    BlockInfo* AddBlockInfo(int offset, int size, int amount_D1, int stride_D1);

    BlockInfo* AddBlockInfo(int offset, int size, int amount_D1, int stride_D1, int amount_D2, int stride_D2);

    BlockInfo* AddBlockInfo(int offset, int size, int amount_D1, int stride_D1, int amount_D2, int stride_D2,
                            int amount_D3, int stride_D3);

    BlockInfo* AddBlockInfo(int offset, int size, int amount_D1, int stride_D1, int amount_D2, int stride_D2,
                            int amount_D3, int stride_D3, int amount_D4, int stride_D4);

    //
    // Add var. administration ( for int, float )
    //

    void AddVarInfo(int varId, int* varAddress, VarType varType, BlockInfo* blockInfo);

    void AddVarInfo(int varId, float* varAddress, VarType varType, BlockInfo* blockInfo);

    void AddVarInfo(int varId, double* varAddress, VarType varType, BlockInfo* blockInfo);

    void AddGroup(int groupId // group be added
    );

    void AddVarToGroup(int groupId, // group to add var.to
                       int varId    // var to be added
    );

    int GetNumBytes(int groupId // group for which #bytes must be determined
    );
    ...
    void CopyVar(int varId,                           // identification
    ...
    // Find a var info
    //
    VarInfo* FindVar(int varId // var to be found
    );

    //
    // Find a var. group
    //
    VarGroup* FindGroup(int groupId // group to be found
    );

    //
    // Print info on a var
    //
    void PrintVarInfo(int varId // var to be printed
    );

    //
    // Print info on a var.-group
    //
    void PrintGroupInfo(int groupId // group to be printed
    );
