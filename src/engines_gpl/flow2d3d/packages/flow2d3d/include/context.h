...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/context.h
// $
//------------------------------------------------------------------------------
...
public:
    ...
    virtual void SetOtherContext(D3dFlowContext* otherC // pointer to other context
    );
    ...
    virtual void AttachVars(int cntxtId // context-id
    );

    int AttachContextVar(int aContextId, // context Id
                         void** var,     // adress of var. pointer
                         char* name      // name of var in context
    );

    int CreateContextVar(void** var, // adress of var. pointer
                         char* name, // name of var in context
...
//
    ...
    #define ATTACH(cid, var, name) AttachContextVar((cid), (void**)&(var), (name));

    #define CREATE_REAL_FP(var, name, size) CreateContextVar((void**)&(var), (name), sizeof(REAL_FP) * size);

    #define CREATE_REAL_PREC(var, name, size) CreateContextVar((void**)&(var), (name), sizeof(REAL_PREC) * size);

    #define CREATE_INT(var, name, size) CreateContextVar((void**)&(var), (name), sizeof(int) * size);
...
#define I3D(var, m, n, k) \
    *((var) + ((k) - 1) * (mArraySize) * (nArraySize) + ((m) + (mArrayOffset)) * (nArraySize) + ((n) + (nArrayOffset)))

#define I3DT(var, m, n, k) \
    *((var) + ((k)) * (mArraySize) * (nArraySize) + ((m) + (mArrayOffset)) * (nArraySize) + ((n) + (nArrayOffset)))

#define O3DT(m, n, k) \
    (((k)) * (mArraySize) * (nArraySize) + ((m) + (mArrayOffset)) * (nArraySize) + ((n) + (nArrayOffset)))

#define I4D(var, m, n, k, l)                                                                               \
    *((var) + ((l) - 1) * (mArraySize) * (nArraySize) * (kMax) + ((k) - 1) * (mArraySize) * (nArraySize) + \
      ((m) + (mArrayOffset)) * (nArraySize) + ((n) + (nArrayOffset)))

#define O4D(m, n, k, l)                                                                           \
    (((l) - 1) * (mArraySize) * (nArraySize) * (kMax) + ((k) - 1) * (mArraySize) * (nArraySize) + \
     ((m) + (mArrayOffset)) * (nArraySize) + ((n) + (nArrayOffset)))

#define I4DT(var, m, n, k, l)                                                                                \
    *((var) + ((l) - 1) * (mArraySize) * (nArraySize) * ((kMax) + 1) + ((k)) * (mArraySize) * (nArraySize) + \
      ((m) + (mArrayOffset)) * (nArraySize) + ((n) + (nArrayOffset)))

#define O4DT(m, n, k, l)                                                                            \
    (((l) - 1) * (mArraySize) * (nArraySize) * ((kMax) + 1) + ((k)) * (mArraySize) * (nArraySize) + \
     ((m) + (mArrayOffset)) * (nArraySize) + ((n) + (nArrayOffset)))
