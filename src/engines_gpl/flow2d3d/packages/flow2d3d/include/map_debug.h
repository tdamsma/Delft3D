...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/map_debug.h
// $
//------------------------------------------------------------------------------
...
#define MAPDBG_FUN(name)                                            \
    if (GetFuncPrintLevel(PRT_F1))                                  \
    {                                                               \
        FLOW2D3D->dd->log->Write(Log::ITER_MAJOR, "FUN: %s", name); \
    }

#define MAPDBG_FUN2(name)                                           \
    if (GetFuncPrintLevel(PRT_F2))                                  \
    {                                                               \
        FLOW2D3D->dd->log->Write(Log::ITER_MAJOR, "FUN: %s", name); \
    }

#define MAPDBG_FUN3(name)                                           \
    if (GetFuncPrintLevel(PRT_F3))                                  \
    {                                                               \
        FLOW2D3D->dd->log->Write(Log::ITER_MAJOR, "FUN: %s", name); \
    }
...
bool GetFuncPrintLevel(int callerLevel // print-level of calling function
);
