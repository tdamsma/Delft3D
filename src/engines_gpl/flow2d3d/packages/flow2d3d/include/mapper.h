...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/mapper/mapper.h
// $
//------------------------------------------------------------------------------
//  Flow2D3D DD Mapper
//
...
public:
    ...
    int Setup(Iterator* mapper,               // current mapper
...
protected:
    ...
    void FMapLog(char* format, // 'fprintf-format' for log
                 ...           // arguments of log message
    );

    void FMapLog2DIndex(char* messStr, // message string
                        int ctx,       // context counter
                        int mIndex,    // M index
                        int nIndex     // N index
    );

    int InitAndParseConfigString(char* configString // configuration file
    );

    int CheckConfig(void);

    int CheckConfigContext(int ctx // current context
    );

    int CheckConfigBothContexts(void);

    int GetStartCell(int curCtx, // current context
                     int eq      // equation type
    );

    virtual int GetEndCell(int ctx, // current context
                           int eq   // equation type
    );

    virtual int GetNormalCell(int ctx,     // current context
                              int eq,      // equation type
                              CtxType type // equation type
    );

    Vel GetVelocityOrientation(int curCtx, // current context
                               int eq      // equation type
    );

    virtual void CentreCellOtherContext(int ctx,         // current context
                                        int curCentre,   // current centre cell
                                        int* otherCentre // O: centre cell other context
    );

    virtual int Other_MN_Centre(int ctx,      // current context
                                int curCentre // current centre cell
    );
    ...
    void SendDataToFlow(int myStep,      // I: Step caller just completed
    ...
    virtual void InitKc(Eq eq // Equation type
    );

    virtual void CheckDDbounds(Eq eq // Equation type
    );

    virtual void CheckDepthUV(Eq eq // Equation type
    );

    virtual void InitFlowVariables(void);

    virtual void CopyFlowVariables(Seq seq // Type of variable
    );

    virtual void CopyOuterNormalVelocities(Seq seq // Type of variable
    );

    virtual void CopyOuterTangentVelocities(Seq seq // Type of variable
    );

    virtual void CopySediment(Seq seq // Type of variable
    );

    virtual void Build(Seq seq, // Type of variable
                       int l    // conc. loop counter
    );

    virtual int Proces(Seq seq, // Type of variable
                       int l    // conc. loop counter
    );

    virtual int CheckConvergence(Seq seq,               // Variable type
                                 int l,                 // conc. loop counter
                                 int resolve[NR_CNTXTS] // which context must be
    );                                                  //     resolved?

    virtual void Adjust(Seq seq,               // Type of variable
                        int l,                 // conc. loop counter
                        int resolve[NR_CNTXTS] // which context must be
    );                                         //     resolved?

    virtual double InterpolateNormalU1(int fineCtx, int coarseCtx, int fineN, int coarseM, int coarseN, int coarseK);

    virtual double InterpolateTangentialU1(int fineCtx, int coarseCtx, int fineM, int coarseM, int coarseN,
                                           int coarseK);

    virtual double InterpolateNormalV1(int fineCtx, int coarseCtx, int fineM, int coarseM, int coarseN, int coarseK);

    virtual double InterpolateTangentialV1(int fineCtx, int coarseCtx, int fineN, int coarseM, int coarseN,
                                           int coarseK);

    virtual double InterpolateS1(int fineCtx, int coarseCtx, int fineM, int fineN, int coarseM, int coarseN,
                                 int coarseK);
    virtual int CheckForDry();

    virtual void StateMachineAdiOrWang(int transportUsed, // boolean calculate transp.?
...
#define LOG_ITER(iter) ON_DEBUG(DBLEV3 + DBLEV6, FMapLog((char*)"\nIt %d ...\n", iter);)

#define cI2D(ctx, m, n, var) \
    *((C[ctx]->var) + (m + (C[ctx]->mArrayOffset)) * (C[ctx]->nArraySize) + (n + (C[ctx]->nArrayOffset)))

#define cI3D(ctx, m, n, k, var)                                                 \
    *((C[ctx]->var) + ((k) - 1) * (C[ctx]->mArraySize) * (C[ctx]->nArraySize) + \
      (m + (C[ctx]->mArrayOffset)) * (C[ctx]->nArraySize) + (n + (C[ctx]->nArrayOffset)))

#define cI4D(ctx, m, n, k, l, var)                                                                                    \
    *((C[ctx]->var) + ((l) - 1) * (C[ctx]->mArraySize) * (C[ctx]->nArraySize) * (C[ctx]->kMax) +                      \
      ((k) - 1) * (C[ctx]->mArraySize) * (C[ctx]->nArraySize) + (m + (C[ctx]->mArrayOffset)) * (C[ctx]->nArraySize) + \
      (n + (C[ctx]->nArrayOffset)))
...
static const char* seqTxt[NR_SEQ] = {
    "U", "V", "Zeta", "Conc", "Turb", "Zeta_ADI_St1", "Zeta_ADI_St2", "Conc_ADI_St1", "Conc_ADI_St2"};

static const Eq location[NR_SEQ] = {Eq_U, Eq_V, Eq_Zeta, Eq_Zeta, Eq_Zeta, Eq_Zeta, Eq_Zeta, Eq_Zeta, Eq_Zeta};
