...
// $HeadURL:
// https://svn.oss.deltares.nl/repos/delft3d/branches/research/Deltares/20110420_OnlineVisualisation/src/engines_gpl/flow2d3d/packages/flow2d3d/src/dd/dd.h
// $
//------------------------------------------------------------------------------
...
{
public:
    DD(Flow2D3D* flow, XmlTree* config);
...
private:
    ...
    void ReadDDBounds(FILE* input, const char* filename);

    Iterator* AddProcess(const char* name, const char* config, int node);

    Iterator* AddMapper(const char* name, const char* config, Iterator* leftprocess, Iterator* rightprocess, int node);

    void JoinIterators(Iterator* iter1, Iterator* iter2);
...
private:
    ...
    enum
    { // special thread self (thiter) values
        ID_MAIN = -12000,
        ID_MASTER,
        ID_SLAVE
    };
