...
void parallell_(/*** FORTRAN-90 -> C INTERFACE ***/
                int* pNumEvents)
{
    /*
    ...
    if ((s = getenv(ENVAR)) == NULL || sscanf(s, "%d", &nProcs) != 1 || nProcs < 0)
    {
        nProcs = 1;
    }
...
static void StartEvent(int i, int nEvents, event* ev)
{
    /*
...
static void WaitForEventTermination(int nEvents, event* evTab)
{
    /*
...
static void SummarizeEventTable(int nEvents, event* evTab)
{
    /*
