...
{
    date myDate(dateArray[0],  // year
                dateArray[1],  // month
                dateArray[2]); // day

    time_duration myDuration(dateArray[3],                                             // hours
                             dateArray[4],                                             // minutes
                             dateArray[5],                                             // seconds
...
char* utils::time2datetimestring(long long time, char* buffer)
{
    ...
    {
        time2date(time, dateArray);
        snprintf(buffer, 20, "%04d-%02d-%02d %02d:%02d:%02d", dateArray[0], dateArray[1], dateArray[2], dateArray[3],
...
bool utils::fileAvailable(string filename) { return access(filename.c_str(), 0) == 0; }
