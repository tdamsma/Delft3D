...
lookupTableConverter::lookupTableConverter(int n, double* xArray, double* yArray, interpolationOption intOpt,
...
double lookupTableConverter::convert(double xVal) { return interpolate(xArray, yArray, xVal); }
...
double lookupTableConverter::interpolate(vector<double>& x, vector<double>& y, double xVal)
{
    ...
    {
        if ((xVal >= x[it]) & (xVal < x[it + 1]) || (xVal <= x[it]) & (xVal > x[it + 1]))
        {
            if (intOpt == BLOCK)
            {
                return y[it];
    ...
    if ((xVal < x[0] && x[0] < x[n - 1]) || (xVal > x[0] && x[0] > x[n - 1]))
    {
    ...
    else if ((xVal >= x[n - 1] && x[n - 1] > x[0]) || (xVal <= x[n - 1] && x[n - 1] < x[0]))
    {
