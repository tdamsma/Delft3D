    ...
    : n(n), xArray(xArray), yArray(yArray), intOpt(intOpt), extOpt(extOpt)
{
    ...
    {
        throw runtime_error(
            "table needs >= 2 elements - monotonLookupTableConverter::monotonLookupTableConverter(int n, double "
            "*xArray, double *yArray, interpolationOption intOpt, interpolationOption extOpt)");
    }
    convertable = true;
    for (int i = 1; i < n; i++)
    {
        ...
        {
            std::stringstream ss;
            ss << "monotonLookupTableConverter::monotonLookupTableConverter(...) - error - table is not monoton at "
                  "index "
               << i;
...
void monotonLookupTableConverter::checkProperty(converter::propertyEnum prop)
{
    if (prop == converter::POSITIVE_VALUE)
    {
        for (int i = 1; i < n; i++)
        {
            ...
            {
                throw runtime_error(
                    "void monotonLookupTableConverter::checkProperty(converter::propertyEnum prop) - error - "
                    "non-negative value property violated");
            }
        }
    }
    ...
    else if (prop == converter::STRICTLY_MONOTONIC)
    {
        ...
        {
            throw runtime_error(
                "void monotonLookupTableConverter::checkProperty(converter::propertyEnum prop) - error - table is not "
                "strictly monotonic");
        }
    }
    else if (prop == converter::MONOTONIC_2ND)
    {
        ...
        for (int i = 2; i < n; i++)
        {
            ...
            {
                throw runtime_error(
                    "void monotonLookupTableConverter::checkProperty(converter::propertyEnum prop) - error - table "
                    "1st-order derivative is not monotonic");
            }
            else
            {
                dy = dyNew;
    ...
    {
        throw runtime_error(
            "void monotonLookupTableConverter::checkProperty(converter::propertyEnum prop) - error - property is not "
            "supported");
    }
}
...
double monotonLookupTableConverter::reverseConvert(double yVal)
{
    if (convertable == false)
        throw runtime_error(
            "double monotonLookupTableConverter::reverseConvert(double yVal) - table is not convertable");
...
double monotonLookupTableConverter::reverseConvertDer(double yVal)
{
    if (convertable == false)
        throw runtime_error(
            "double monotonLookupTableConverter::reverseConvertDer(double yVal) - table is not convertable");
...
monotonLookupTableConverter* monotonLookupTableConverter::getIntegrator()
{
     ...
     */
    return new monotonLookupTableConverter(n, xArray, yInt, monotonLookupTableConverter::LINEAR,
                                           monotonLookupTableConverter::LINEAR);
}
