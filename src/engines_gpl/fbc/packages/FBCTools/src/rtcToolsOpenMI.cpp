...
{
    string id =
        ((rtcToolsRuntime*)pObject)->getTimeSeriesModel()->getOpenMIInterface()->getInputExchangeItem(index)->getID();
...
{
    string id = ((rtcToolsRuntime*)pObject)
                    ->getTimeSeriesModel()
                    ->getOpenMIInterface()
                    ->getInputExchangeItem(index)
                    ->getElementID();
    utils::convert2CStr(id, retstring, len);
    return;
}
...
{
    string id =
        ((rtcToolsRuntime*)pObject)->getTimeSeriesModel()->getOpenMIInterface()->getInputExchangeItem(index)->getUnit();
    utils::convert2CStr(id, retstring, len);
    return;
}
...
{
    string id =
        ((rtcToolsRuntime*)pObject)->getTimeSeriesModel()->getOpenMIInterface()->getOutputExchangeItem(index)->getID();
    utils::convert2CStr(id, retstring, len);
    return;
}
...
dllexp void OmiWriteStateFile(void* pObject, char filepath[], char filename[])
{
    if (pObject != NULL)
    {
        ...
        string resolvedFilename =
            string(filepath).empty() ? string(filename) : string(filepath).append(string("/").append(filename));
