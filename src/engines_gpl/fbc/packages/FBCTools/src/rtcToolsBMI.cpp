...
#include "fbc_version.h"
    ...
    // See
    // https://social.msdn.microsoft.com/Forums/vstudio/en-US/e94865f7-84cd-4912-8339-6ee95fb58825/numericlimitsdoublemax-not-recognized?forum=vcgeneral
    #undef min
    #undef max
#endif
...
dllexp char* get_rtctools_version() { return rtctools_version; }

dllexp char* get_rtctools_version_id() { return rtctools_version_id; }
...
BMI_API int BMI::update(double dt)
{
    ...
    // Write output
    // We perform this task here, rather than after execute().  In this way, all set values are logged, regardless of
    // whether they are set implicitly or explicitly: Step n - 1:  Set values at n - 1, n    .  Log values at n - 1.
    // Compute values at n    . Step n    :  Set values at n    , n + 1.  Log values at n    .  Compute values at n + 1.
...
BMI_API void BMI::get_var_name(int n, char* name)
{
    ...
    map<string, int>::const_iterator it =
        runtime->getTimeSeriesModel()->getTimeSeriesTensor()->getScalarIDMap().begin();
...
BMI_API void BMI::get_current_time(double* current_time) { *current_time = step * dt; }
