    ...
    try
    {
        ...
        else
        {
            for (int step = runtime->iStart; step <= runtime->iEnd; step++)
            {
                ...
                // Write state
                auto currentTime =
                    runtime->getTimeSeriesModel()->getTimeSeriesTensor()->getTime(step) / 1000.0; // in seconds
                if (runtime->getStateFileExportTimeStep() > 0 && currentTime >= runtime->getStartExportStateFile() &&
...
rtcToolsRuntime::rtcToolsRuntime(const char schemaDir[], const char workDir[], versionInfo info)
{
    ...
    piDiagInterface::addLine(
        3, "rtcToolsRuntime: RTC-Tools, version " + info.version + "." + info.revision + " (" + license + ")");
...
void rtcToolsRuntime::parseRuntimeConfigFile()
{
    ...
    rtcRunTimeConfigProperties.schema_location(
    ...
    if (root->getStateFiles().present())
    {
        ...
        startExportStateFile =
            getTime(udsf.getStartDate().getDate(), udsf.getStartDate().getTime()) / 1000.0; // in seconds
    ...
    if (root->getPeriod().getPIInput().present())
    {
        ...
        if (!utils::fileAvailable(filename))
        {
            piDiagInterface::addLine(1,
                                     "void rtcToolsRuntime::parseRunTimeConfigSettings(), file not found - " + filename,
                                     RTCTOOLSRUNTIME_CODE);
            throw runtime_error(
                "void rtcToolsRuntime::parseRunTimeConfigSettings() - error - time series import file not found");
        }
        ::xml_schema::Properties timeSeriesCollectionComplexTypeProperties;
        timeSeriesCollectionComplexTypeProperties.schema_location(
    ...
    if (root->getPeriod().getPIRunFile().present())
    {
        ...
        runComplexTypeProperties.schema_location(
            "http://www.wldelft.nl/fews/PI",
    ...
    char buffer[500];
    char b[4][50];
    snprintf(buffer, sizeof(buffer), "%s %s - %s %s, dt[ms] = %d", utils::time2datestring(t1, b[0]),
             utils::time2timestring(t1, b[1]), utils::time2datestring(t2, b[2]), utils::time2timestring(t2, b[3]), dt);
    ...
    piDiagInterface::addLine(4, "rtcToolsRuntime: number of ensemble members = " + string(buffer),
                             RTCTOOLSRUNTIME_CODE);
    ...
    else if (root->getModes().present())
    {
        ...
        {
            runtimeSettings.modeInfo.push_back(getModeInfo(root->getModes().get().getMode()[i]));
        }
...
void rtcToolsRuntime::parseInputFiles()
{
    ...
    assertFile(filename);
    tsModel = new timeSeriesModel(sDir, wDir, filename, t1, t2, dt, nEnsemble, ensembleMap, allocateAdjoint,
                                  runtimeSettings.limitedMemory);
...
void rtcToolsRuntime::setParameters(int n, string* id, double* input)
{
    ...
    {
        piDiagInterface::addLine(1, "rtcToolsRuntime::setParameters(int n, double *input) - unknown error.",
                                 RTCTOOLSRUNTIME_CODE);
        throw;
    }
}
...
double rtcToolsRuntime::simulate()
{
    ...
    {
        piDiagInterface::addLine(1, "rtcToolsRuntime::simulate() - error - " + string(e.what()) + ".",
                                 RTCTOOLSRUNTIME_CODE);
        throw;
    }
    catch (...)
...
void rtcToolsRuntime::simulate(int iStep)
{
    try
    {
        ...
        {
            piDiagInterface::addLine(1, "void rtcToolsRuntime::simulate(int iStep), time step index outside of range.",
                                     RTCTOOLSRUNTIME_CODE);
            return;
        }
    ...
    {
        piDiagInterface::addLine(1, "rtcToolsRuntime::simulate(int iStep) - error - " + string(e.what()),
                                 RTCTOOLSRUNTIME_CODE);
        throw;
    }
    catch (...)
    {
        piDiagInterface::addLine(1, "rtcToolsRuntime::simulate(int iStep) - unknown error in rtcTools",
                                 RTCTOOLSRUNTIME_CODE);
        throw;
    }
}
...
#pragma omp parallel for schedule(dynamic)
            ...
            {
                {
                    piDiagInterface::addLine(
                        1,
                        "double rtcToolsRuntime::simulate(int n, double *input) - error in parallel simulation - " +
                            string(e.what()),
                        RTCTOOLSRUNTIME_CODE);
                    throw runtime_error(
                        "double rtcToolsRuntime::simulate(int n, double *input) - error in parallel simulation - " +
            ...
            {
                {
                    piDiagInterface::addLine(
                        1,
                        "double rtcToolsRuntime::simulate(int n, double *input) - unknown error in parallel simulation",
                        RTCTOOLSRUNTIME_CODE);
                    throw runtime_error(
                        "double rtcToolsRuntime::simulate(int n, double *input) - unknown error in parallel simulation "
                        "- ");
    ...
    else
    {
        ...
        catch (exception& e)
        {
            piDiagInterface::addLine(
                1,
                "double rtcToolsRuntime::simulate(int n, double *input) - error in sequential simulation - " +
                    string(e.what()),
                RTCTOOLSRUNTIME_CODE);
            throw runtime_error(
                "double rtcToolsRuntime::simulate(int n, double *input) - error in sequential simulation - " +
        ...
        {
            piDiagInterface::addLine(
                1, "double rtcToolsRuntime::simulate(int n, double *input) - unknown error in sequential simulation",
                RTCTOOLSRUNTIME_CODE);
            throw runtime_error(
                "double rtcToolsRuntime::simulate(int n, double *input) - unknown error in sequential simulation");
        }
    }
    ...
    {
        piDiagInterface::addLine(
            1, "double rtcToolsRuntime::simulate(int n, double *input) - objective function value is NaN",
            RTCTOOLSRUNTIME_CODE);
...
void rtcToolsRuntime::openOutput()
{
    ...
    {
        piDiagInterface::addLine(1, "void rtcToolsRuntime::openOutput(void) - error - " + string(e.what()),
                                 RTCTOOLSRUNTIME_CODE);
        throw;
    }
    catch (...)
    {
        piDiagInterface::addLine(1, "void rtcToolsRuntime::openOutput(void) - unknown error in rtcTools",
                                 RTCTOOLSRUNTIME_CODE);
        throw;
    }
...
void rtcToolsRuntime::writeOutput(int timeStep, bool isFinalTimeStep)
{
    ...
    {
        piDiagInterface::addLine(1, "void rtcToolsRuntime::writeOutput(int, bool) - error - " + string(e.what()),
                                 RTCTOOLSRUNTIME_CODE);
        throw;
    }
    catch (...)
    {
        piDiagInterface::addLine(1, "void rtcToolsRuntime::writeOutput(int, bool) - unknown error in rtcTools",
                                 RTCTOOLSRUNTIME_CODE);
        throw;
    }
}
...
void rtcToolsRuntime::finish(int timeStep /* =0*/)
{
    try
    {
        ...
        for (int i = 0; i < (int)runtimeSettings.modeInfo.size(); i++)
        {
            if (runtimeSettings.modeInfo[i].mode == FIRSTORDERSENSITIVITY ||
                runtimeSettings.modeInfo[i].mode == OPTIMIZE || runtimeSettings.modeInfo[i].mode == CLOSEDLOOP)
            {
    ...
    {
        piDiagInterface::addLine(1, "void rtcToolsRuntime::finish(void) - error - " + string(e.what()),
                                 RTCTOOLSRUNTIME_CODE);
        piDiagInterface::write();
        throw;
    }
    ...
    {
        piDiagInterface::addLine(1, "void rtcToolsRuntime::finish(void) - unknown error in rtcTools",
                                 RTCTOOLSRUNTIME_CODE);
        piDiagInterface::write();
...
void rtcToolsRuntime::getTimeSeriesMatrix(int nTimeStep, int nSeries, double* tsm)
{
    ...
    {
        timeseries::timeSeriesMatrix* tsm_temp =
            dynamic_cast<timeseries::timeSeriesMatrix*>(tsModel->getTimeSeriesTensor()->getTimeSeriesMatrix(0));
    ...
    {
        piDiagInterface::addLine(
            1, "void rtcToolsRuntime::getTimeSeriesMatrix - method not available if limitedMemory option is true",
            RTCTOOLSRUNTIME_CODE);
    }
}
...
void rtcToolsRuntime::setTimeSeriesMatrix(int nTimeStep, int nSeries, double* tsm)
{
    if (!runtimeSettings.limitedMemory)
    {
        timeseries::timeSeriesMatrix* tsMatrix =
            dynamic_cast<timeSeriesMatrix*>(tsModel->getTimeSeriesTensor()->getTimeSeriesMatrix(0));
    ...
    {
        piDiagInterface::addLine(
            1, "void rtcToolsRuntime::setTimeSeriesMatrix - method not available if limitedMemory option is true",
            RTCTOOLSRUNTIME_CODE);
    }
}

bool rtcToolsRuntime::showDebug() { return piDiagInterface::getLogLevel() == 4; }
...
void rtcToolsRuntime::assertFile(string filename)
{
    if (utils::fileAvailable(filename) == false)
    {
        ...
        piDiagInterface::addLine(1, "void rtcToolsRuntime::assertFile(string filename), " + filename,
                                 RTCTOOLSRUNTIME_CODE);
...
long long rtcToolsRuntime::getTimeStep(fews::TimeStepComplexType::UnitType unit)
{
    ...
    {
        throw runtime_error(
            "error - time step unit not implemented in long long "
            "rtcToolsHelperFunctions::getTimeStep(TimeStepComplexType::UnitType unit)");
    }
...
double rtcToolsRuntime::getStateFileExportTimeStep() const { return stateFileExportTimeStep; }

double rtcToolsRuntime::getStartExportStateFile() const { return startExportStateFile; };

double rtcToolsRuntime::getEndExportStateFile() const { return endExportStateFile; };

double rtcToolsRuntime::getLastTimeStateExported() { return lastTimeStateExported; };

void rtcToolsRuntime::setLastTimeStateExported(const double value) { lastTimeStateExported = value; };
