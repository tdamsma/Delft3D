...
void parameterInterface::addFileContent(string schemaDir, string filename, fileTypeEnum type, prefixEnum prefix)
{
    ...
    if (type == TREEVECTOR)
    {
        ...
        properties.schema_location("http://www.openda.org",
                                   utils::xsd_filename(utils::getAbsoluteFilename(schemaDir, "treeVector.xsd")));
    ...
    {
        ::xml_schema::Properties properties;
        properties.schema_location("http://www.wldelft.nl/fews/PI", utils::xsd_filename(utils::getAbsoluteFilename(
                                                                        schemaDir, "pi_modelparameters.xsd")));
            ...
            for (int j = 0; j < (int)root->getGroup()[i].getParameter().size(); j++)
            {
                ...
                else if (root->getGroup()[i].getParameter()[j].getTable().present())
                {
                    ...
                    if (t.getColumnIds().get().getB().present())
                    {
                        ...
                        if (t.getColumnTypes().present() &&
                            t.getColumnTypes().get().getB().get().compare("string") == 0)
                        {
...
void parameterInterface::setDblParameter(string id, double value) { dblParMap[id] = value; }
...
bool parameterInterface::getBoolParameter(string id)
{
    ...
    if (id[0] == '$')
    {
        ...
        {
            if (boolParMap.find(id.substr(1, id.size() - 2)) == boolParMap.end())
                throw runtime_error(
...
int parameterInterface::getIntParameter(string id)
{
    ...
    if (id[0] == '$')
    {
        if ((id.size() > 2) && (id[id.size() - 1] == '$'))
        {
            if (intParMap.find(id.substr(1, id.size() - 2)) == intParMap.end())
                throw runtime_error(
                    string("parameter interface: external parameter Id '" + id + "'not available in file").c_str());
...
string parameterInterface::getStringParameter(string id)
{
    ...
    if (id[0] == '$')
    {
        if ((id.size() > 2) && (id[id.size() - 1] == '$'))
        {
            if (stringParMap.find(id.substr(1, id.size() - 2)) == stringParMap.end())
                throw runtime_error(
                    string("parameter interface: external parameter Id '" + id + "'not available in file").c_str());
...
int parameterInterface::getNDblParameter() { return (int)dblParMap.size(); }
...
double parameterInterface::getDblParameter(string id)
{
    ...
    if (id[0] == '$')
    {
        if ((id.size() > 2) && (id[id.size() - 1] == '$'))
        {
            if (dblParMap.find(id.substr(1, id.size() - 2)) == dblParMap.end())
                throw runtime_error(
                    string("parameter interface: external parameter Id '" + id + "'not available in file").c_str());
...
{
    if (tableRowMap.find(id) == tableRowMap.end())
        throw runtime_error(
            string("parameter interface: external parameter Id '" + id + "'not available in file").c_str());
...
{
    if (tableColStrMap.find(id) == tableColStrMap.end())
        throw runtime_error(
            string("parameter interface: external parameter Id '" + id + "'not available in file").c_str());
...
{
    if (tableColDblMap.find(id) == tableColDblMap.end())
        throw runtime_error(
            string("parameter interface: external parameter Id '" + id + "'not available in file").c_str());
...
void parameterInterface::write(string filename)
{
    ...
    xmlFile
        << "<treeVectorFile xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns=\"http://www.openda.org\" "
           "xsi:schemaLocation=\"http://www.openda.org ../../../xsd/treeVector.xsd\">"
        << endl;
