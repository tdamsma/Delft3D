...
expression::expression(string id, string name, int nVec, INPUT x1In, OPERATOR op, INPUT x2In, vector<int> iYOut)
...
void expression::solve(double* stateOld, double* stateNew, long long t, double dt)
{
    for (int i = 0; i < nVec; i++)
    {
        double x1, x2;
        if (x1In.iVal[i] > -1)
            x1 = stateOld[x1In.iVal[i]];
        else
            x1 = x1In.val;
        if (x2In.iVal[i] > -1)
            x2 = stateOld[x2In.iVal[i]];
        else
            x2 = x2In.val;
        ...
        else if (op == POWER)
        {
            ...
            if (isInvalid)
            {
                if (diagnosticMessagesCounter < maxNumDiagnostic)
                {
                    ...
                    piDiagInterface::addLine(1, "expression::solve - warning invalid operands: base x1 " +
                                                    std::to_string(x1) + " exponent x2 " + std::to_string(x2));
                }
            }
            else
            {
...
void expression::solveDer(double* stateOld, double* stateNew, long long t, double dt, double* dStateOld,
                          double* dStateNew)
{
    for (int i = 0; i < nVec; i++)
    {
        ...
        double dX1, dX2;
        if (x1In.iVal[i] > -1)
            dX1 = dStateOld[x1In.iVal[i]];
        else
            dX1 = 0.0;
        if (x2In.iVal[i] > -1)
            dX2 = dStateOld[x2In.iVal[i]];
        else
            dX2 = 0.0;
        ...
        else if (op == POWER)
        {
            ...
            const bool isInvalid =
                x1 < 0.0 || abs(x1) <= eps && abs(x2) <= eps || abs(x1) <= eps && abs(x2 - 1.0) <= eps;
            ...
            if (x1 < 0.0)
            {
                if (diagnosticMessagesCounter < maxNumDiagnostic)
                {
                    piDiagInterface::addLine(
                        1, "expression::solveDer - warning invalid operands: base x1 " + std::to_string(x1));
                    diagnosticMessagesCounter++;
                }
            }
            else if (abs(x1) <= eps && abs(x2) <= eps)
            {
                if (diagnosticMessagesCounter < maxNumDiagnostic)
                {
                    piDiagInterface::addLine(1, "expression::solveDer - warning invalid operands: base x1 " +
                                                    std::to_string(x1) + " exponent x2 " + std::to_string(x2));
                    diagnosticMessagesCounter++;
                }
            }
            else if (abs(x1) <= eps && abs(x2 - 1.0) <= eps)
            {
                ...
                {
                    piDiagInterface::addLine(1, "expression::solveDer - warning invalid operands: base x1 " +
                                                    std::to_string(x1) + " exponent x2 - 1.0 " +
                                                    std::to_string(x2 - 1.0));
                    diagnosticMessagesCounter++;
                }
            }
            else if (!isInvalid)
