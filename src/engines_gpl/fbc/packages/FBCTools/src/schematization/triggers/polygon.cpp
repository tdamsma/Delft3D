    ...
    : nEdges(nEdges), x(x), y(y), value(value)
{
    ...
    if ((x[nEdges - 1] - x[0]) > 1e-10 || (y[nEdges - 1] - y[0]) > 1e-10)
    {
        throw std::runtime_error(
            "polygon::polygon(int nEdges, double *x, double *y, double value) - polygon not closed.");
    }
    ...
    {
        throw std::runtime_error(
            "polygon::polygon(int nEdges, double *x, double *y, double value) - polygon is not simple.");
    }
}
...
bool polygon::contains(double tx, double ty)
{
    ...
    {
        if (((y[i] > ty) != (y[j] > ty)) && (tx < (x[j] - x[i]) * (ty - y[i]) / (y[j] - y[i]) + x[i]))
        {
            c = !c;
        }
    }
