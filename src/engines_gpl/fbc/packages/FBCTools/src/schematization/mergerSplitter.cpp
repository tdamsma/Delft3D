...
mergerSplitter::mergerSplitter(string id, string name, modeEnum mode, INPUT iInput, OUTPUT iOutput)
...
void mergerSplitter::solve(double* stateOld, double* stateNew, long long t, double dt)
{
    ...
    if (iInput.iNStepStart > -1 && stateNew[iInput.iNStepStart] == stateNew[iInput.iNStepStart])
    {
        ...
        if (iStart < 0 || iStart > nInput - 1)
            throw runtime_error("void mergerSplitter::solve(...) - error in nStep value");
    }
    int iEnd = nInput - 1;
    if (iInput.iNStepEnd > -1 && stateNew[iInput.iNStepEnd] == stateNew[iInput.iNStepEnd])
    {
        iEnd = (int)stateNew[iInput.iNStepEnd];
        if (iEnd < 0 || iEnd > nInput - 1 || iEnd < iStart)
            throw runtime_error("void mergerSplitter::solve(...) - error in nStep value");
    }
...
void mergerSplitter::solveDer(double* stateOld, double* stateNew, long long t, double dt, double* dStateOld,
                              double* dStateNew)
{
    // optional nStep series
    int iStart = 0;
    if (iInput.iNStepStart > -1 && stateNew[iInput.iNStepStart] == stateNew[iInput.iNStepStart])
    {
        ...
        iStart = (int)stateNew[iInput.iNStepStart];
        if (iStart < 0 || iStart > nInput - 1)
            throw runtime_error("void mergerSplitter::solveDer(...) - error in nStep value");
    }
    int iEnd = nInput - 1;
    if (iInput.iNStepEnd > -1 && stateNew[iInput.iNStepEnd] == stateNew[iInput.iNStepEnd])
    {
