 ...
 */
pidControllerSobek2::pidControllerSobek2(string id, string name, double kp, double ki, double kd, double kf,
                                         double settingMin, double settingMax, double settingMaxSpeed, int iXIn,
                                         int iSPIn, int iFIn, int iYOut, int iIOut, int iDOut)
    : rule(id, name)
{
    this->kp = kp;
    this->ki = ki;
...
pidControllerSobek2::pidControllerSobek2(string id, string name, double kp, double ki, double kd, double kf,
                                         double settingMin, double settingMax, double settingMaxSpeed, int iXIn,
                                         double SPIn, int iFIn, int iYOut, int iIOut, int iDOut)
    : rule(id, name)
{
    this->kp = kp;
    this->ki = ki;
...
void pidControllerSobek2::solve(double* stateOld, double* stateNew, long long t, double dt)
{
    ...
    if (diffFromSetPSum == diffFromSetPSum)
    { // Check for NaN's
...
void pidControllerSobek2::solveDer(double* stateOld, double* stateNew, long long t, double dt, double* dStateOld,
                                   double* dStateNew)
{
    throw runtime_error(
        "void pidControllerSobek2::solveDer(double *stateOld, double *stateNew, long long t, double dt, double "
