 ...
 */
pidControllerVelocity::pidControllerVelocity(string id, string name, double kp, double ki, double kd, double kf,
                                             double settingMin, double settingMax, double settingMaxSpeed, int iXIn,
                                             int iSPIn, int iFIn, int iYOut, int iIOut, int iDOut, int numSeries)
    : rule(id, name)
{
    this->kp = kp;
    this->ki = ki;
...
pidControllerVelocity::pidControllerVelocity(string id, string name, double kp, double ki, double kd, double kf,
                                             double settingMin, double settingMax, double settingMaxSpeed, int iXIn,
                                             double SPIn, int iFIn, int iYOut, int iIOut, int iDOu, int numSeries)
    : rule(id, name)
{
    this->kp = kp;
    this->ki = ki;
...
void pidControllerVelocity::solve(double* stateOld, double* stateNew, long long t, double dt)
{
    ...
    double u_n = u_nd + this->kp * (this->e_n - this->e_nd) + this->ki * dt * this->e_n +
                 this->kd * (this->e_n - 2.0 * this->e_nd + this->e_ndd) / dt;
...
void pidControllerVelocity::solveDer(double* stateOld, double* stateNew, long long t, double dt, double* dStateOld,
                                     double* dStateNew)
{
    throw runtime_error(
        "void pidControllerVelocity::solveDer(double *stateOld, double *stateNew, long long t, double dt, double "
