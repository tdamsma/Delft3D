    ...
    : component(id, name), rule(id, name)
{
    ...
    {
        throw runtime_error(string("unitDelay::unitDelay(string id, string name, double nStep, int iXIn, OUTPUT "
                                   "iOutput) - nStep beyond output vector range, component = " +
                                   id)
                                .c_str());
    }
...
void unitDelay::solveDer(double* stateOld, double* stateNew, long long t, double dt, double* dStateOld,
                         double* dStateNew)
{
    // TODO min, max
