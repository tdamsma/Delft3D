...
timeSeriesSparseMatrix::timeSeriesSparseMatrix(int nTimeStep, vector<long long> time, int nSeries,
                                               vector<string> seriesID, vector<validationEnum> seriesValidation,
 ...
 */
void timeSeriesSparseMatrix::setSeries(int sIndex, vector<long long>* times, vector<double>* vals, long long t1,
...
double* timeSeriesSparseMatrix::getState(int tIndex)
{
    ...
    throw runtime_error("timeSeriesSparseMatrix::getState(int tIndex) - time series index " + std::to_string(tIndex) +
                        " is invalid. Legal indexes values are " + std::to_string(iState - 3) + " " +
                        std::to_string(iState - 2) + " " + std::to_string(iState - 1));
}
...
void timeSeriesSparseMatrix::incrementTimeStep()
{
    ...
    {
        throw runtime_error(
            "timeSeriesSparseMatrix::incrementTimeStep: state was not initialized yet, no increment possible.");
    }
    iState++;
    assert(iState > 1);
...
double timeSeriesSparseMatrix::getInterpolatedValue(int tIndex, int sIndex)
{
    ...
    {
        if (sparseseriesvector.at(i).getSeriesIndex() == sIndex)
        {
            index = i;
        }
        i++;
    ...
    double val = getValue_sparseseries(time, sparsetime, ts->getvals(), ts->getStarttime(), ts->getEndtime(),
                                       ts->getdt(), ts->getInterpolationOption(), ts->getExtrapolationOption());
    return val;
}

double timeSeriesSparseMatrix::getValue_sparseseries(long long time, vector<long long> tArray, vector<double> vArray,
                                                     long long t1, long long t2, long long dt,
                                                     interpolationOption intOpt, interpolationOption extOpt)
{
    ...
    else if (idx_left < nTimeSparse - 1)
    {
        ...
        else if (intOpt == LINEAR)
        {
            ...
            return (vArray[idx_left] +
                    (double)(dt_left) / (double)(dt_right + dt_left) * (vArray[idx_left + 1] - vArray[idx_left]));
        }
    }
    else if (idx_left == nTimeSparse - 1)
    {
        ...
        else if (extOpt == PERIODIC)
        {
            ...
            else if (intOpt == LINEAR)
            {
                ...
                dt_right = t2 + dt - time;
                return (vArray[nTimeSparse - 1] +
                        (double)(dt_left) / (double)(dt_right + dt_left) * (vArray[0] - vArray[nTimeSparse - 1]));
            }
        }
    }
