...
timeSeriesMatrix::timeSeriesMatrix(int nTimeStep, vector<long long> time, int nSeries, vector<string> seriesID,
...
timeSeriesMatrix::~timeSeriesMatrix(void) { delete[] state; }
...
double* timeSeriesMatrix::getState(int tIndex)
{
    ...
    throw runtime_error("timeSeriesMatrix::getState(int tIndex) - time series index " + std::to_string(tIndex) +
    ...
    for (int i = 0; i < nSeriesEnd; i++)
    {
        ...
        else if (seriesValidation[i] == VALIDATION_UPDATE)
        {
            for (int j = 0; j <= nTimeStepT0; j++)
            {
                ...
                {
                    ss << "void timeSeriesMatrix::validate() - error in time series with ID = '" << seriesID[i]
                       << "' at time index " << j << " for VALIDATION_UPDATE";
                    throw runtime_error(ss.str().c_str());
                }
            }
        }
        else if (seriesValidation[i] == VALIDATION_UPDATE_EXCEPT_STATE)
        {
            for (int j = 1; j <= nTimeStepT0; j++)
            {
                ...
                {
                    ss << "void timeSeriesMatrix::validate() - error in time series with ID = '" << seriesID[i]
                       << "' at time index " << j << " for VALIDATION_UPDATE_EXCEPT_STATE";
                    throw runtime_error(ss.str().c_str());
                }
            }
        }
        else if (seriesValidation[i] == VALIDATION_FORECAST)
        {
            for (int j = nTimeStepT0; j < nTimeStep; j++)
            {
                ...
                {
                    ss << "void timeSeriesMatrix::validate() - error in time series with ID = '" << seriesID[i]
                       << "' at time index " << j << " for VALIDATION_FORECAST";
                    throw runtime_error(ss.str().c_str());
                }
            }
        }
        else if (seriesValidation[i] == VALIDATION_FORECAST_EXCEPT_T0)
        {
            for (int j = nTimeStepT0 + 1; j < nTimeStep; j++)
            {
                ...
                {
                    ss << "void timeSeriesMatrix::validate() - error in time series with ID = '" << seriesID[i]
                       << "' at time index " << j << " for VALIDATION_FORECAST_EXCEPT_T0";
                    throw runtime_error(ss.str().c_str());
                }
            }
        }
        else if (seriesValidation[i] == VALIDATION_ALL)
        {
            for (int j = 0; j < nTimeStep; j++)
            {
                ...
                {
                    ss << "void timeSeriesMatrix::validate() - error in time series with ID = '" << seriesID[i]
                       << "' at time index " << j << " for VALIDATION_ALL";
                    throw runtime_error(ss.str().c_str());
                }
            }
        }
        else if (seriesValidation[i] == VALIDATION_ALL_EXCEPT_STATE)
        {
            for (int j = 1; j < nTimeStep; j++)
            {
                ...
                {
                    ss << "void timeSeriesMatrix::validate() - error in time series with ID = '" << seriesID[i]
                       << "' at time index " << j << " for VALIDATION_ALL_EXCEPT_STATE";
                    throw runtime_error(ss.str().c_str());
                }
            }
        }
