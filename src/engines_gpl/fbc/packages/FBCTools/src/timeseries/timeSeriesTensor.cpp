...
timeSeriesTensor::timeSeriesTensor(int nEnsemble, int nTimeStep, vector<long long> time, int nSeries,
    ...
    : timeSeriesInterface(nTimeStep, time, nSeries, seriesID, seriesValidation, scalarIDMap, vectorIDMap)
{
    ...
    for (int i = 0; i < nEnsemble; i++)
    {
        valueMatrix[i] = new timeSeriesMatrix(nTimeStep, time, nSeries, seriesID, seriesValidation, scalarIDMap,
...
int timeSeriesTensor::getNEnsemble() { return nEnsemble; }

int timeSeriesTensor::getNSeriesMatrix() { return nSeries; }

timeSeriesMatrix* timeSeriesTensor::getTimeSeriesMatrix(int ensembleId) { return valueMatrix[ensembleId]; }

double*** timeSeriesTensor::getValueTensor() { return valueTensor; }

double*** timeSeriesTensor::getObjTensor() { return objTensor; }
...
void timeSeriesTensor::validate(int nSeriesEnd, int nTimeStepT0)
{
    for (int i = 0; i < nEnsemble; i++)
    {
        ...
        {
            stringstream ss;
            ss << "void timeSeriesTensor::validate() - validation error in time series matrix " << i << ": "
               << e.what();
...
vector<int> timeSeriesTensor::getEnsembleMap() { return this->ensembleMap; }

void timeSeriesTensor::setEnsembleMap(vector<int> ensembleMap) { this->ensembleMap = ensembleMap; }
