...
namespace rtctools
{
    namespace timeseries
    {
        ...
        private:
            ...
            // where to look for pi schema (default is
                    ...
                    for (int j = iArray[i] + 1; j < iArray[i + 1]; j++)
                    {
                        ...
                        else if (intOpt == LINEAR)
                        {
                            vArray[j] = (float)(j - iArray[i]) / (float)(iArray[i + 1] - iArray[i]) *
                                            (vArray[iArray[i + 1]] - vArray[iArray[i]]) +
                                        vArray[iArray[i]];
                        }
                    }
                }
        ...
        public:
            piTimeSeriesSAX2Handler(boost::filesystem::path schemaLocation, timeSeriesInterface* valueInterface,
                                    vector<piTimeSeries> input, string outputFile, bool outputBin,
                                    vector<piTimeSeries> output, int nEnsemble);
            ...
            void write(string filename, vector<string> series, int nEnsemble, double*** tensor, string par_prefix = "");
            void write(string filename, vector<piTimeSeries> series, int nEnsemble, double*** tensor,
                       string par_prefix = "", const stringContainer& sc = stringContainer());
            long long getUnit(string unit);
            void transformation(vector<float>& valueArray, interpolationOption intOpt, interpolationOption extOpt);
        };
