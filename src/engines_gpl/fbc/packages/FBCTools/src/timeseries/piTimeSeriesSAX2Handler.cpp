...
piTimeSeriesSAX2Handler::piTimeSeriesSAX2Handler(boost::filesystem::path schemaLocation,
                                                 timeSeriesInterface* tsInterface, vector<piTimeSeries> input,
...
void piTimeSeriesSAX2Handler::startElement(const XMLCh* const uri, const XMLCh* const localname,
                                           const XMLCh* const qname, const Attributes& attrs)
{
    ...
    else if (XMLString::compareIString(message, "timeStep") == 0)
    {
        ...
        for (XMLSize_t i = 0; i < attrs.getLength(); i++)
        {
            ...
            if (XMLString::compareIString(nameCArray, "unit") == 0) u = getUnit(string(dataCArray));
            if (XMLString::compareIString(nameCArray, "multiplier") == 0) m = atoi(dataCArray);
            if (XMLString::compareIString(nameCArray, "divider") == 0) d = atoi(dataCArray);
...
void piTimeSeriesSAX2Handler::characters(const XMLCh* const chars, const XMLSize_t length)
{
...
void piTimeSeriesSAX2Handler::endElement(const XMLCh* const uri, const XMLCh* const localname, const XMLCh* const qname)
{
    char* message = XMLString::transcode(localname);
    ...
    if (XMLString::compareIString(message, "series") == 0)
    {
        ...
        for (multimap<string, piTimeSeries>::iterator iter = range.first; iter != range.second; ++iter)
        {
            ...
            else
            {
                ...
                dynamic_cast<timeSeriesSparseTensor*>(tsInterface)
                    ->setSeries(eIndex, sIndex, &temptime, &tempvals, t1F, t2F, dt, interpol, extrapol);
...
void piTimeSeriesSAX2Handler::error(const SAXParseException& e)
{
    ...
    XERCES_STD_QUALIFIER cerr << "\nError at file " << inputFile << ", line " << e.getLineNumber() << ", char "
...
{
    char* messCArray = XMLString::transcode(e.getMessage());
    XERCES_STD_QUALIFIER cerr << "\nFatal Error at file " << inputFile << ", line " << e.getLineNumber() << ", char "
                              << e.getColumnNumber() << "\n  Message: " << messCArray << XERCES_STD_QUALIFIER endl;
    XMLString::release(&messCArray);
}
...
{
    char* messCArray = XMLString::transcode(e.getMessage());
    XERCES_STD_QUALIFIER cerr << "\nWarning at file " << inputFile << ", line " << e.getLineNumber() << ", char "
                              << e.getColumnNumber() << "\n  Message: " << messCArray << XERCES_STD_QUALIFIER endl;
    XMLString::release(&messCArray);
}
...
void piTimeSeriesSAX2Handler::read(string filename, bool hasBin)
{
    ...
    string schemaLoc("http://www.wldelft.nl/fews/PI " +
                     utils::xsd_filename(utils::getAbsoluteFilename(schemaLocation, "pi_timeseries.xsd")));
...
void piTimeSeriesSAX2Handler::write(bool adjointOutput)
{
    if (!this->limitedMemory)
    {
        this->write(outputFile, output, this->nEnsemble, dynamic_cast<timeSeriesTensor*>(tsInterface)->getValueTensor(),
                    "");
    }
    else
    {
...
void piTimeSeriesSAX2Handler::write(string filename, vector<string> id, int nEnsemble, double*** tensor,
                                    string par_prefix)
{
    int n = (int)id.size();
...
void piTimeSeriesSAX2Handler::write(string filename, vector<piTimeSeries> series, int nEnsemble, double*** tensor,
                                    string par_prefix, const stringContainer& sc)
{
    char datestring[30];
    char timestring[30];
    ...
    if (outputBin)
    {
        binFile.open((filename.substr(0, filename.size() - 4).append(".bin")).c_str(),
    ...
    xmlFile << WSTRING(
                   ...
                   "xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns=\"http://www.wldelft.nl/fews/PI\">")
            << endl;
    ...
    for (int i = 0; i < nEnsemble; i++)
    {
        for (int j = 0; j < (int)series.size(); j++)
        {
            ...
            xmlFile << WSTRING("      <parameterId>" << par_prefix.c_str() << series[j].getParameterID().c_str()
                                                     << "</parameterId>")
                    << endl;
            ...
            xmlFile << "      <timeStep unit=\"second\" multiplier=\"" << tsInterface->getDT() / 1000 << "\" />"
                    << endl;
            xmlFile << "      <startDate date=\"" << utils::time2datestring(tsInterface->getStartTime(), datestring)
                    << "\" time=\"" << utils::time2timestring(tsInterface->getStartTime(), timestring) << "\" />"
                    << endl;
            xmlFile << "      <endDate date=\"" << utils::time2datestring(tsInterface->getEndTime(), datestring)
                    << "\" time=\"" << utils::time2timestring(tsInterface->getEndTime(), timestring) << "\" />" << endl;
            ...
            for (int k = 0; k < tsInterface->getNTimeStep(); k++)
            {
                ...
                if (value != value)
                {
                    ...
                    {
                        xmlFile << "    <event date=\"" << utils::time2datestring(time[k], datestring) << "\" time=\""
                                << utils::time2timestring(time[k], timestring) << "\" ";
