...
timeSeriesSparseTensor::timeSeriesSparseTensor(int nEnsemble, int nTimeStep, vector<long long> time, int nSeries,
    ...
    : timeSeriesInterface(nTimeStep, time, nSeries, seriesID, seriesValidation, scalarIDMap, vectorIDMap)
{
    ...
    {
        timeSeriesSparseMatrix* sm = new timeSeriesSparseMatrix(nTimeStep, time, nSeries, seriesID, seriesValidation,
                                                                scalarIDMap, vectorIDMap, nimport);
...
int timeSeriesSparseTensor::getNEnsemble() { return this->nEnsemble; }
...
void timeSeriesSparseTensor::setValue(int ensembleId, int tIndex, int sIndex, double value)
{
    ...
    if (ensembleId < 0)
    {
        ...
        for (int i = 0; i < nEnsemble; i++)
        {
            ...
            if (!set)
            {
                throw runtime_error("timeSeriesSparseTensor::setValue: no reset possible for given indices.");
            }
        }
    }
    else
...
void timeSeriesSparseTensor::setSeries(int ensembleId, int sIndex, vector<long long>* times, vector<double>* vals,
                                       long long t1, long long t2, long long dt, interpolationOption interpol,
...
int timeSeriesSparseTensor::getNSeriesMatrix() { return smVector.at(0).nSparse; }
...
vector<int> timeSeriesSparseTensor::getEnsembleMap() { return this->ensembleMap; }

void timeSeriesSparseTensor::setEnsembleMap(vector<int> ensembleMap) { this->ensembleMap = ensembleMap; }

void timeSeriesSparseTensor::incrementTimeStep(int ensembleId) { smVector.at(ensembleId).incrementTimeStep(); }
