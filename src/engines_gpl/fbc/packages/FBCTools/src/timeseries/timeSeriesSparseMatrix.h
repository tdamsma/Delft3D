...
namespace rtctools
{
    namespace timeseries
    {
        ...
        public:
             ...
             * @brief Constructor
             */
            timeSeriesSparseMatrix(int nTimeStep, vector<long long> time, int nSeries, vector<string> seriesID,
                                   vector<validationEnum> seriesValidation, map<string, int> scalarIDMap,
                                   map<string, pair<int, int>> vectorIDMap, int nimport);
            /**
             * @brief Destructor
             */
             ...
             *
             * @return       true if value in state was (re)set, false if input indices are out of bounds for the state
             * vector.
             */
            ...
            void setSeries(int sIndex,
                           ...
                           interpolationOption interpol, ///< interpolation option to be used in retrieving values for
                                                         ///< times in between index values
                           interpolationOption extrapol  ///< extrapolation option to be used in retrieving values for
                                                         ///< times outside index values
            );
            /**
             ...
             * @return the interpolated value
             */
            double getValue_sparseseries(
                long long t,              ///< the current time
                vector<long long> tArray, ///<
                vector<double> vArray,    ///< vector containing the values of the sparse timeseries
                long long t1, long long t2,
                long long dt, ///<
                interpolationOption
                    intOpt, ///< interpolation option to be used in retrieving values for times in between index values
                interpolationOption
                    extOpt ///< extrapolation option to be used in retrieving values for times outside index values
            );
