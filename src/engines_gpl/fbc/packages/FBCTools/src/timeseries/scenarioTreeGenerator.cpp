...
vector<vector<double>> scenarioTreeGenerator::getEnsemble(timeSeriesTensorInterface* tsTensor, int indx)
{
    ...
    for (int i = 0; i < tsTensor->getNEnsemble(); i++)
    {
        ...
        for (int j = 1; j < tsTensor->getNTimeStep(); j++)
        {
            ...
            {
                throw runtime_error(
                    "vector<vector<double> > scenarioTreeGenerator::getEnsemble(...) - error - NaN value detected");
            }
        }
...
void scenarioTreeGenerator::reduction(int nStep, vector<vector<double>> ensemble, vector<double>& p,
                                      vector<vector<int>>& direct, int s)
{
    int nTree = 0;
    ...
    for (int step = 0; step < nTree - s; step++)
    {
        ...
        for (int l = 0; l < nEns; l++)
        {
            ...
            for (int j = 0; j < nEns; j++)
            {
                ...
                if (p[l] > 0.0 && j != l && find(J.begin(), J.end(), j) == J.end())
                    z[l] = min(z[l], p[l] * distance[l][j]);
            }
        }
    ...
    for (int i = 0; i < (int)J.size(); i++)
    {
        ...
        for (int j = 0; j < nEns; j++)
        {
            if (distance[J[i]][j] < dMin                  // smallest distance so far
                ...
                && p[j] > 0.0)
            { // still active, 0 if already reduced
                dMin = distance[J[i]][j];
                indx = j;
            }
        }
        ...
        for (int j = 0; j < nStep; j++)
        {
            ...
            for (int k = 0; k < nEns; k++)
                if (direct[k][j] == toReplace) direct[k][j] = indx;
        }
    }
}

void scenarioTreeGenerator::average(timeSeriesTensorInterface* tsTensor, vector<vector<int>>& direct, vector<double>& p,
                                    int in, int out)
{
...
scenarioTreeGenerator::scenarioTree scenarioTreeGenerator::generateScenarioTree(vector<double> p,
