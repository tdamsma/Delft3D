...
/* constructor */
timeSeriesModel::timeSeriesModel(boost::filesystem::path schemaDir, boost::filesystem::path workDir, string filename,
                                 ...
                                 bool adjointOutput, bool limitedMemory)
{
    ...
    {
        // read data
        readTimeSeriesTensor(schemaDir, workDir, filename, t1, t2, dt, nEnsemble, ensembleMap, adjointOutput,
                             limitedMemory);
    }
    catch (exception& e)
    {
        piDiagInterface::addLine(1, "timeSeriesModel::timeSeriesModel(...) - error - " + string(e.what()),
                                 TIMESERIESMODEL_CODE);
        throw;
    }
}
 ...
 */
void timeSeriesModel::write(bool adjointOutput, int timeStep, bool isLastTimeStep,
                            std::vector<string>& additionalTimeseriesNames, std::vector<int>& additionalTimeseries)
{
    try
    {
        if (isLastTimeStep)
...
timeSeriesTensorInterface* timeSeriesModel::getTimeSeriesTensor() { return valueTensor; }

openMIInterface* timeSeriesModel::getOpenMIInterface() { return openMIInt; }

void timeSeriesModel::readTimeSeriesTensor(boost::filesystem::path schemaDir, boost::filesystem::path workDir,
                                           ...
                                           vector<int>& ensembleMap, bool adjointOutput, bool limitedMemory)
{
    ...
    piDiagInterface::addLine(4, "timeSeriesModel: number of time steps = " + to_string(nTimeStep),
                             TIMESERIESMODEL_CODE);

    // rtc data config
    ::xml_schema::Properties rtcDataConfigProperties;
    rtcDataConfigProperties.schema_location(
        "http://www.wldelft.nl/fews", utils::xsd_filename(utils::getAbsoluteFilename(schemaDir, "rtcDataConfig.xsd")));
    ...
    // diagnostics
    piDiagInterface::addLine(4, "time series model: number of import time series = " + to_string(nImport),
                             TIMESERIESMODEL_CODE);
    piDiagInterface::addLine(4, "time series model: number of export time series = " + to_string(nExport),
                             TIMESERIESMODEL_CODE);
    piDiagInterface::addLine(4, "time series model: total number of time series = " + to_string(nSeries),
                             TIMESERIESMODEL_CODE);

    if (limitedMemory)
    {
        timeSeriesSparseTensor* tsSparseTensor = new timeSeriesSparseTensor(
            nEnsemble, nTimeStep, time, nSeries, idArray, validationArray, scalarIDMap, vectorIDMap, nImport);
        valueTensor = dynamic_cast<timeSeriesTensorInterface*>(tsSparseTensor);
    }
    else
    {
        timeSeriesTensor* tsTensor = new timeSeriesTensor(nEnsemble, nTimeStep, time, nSeries, idArray, validationArray,
                                                          scalarIDMap, vectorIDMap);
    ...
    if (dataConfig->getExportSeries().getCSVTimeSeriesFile().present())
    {
        char decimalSeparator =
            dataConfig->getExportSeries().getCSVTimeSeriesFile().get().getDecimalSeparator().c_str()[0];
    ...
    {
        importPITimeSeriesFile = utils::getAbsoluteFilename(
            workDir, string(dataConfig->getImportSeries().getPITimeSeriesFile().get().getTimeSeriesFile()));
        // check file for availability
        if (!utils::fileAvailable(importPITimeSeriesFile))
        {
            string m =
                "void timeSeriesModel::readTimeSeriesTensor(...) - file not available: " + importPITimeSeriesFile;
    ...
    {
        exportPITimeSeriesFile = utils::getAbsoluteFilename(
            workDir, string(dataConfig->getExportSeries().getPITimeSeriesFile().get().getTimeSeriesFile()));
    ...
    for (int i = 0; i < (int)rtcTSI.size(); i++)
    {
        if (rtcTSI[i].getPITimeSeries().present())
        {
            ...
            {
                if (piTS.getInterpolationOption().get().compare("BLOCK") == 0)
                    intOpt = BLOCK;
                else if (piTS.getInterpolationOption().get().compare("LINEAR") == 0)
                    intOpt = LINEAR;
                else
                    throw runtime_error(
                        "void timeSeriesModel::readTimeSeriesTensor(string schemaLocation, string runtimeFilename, "
                        "string dataFilename) - interpolation option not implemented");
            }
            if (piTS.getExtrapolationOption().present())
            {
                if (piTS.getExtrapolationOption().get().compare("BLOCK") == 0)
                    extOpt = BLOCK;
                else if (piTS.getExtrapolationOption().get().compare("PERIODIC") == 0)
                    extOpt = PERIODIC;
                else
                    throw runtime_error(
                        "void timeSeriesModel::readTimeSeriesTensor(string schemaLocation, string runtimeFilename, "
                        "string dataFilename) - extrapolation option not implemented");
            }
            ...
            if (rtcTSI[i].getVectorLength().present())
            {
                for (int j = 0; j < rtcTSI[i].getVectorLength().get(); j++)
                {
                    stringstream ss;
                    ss << locID << "[" << j << "]";
                    importPITimeSeries[piIndex + j] =
                        piTimeSeries(id[rtcIndex + j], rtcIndex + j, ss.str(), parID, quaID, unit, intOpt, extOpt);
                }
                piIndex += rtcTSI[i].getVectorLength().get();
            }
            else
            {
                importPITimeSeries[piIndex] =
                    piTimeSeries(id[rtcIndex], rtcIndex, locID, parID, quaID, unit, intOpt, extOpt);
                piIndex++;
            }
        }
        if (rtcTSI[i].getVectorLength().present())
    ...
    {
        piSAX2Int = new piTimeSeriesSAX2Handler(schemaDir, valueTensor, importPITimeSeries, exportPITimeSeriesFile,
                                                exportBin, exportPITimeSeries, nEnsemble);
        ...
        if (rtcTSI[i].getOpenMIExchangeItem().present())
        {
            ...
            if (rtcTSI[i].getVectorLength().present())
            {
                for (int j = 0; j < rtcTSI[i].getVectorLength().get(); j++)
                {
                    stringstream ss;
                    ss << eID << "[" << j << "]";
                    inputExchangeItem[oMIIndex + j] =
                        openMIExchangeItem(id[rtcIndex + j], rtcIndex + j, ss.str(), qID, unit);
                }
    ...
    openMIInt = new openMIInterface(valueTensor->getTimeSeriesMatrix(0), inputExchangeItem, outputExchangeItem);
}
...
void timeSeriesModel::writeState(string filename, int tIndex) { this->stateInt->writeData(filename, tIndex); }
