...
stateInterface::stateInterface(boost::filesystem::path schemaDir, boost::filesystem::path workDir,
...
void stateInterface::read()
{
    ...
    {
        piDiagInterface::addLine(2,
                                 "stateInterface: number of states in state file is not consistent with schematization",
                                 STATEINTERFACE_CODE);
    }
    ...
    for (int i = 0; i < nSeriesFile; i++)
    {
        ...
        catch (...)
        {
            piDiagInterface::addLine(
                2, "stateInterface: id = '" + stateId + "' not found in time series model, skipping state value",
                STATEINTERFACE_CODE);
            continue;
        }
        ...
        else
        {
            ...
            if (!isnan(stateValue_m3))
            {
                ...
                piDiagInterface::addLine(4,
                                         "stateInterface: id = '" + stateId +
                                             "' found in time series model, state value = '" + stateValueString +
                                             "' replaced.",
                                         STATEINTERFACE_CODE);
            }
            else
...
void stateInterface::write(int tIndex)
{
    ...
    piDiagInterface::addLine(4, "stateInterface: start writing meta data file for states 'statePI.xml'",
                             STATEINTERFACE_CODE);

    xmlFile.open(utils::getAbsoluteFilename(workDir, "statePI.xml").c_str(), ios::out);

    xmlFile << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" << endl;
    xmlFile << "<State xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns=\"http://www.wldelft.nl/fews/PI\" "
               "xsi:schemaLocation=\"http://www.wldelft.nl/fews/PI ../../../../xsd/pi_state.xsd\" version=\"1.2\">"
            << endl;
