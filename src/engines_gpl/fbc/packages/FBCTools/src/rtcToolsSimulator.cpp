...
rtcToolsSimulator::rtcToolsSimulator(int iEnsemble, timeSeriesMatrixInterface* tsMatrix, schematisation* schema,
                                     double p, rtcRuntimeConfigSettings* runtimeSettings)
    : iEnsemble(iEnsemble), schema(schema), tsMatrix(tsMatrix), p(p), runtimeSettings(runtimeSettings)
{
}
...
double rtcToolsSimulator::simulate(int iStart, int iEnd, double** JInc2DArray)
{
    ...
    catch (exception& e)
    {
        piDiagInterface::addLine(
            1,
            "double rtcToolsSimulator::simulate(int iStart, int iEnd) - error during simulation - " + string(e.what()),
            RTCTOOLSSIMULATOR_CODE);
        throw;
    }
    catch (...)
    {
        piDiagInterface::addLine(
            1, "double rtcToolsSimulator::simulate(int iStart, int iEnd) - unknown error during simulation",
            RTCTOOLSSIMULATOR_CODE);
        throw;
    }
...
void rtcToolsSimulator::simulate(int iStep)
{
    if ((iStep < 1) || (iStep >= this->tsMatrix->getNTimeStep()))
        throw runtime_error(
            "void rtcToolsSimulator::simulate(int iStep) - time step is not available in time series matrix");
    ...
    catch (exception& e)
    {
        piDiagInterface::addLine(
            1,
            "void rtcToolsSimulator::simulate(int iStep) - error during rule state transfer execution - " +
                string(e.what()),
            RTCTOOLSSIMULATOR_CODE);
        throw;
    }
    ...
    {
        piDiagInterface::addLine(
            1, "void rtcToolsSimulator::simulate(int iStep) - unknown error during rule state transfer execution",
            RTCTOOLSSIMULATOR_CODE);
        throw;
    }
    ...
    {
        piDiagInterface::addLine(
            1, "void rtcToolsSimulator::simulate(int iStep) - error during trigger execution - " + string(e.what()),
            RTCTOOLSSIMULATOR_CODE);
        throw;
    }
    catch (...)
    {
        piDiagInterface::addLine(1,
                                 "void rtcToolsSimulator::simulate(int iStep) - unknown error during trigger execution",
                                 RTCTOOLSSIMULATOR_CODE);
        throw;
    }
        ...
        for (int i = 0; i < numberOfRules; i++)
        {
            if (hydraulicRules[i]->isActive())
            {
                ...
                if (activeOutputsWithRuleName.count(idOutput))
                {
                    ...
                    const auto idMap = std::find_if(
                        idMaps.cbegin(), idMaps.cend(),
                        [idOutput](const std::pair<string, int>& element) { return element.second == idOutput; });

                    throw std::runtime_error(string(currentTime) + " " + hydraulicRules[i]->getName() +
                                             " is going to enable " + idMap->first + " when it is already enabled in " +
                                             activeOutputsWithRuleName[idOutput]);
                }
    ...
    {
        piDiagInterface::addLine(
            1, "void rtcToolsSimulator::simulate(int iStep) - error during rule execution - " + string(e.what()),
            RTCTOOLSSIMULATOR_CODE);
        throw;
    }
    catch (...)
    {
        piDiagInterface::addLine(1, "void rtcToolsSimulator::simulate(int iStep) - unknown error during rule execution",
                                 RTCTOOLSSIMULATOR_CODE);
        throw;
    }
    ...
    {
        piDiagInterface::addLine(
            1, "void rtcToolsSimulator::simulate(int iStep) - error during component execution - " + string(e.what()),
            RTCTOOLSSIMULATOR_CODE);
        throw;
    }
    catch (...)
    {
        piDiagInterface::addLine(
            1, "void rtcToolsSimulator::simulate(int iStep) - unknown error during component execution",
            RTCTOOLSSIMULATOR_CODE);
        throw;
    }
