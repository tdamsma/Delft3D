...
BMI_API int initialize(const char* configfile)
{
    ...
    try
    {
        ...
        else
        {
            ...
            nSettingsSet = thisDimr->control->subBlocks[0].unit.component->dllSetKeyVals(
                thisDimr->control->subBlocks[0].unit.component->settings);
            thisDimr->timerStart(thisDimr->control->subBlocks[0].unit.component);
            thisDimr->control->subBlocks[0].unit.component->result =
                (thisDimr->control->subBlocks[0].unit.component->dllInitialize)(
                    thisDimr->control->subBlocks[0].unit.component->inputFile);
            ...
            nParamsSet = thisDimr->control->subBlocks[0].unit.component->dllSetKeyVals(
                thisDimr->control->subBlocks[0].unit.component->parameters);
...
BMI_API void get_var(const char* key, void** ref)
{
    ...
    int sourceProcess = 0;       // With multiple possible source processes, sourceProcess flags what process is actualy
                           // delivering the value Only relevant for parallel calculations. Possibly not working yet?
    ...
    if (strlen(sourceName) < 1)
    {
        throw Exception(Exception::ERR_INVALID_INPUT,
                        "dimr::get_var: No parameter specified. Expecting \"componentName/parameterName\"\n");
    }
    ...
    {
        throw Exception(Exception::ERR_INVALID_INPUT,
                        "dimr::get_var: Unrecognized component \"%s\". Expecting \"componentName/parameterName\"\n",
                        componentName);
    }
    // Get the pointer to the variable being asked for and put it in argument "ref"
    double transfer = -999000.0;

    thisDimr->getAddress(sourceName, compPtr->type, compPtr->dllGetVar, &sourceVarPtr, compPtr->processes,
                         compPtr->numProcesses, transfer);
    *ref =
        thisDimr->send(sourceName, compPtr->type, sourceVarPtr, compPtr->processes, compPtr->numProcesses, &transfer);
...
BMI_API void set_var(const char* key, const void* value)
{
    ...
    else
    {
        ...
        {
            throw Exception(Exception::ERR_INVALID_INPUT,
                            "dimr::set_var: No parameter specified. Expecting \"componentName/parameterName\"\n");
        }
        // Search componentName in the list of components of thisDimr
        for (int i = 0; i < thisDimr->componentsList.numComponents; i++)
        {
        ...
        {
            throw Exception(Exception::ERR_INVALID_INPUT,
                            "dimr::set_var: Unrecognized component \"%s\". Expecting \"componentName/parameterName\"\n",
                            componentName);
        }
        // Send value to the receiving component
        thisDimr->receive(targetName, compPtr->type, compPtr->dllSetVar, compPtr->dllGetVar, targetVarPtr,
                          compPtr->processes, compPtr->numProcesses, -1, value);
    }
    delete[] componentName;
}
