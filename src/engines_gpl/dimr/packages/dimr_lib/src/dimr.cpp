...
bool readComputeTimesFile(const char* fileName, dimr_control_block* controlBlock)
{
    ...
    if (controlblock_timearray.size() < MINIMUM_TIME_POINTS)
        throw Exception(Exception::ERR_INVALID_INPUT, "File '%s' must contain at least %d times", fileName,
                        MINIMUM_TIME_POINTS);
...
void Dimr::createDistributeMPISubGroupCommunicator(dimr_component* component)
{
    ...
    if (use_mpi && multipleProcessesCheck)
    {
        ...
        if (ierr != MPI_SUCCESS)
        {
            throw Exception(Exception::ERR_MPI,
                ...
                if (cb->subBlocks[i].subBlocks[j].type != CT_START)
                {
                    ...
                    if (thisCoupler->logger != NULL && my_rank == 0)
                    {
                        ...
                        if (nc_create(fileName.c_str(), nc_mode, &ncid))
                            throw Exception(Exception::ERR_OS, "Could not create NetCDF file at location \"%s\".",
                                            fileName.c_str());
                        ...
                        const char units[] = "seconds since 1980-01-01T00:00:00"; // TODO: Get simulation reference time
                                                                                  // from one of the kernels
                        ...
                        for (int k = 0; k < thisCoupler->numItems; k++)
                        {
                            ...
                            if (status != NC_NOERR)
                            {
                                throw Exception(Exception::ERR_OS, "Could not create dimension \"%s\".",
                                                valuestr.c_str());
                            }
                            ...
                            itemValuesLongName << sourceName << " -> " << std::string(thisCoupler->items[k].targetName);
...
void Dimr::runParallelUpdate(dimr_control_block* cb, double tStep)
{
    ...
    log->Write(DEBUG, my_rank, "Parallel Update: Current time: %15.5f -- %s -- %d", *currentTime,
    ...
    masterComponent->tNext = std::min(*currentTime + tStep, masterComponent->tEnd);
    ...
    while (*currentTime < masterComponent->tNext)
    {
            ...
            if (i != cb->masterSubBlockId)
            {
                ...
                if (tStepFollower <= 0.0)
                {
                    ...
                    if (i < cb->masterSubBlockId)
                    {
                        ...
                        tStep = std::min(tStep, cb->subBlocks[i].tStep);
                    }
                    else
                    {
                ...
                log->Write(DEBUG, my_rank, "TIME LOOP %15.5f -- %15.5f -- %15.5f -- %15.5f ", tStep,
        ...
        for (int i = 0; i < cb->numSubBlocks; i++)
        {
            ...
            if (i == cb->masterSubBlockId)
            {
                ...
                log->Write(DEBUG, my_rank, "%10.1f:    %s.Update(%15.5f)", *currentTime,
                           masterComponent->unit.component->name, tStep);
                ...
                if (*currentTime >= cb->subBlocks[i].tNext)
                {
                    ...
                    for (int j = 0; j < cb->subBlocks[i].numSubBlocks; j++)
                    {
                        if (cb->subBlocks[i].subBlocks[j].type == CT_START)
                        {
                            ...
                            log->Write(DEBUG, my_rank, "Parallel Startgroup: Current time: %15.5f -- %15.5f",
                            ...
                            // Update
                            chdir(thisComponent->workingDir);
                            log->Write(DEBUG, my_rank, "%10.1f:    %s.Update(%10.1f)", *currentTime,
                                       thisComponent->name, tUpdate);
                            timerStart(thisComponent);
                            ...
                            if (state != 0)
                            {
                                ...
                                ss << *currentTime;
                                ss_nr << state;
                                std::string message = "Could not update the component " +
                                                      std::string(thisComponent->name) + " at time " + ss.str() +
                                                      " because of errnr : " + ss_nr.str() + "\n";
                                printf(message.c_str());
                                log->Write(FATAL, my_rank, message.c_str(), configfile);
                        ...
                        else
                        {
                            ...
                            for (int k = 0; k < thisCoupler->numItems; k++)
                            {
                                ...
                                else if (thisCoupler->targetComponent->onThisRank &&
                                         thisCoupler->sourceComponent->onThisRank)
                                {
                                    receive_ptr(
                                ...
                                {
                                    log->Write(WARNING, my_rank,
                                               "Skipping pointer based exchange on rank %d: target and source "
                                               "component not on same rank!\n",
                                               my_rank);
                                }

                                if (thisCoupler->logger != NULL && my_rank == 0)
                                {
                                    std::string fileName = thisCoupler->logger->GetLoggerFilename(
                                        this->dimrWorkingDirectory, this->dirSeparator);
                                    ...
                                    int status = nc_put_var1_double(
                                        ...
                                        transferValuePtr);
                                    if (status != NC_NOERR)
                                        throw Exception(Exception::ERR_OS, "Could not write value at index (%i, 0).",
                                                        timeIndexCounter);
                                }
                    ...
                    std::vector<double>& subblock_timearray =
...
void Dimr::scanConfigFile(void)
{
    ...
    if (intRead != 1)
        throw Exception(Exception::ERR_INVALID_INPUT, "Configuration file \"%s\" does not have a version number",
                        configfile);
...
void Dimr::connectLibs(void)
{
    ...
    #define GETPROCADDRESS GetProcAddress
...
#else
            if (GetLastError() == 193)
                throw Exception(Exception::ERR_OS,
                                "Cannot load component library \"%s\". Return code: %d\n    Most probably a 32bit - "
                                "64bit conflict.",
                                lib, GetLastError());
            else
                throw Exception(Exception::ERR_OS, "Cannot load component library \"%s\". Return code: %d", lib,
                                GetLastError());
#endif
        }
        ...
        //      if (componentsList.components[i].dllGetStartTime == NULL) {
        //          throw Exception( Exception::ERR_METHOD_NOT_IMPLEMENTED, "Cannot find function \"%s\" in library
        //          \"%s\". Return code: %d", BmiGetAttributeEntryPoint, lib, GetLastError());
        //        }
