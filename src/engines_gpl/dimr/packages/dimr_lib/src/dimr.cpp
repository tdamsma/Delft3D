...
bool readComputeTimesFile(const char* fileName, dimr_control_block* controlBlock)
{
    ...
    while (computeTimesFile >> timeRead) controlBlock->computeTimes->push_back(timeRead);
    // Close the file
    ...
    if (controlblock_timearray.size() < MINIMUM_TIME_POINTS)
        throw Exception(true, Exception::ERR_INVALID_INPUT, "File '%s' must contain at least %d times", fileName,
                        MINIMUM_TIME_POINTS);
...
void Dimr::runStartBlock(dimr_control_block* cb, double tStep, int phase)
{
    ...
    if (phase == GLOBAL_PHASE_FINISH)
    {
        ...
        if (cb->unit.component->result != 0)
        {
            ...
            std::string message = "#### ERROR: dimr initialize ABORT,: " + componentName +
                                  " initialize failed, with return value " + ss.str() + " \n";
    ...
    if (state != 0)
    {
        ...
        curTime << *&cb->tCur;
        std::string componentName = cb->unit.component->name;
        std::string message = "#### ERROR: dimr update ABORT,: " + componentName +
                              " update failed, with return value " + ss.str() + " at time : " + curTime.str() + "\n";
        printf(message.c_str());
        log->Write(FATAL, my_rank, message.c_str(), configfile);
        throw Exception(true, Exception::ERR_UNKNOWN, message.c_str());
    }
    ...
    if (phase == GLOBAL_PHASE_FINISH)
    {
        ...
        if (state != 0)
        {
            ...
            ss << state;
            std::string componentName = cb->unit.component->name;
            std::string message = "#### ERROR: dimr finalize ABORT,: " + componentName +
                                  " finalize failed, with return value " + ss.str() + " \n";
            printf(message.c_str());
            log->Write(FATAL, my_rank, message.c_str(), configfile);
            throw Exception(true, Exception::ERR_UNKNOWN, message.c_str());
        }
...
void Dimr::createDistributeMPISubGroupCommunicator(dimr_component* component)
{
    ...
    if (use_mpi && multipleProcessesCheck)
    {
        ...
        {
            throw Exception(
                true, Exception::ERR_MPI,
                "createDistributeMPISubGroupCommunicator: communicator handle undefined for component \"%s\".",
                component->name);
        }
        ierr = MPI_Group_incl(mpiGroupWorld, component->numProcesses, component->processes, &mpiGroupComp);
        if (ierr != MPI_SUCCESS)
        {
            throw Exception(true, Exception::ERR_MPI,
                            "createDistributeMPISubGroupCommunicator: cannot create a subgroup of %d processes for "
                            "component \"%s\". Code: %d.",
                            component->numProcesses, component->name, ierr);
        }
        ...
        {
            throw Exception(true, Exception::ERR_MPI,
                            "createDistributeMPISubGroupCommunicator: cannot create a subcommunicator of %d processes "
                            "for component \"%s\". Code: %d.",
                            component->numProcesses, component->name, ierr);
        }
        if (component->onThisRank)
        {
            ...
            {
                throw Exception(true, Exception::ERR_MPI,
                                "createDistributeMPISubGroupCommunicator: cannot obtain reference to communicator "
                                "handle \"%s\" from component \"%s\".",
                                component->mpiCommVar, component->name);
            }
    ...
    for (int i = 0; i < cb->numSubBlocks; i++)
    {
        if (cb->subBlocks[i].type == CT_START)
        {
            if (cb->masterSubBlockId == -1)
            {
                ...
                {
                    throw Exception(true, Exception::ERR_INVALID_INPUT,
                                    "runParallelInit: the specified component in the start element was not found.");
                }
            }
            else
            {
                throw Exception(true, Exception::ERR_INVALID_INPUT,
                                "runParallelInit: a parallel block cannot have more than one start element.");
            }
        }
    }
    if (cb->masterSubBlockId == -1)
        ...
        if (i != cb->masterSubBlockId)
        {
            ...
            for (int j = 0; j < cb->subBlocks[i].numSubBlocks; j++)
            {
                if (cb->subBlocks[i].subBlocks[j].type == CT_START)
                {
                    ...
                    {  // TODO: AvD/AM: if FM is not start, but startblock, we need all the MPI stuff here as well: make
                       // a generic initializeComponent helper routine.
                ...
                if (cb->subBlocks[i].subBlocks[j].type != CT_START)
                {
                    ...
                    for (int k = 0; k < thisCoupler->numItems; k++)
                    {
                        if (IsCouplerItemTypePTR(thisCoupler->itemTypes[k]) ||
                            single_instance_component_set.find(thisCoupler->sourceComponent->type) !=
                                single_instance_component_set.end())
                        {
                        ...
                        else
                        {
                            ...
                            for (int m = 0; m < thisCoupler->sourceComponent->numProcesses; m++)
                            {
                                ...
                                {
                                    // Also for RTCTools_BMI: this is a dummy getvar call, just to check whether it
                                    // works for this partition
                                    log->Write(DEBUG, my_rank, "%s.getVar(%s)", thisCoupler->sourceComponentName,
                                               thisCoupler->items[k].sourceName);
                                    (thisCoupler->sourceComponent->dllGetVar)(
                                        thisCoupler->items[k].sourceName,
                                        (void**)(&thisCoupler->items[k].sourceVarPtr));
                            ...
                            // Do not call MPI_Allreduce when the number of partitions is 1. It will cause a crash on
                            // free(gsources)
                            if (numranks > 1)
                            {
                                int ierr = MPI_Allreduce(sources, gsources, thisCoupler->sourceComponent->numProcesses,
                            ...
                            for (int m = 0; m < thisCoupler->sourceComponent->numProcesses; m++)
                            {
                                if (gsources[m] == 1)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (my_rank == m)
                                        {
                                            log->Write(WARNING, my_rank,
                                                       "WARNING: coupler %s: item %d: \"%s\" will be delivered by "
                                                       "partition %d. Ignoring deliverance by partition %d",
                                                       thisCoupler->name, k, thisCoupler->items[k].sourceName,
                                                       thisCoupler->items[k].sourceProcess, m);
                                        }
                                    }
                                }
                        ...
                        // Target variable
                        if (IsCouplerItemTypePTR(thisCoupler->itemTypes[k]) ||
                            single_instance_component_set.find(thisCoupler->targetComponent->type) !=
                                single_instance_component_set.end())
                        {
                            // nothing
                        }
                        else
                        {
                            ...
                            for (int m = 0; m < thisCoupler->targetComponent->numProcesses; m++)
                            {
                                ...
                                {
                                    log->Write(DEBUG, my_rank, "%s.getVar(%s)", thisCoupler->targetComponentName,
                                               thisCoupler->items[k].targetName);
                                    (thisCoupler->targetComponent->dllGetVar)(
                                        thisCoupler->items[k].targetName,
                                        (void**)(&thisCoupler->items[k].targetVarPtr));
                            ...
                            if (numranks > 1)
                            {
                                int ierr = MPI_Allreduce(targets, gtargets, thisCoupler->targetComponent->numProcesses,
                                                         MPI_INT, MPI_SUM, MPI_COMM_WORLD);
                            }
                            else
                            {
                            ...
                            for (int m = 0; m < thisCoupler->targetComponent->numProcesses; m++)
                            {
                                if (gtargets[m] == 1)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            log->Write(WARNING, my_rank,
                                                       "WARNING: coupler %s: item %d: \"%s\" will be delivered to "
                                                       "multiple partitions: %d and %d.",
                            ...
                            {
                                throw Exception(true, Exception::ERR_MPI,
                                                "Coupler %s: item %d: \"%s\" is not accepted by any of the partitions.",
                                                thisCoupler->name, k, thisCoupler->items[k].targetName);
                            }
                            free(targets);
                            free(gtargets);
                    ...
                    if (thisCoupler->logger != NULL && my_rank == 0)
                    {
                        ...
                        if (nc_create(fileName.c_str(), nc_mode, &ncid))
                            throw Exception(true, Exception::ERR_OS, "Could not create NetCDF file at location \"%s\".",
                                            fileName.c_str());
                        ...
                        nc_def_var(ncid, "time_offset", NC_DOUBLE, 1, &thisCoupler->logger->netcdfReferences->timeDim,
                                   &thisCoupler->logger->netcdfReferences->timeVar);
                        const char longnametime[] = "seconds since simulation reference date, T00:00:00";
                        nc_put_att_text(ncid, thisCoupler->logger->netcdfReferences->timeVar, "long_name",
                                        sizeof(longnametime), longnametime);
                        const char units[] = "seconds since 1980-01-01T00:00:00";  // TODO: Get simulation reference
                                                                                   // time from one of the kernels
                        nc_put_att_text(ncid, thisCoupler->logger->netcdfReferences->timeVar, "units", sizeof(units),
                                        units);
                        const char axis[] = "T";
                        nc_put_att_text(ncid, thisCoupler->logger->netcdfReferences->timeVar, "axis", sizeof(axis),
                                        axis);
                        ...
                        for (int k = 0; k < thisCoupler->numItems; k++)
                        {
                            ...
                            int status = nc_def_dim(ncid, valuestr.c_str(), 1,
                                                    &thisCoupler->logger->netcdfReferences->item_values[k]);
                            if (status != NC_NOERR)
                            {
                                throw Exception(true, Exception::ERR_OS, "Could not create dimension \"%s\".",
                                                valuestr.c_str());
                            }

                            int dimensions[2] = {thisCoupler->logger->netcdfReferences->timeDim,
                                                 thisCoupler->logger->netcdfReferences->item_values[k]};
                            int dummyVar;
                            std::ostringstream varName;
                            varName << "item" << k + 1 << "_values";
                            const string varnamestr(varName.str());
                            nc_def_var(ncid, varnamestr.c_str(), NC_DOUBLE, 2, dimensions,
                                       &thisCoupler->logger->netcdfReferences->item_variables[k]);

                            std::ostringstream itemValuesLongName;
                            const string sourceName = string(thisCoupler->items[k].sourceName);
                            itemValuesLongName << sourceName << " -> " << string(thisCoupler->items[k].targetName);
                            const string itemvaluesstr(itemValuesLongName.str());
                            nc_put_att_text(ncid, thisCoupler->logger->netcdfReferences->item_variables[k], "long_name",
                                            itemvaluesstr.size(), itemvaluesstr.c_str());
                            ...
                            nc_put_att_text(ncid, thisCoupler->logger->netcdfReferences->item_variables[k],
                                            "coordinates", itemValuesCoordinatesstr.size(),
                                            itemValuesCoordinatesstr.c_str());
                        }
...
void Dimr::runParallelUpdate(dimr_control_block* cb, double tStep)
{
    ...
    log->Write(INFO, my_rank, "Parallel Update: Current time: %15.5f -- %s -- %d", *currentTime,
               masterComponent->unit.component->name, tStep);
    ...
    if (*currentTime == masterComponent->tStart)
    {
        ...
        for (int i = 0; i < cb->numSubBlocks; i++)
        {
            if (i != cb->masterSubBlockId)
            {
                ...
                for (int j = 0; j < cb->subBlocks[i].numSubBlocks; j++)
                {  // look for the WAVE component (wave-library)
                    if (j != cb->masterSubBlockId && cb->subBlocks[i].subBlocks[j].unit.component != nullptr &&
                        cb->subBlocks[i].subBlocks[j].unit.component->type == COMP_TYPE_WAVE)
                    {
    ...
    while (*currentTime < masterComponent->tNext)
    {
            ...
            if (i != cb->masterSubBlockId)
            {
                ...
                if (tStepFollower <= 0.0)
                {
                    ...
                    else
                    {
                        ...
                        throw Exception(true, Exception::ERR_INVALID_INPUT,
                                        "runParallelUpdate: Zero timestep, needed for block %d, is not possible.", i);
                    }
                }
                else
                {
                ...
                log->Write(INFO, my_rank, "TIME LOOP %15.5f -- %15.5f -- %15.5f -- %15.5f ", tStep,
                           masterComponent->tNext, cb->subBlocks[i].tNext, tStepFollower);
            }
        }
        ...
        for (int i = 0; i < cb->numSubBlocks; i++)
        {
            ...
            if (i == cb->masterSubBlockId)
            {
                ...
                log->Write(INFO, my_rank, "%10.1f:    %s.Update(%15.5f)", *currentTime,
                           masterComponent->unit.component->name, tStep);
                ...
                {
                    ierr =
                        MPI_Bcast(&state0, 4, MPI_INT, masterComponent->unit.component->processes[0], MPI_COMM_WORLD);
                }
                if (state0 != 0 || state != 0)
                {
                    stringstream ss, curTime;
                    ...
                    std::string componentName = cb->unit.component->name;
                    std::string message = "#### ERROR: dimr update ABORT,: " + componentName +
                                          " update failed, with return value " + ss.str() +
                                          " at time : " + curTime.str() + "\n";
                    printf(message.c_str());
                    log->Write(FATAL, my_rank, message.c_str(), configfile);
                ...
                if (*currentTime >= cb->subBlocks[i].tNext)
                {
                    ...
                    for (int j = 0; j < cb->subBlocks[i].numSubBlocks; j++)
                    {
                        if (cb->subBlocks[i].subBlocks[j].type == CT_START)
                        {
                            ...
                            log->Write(INFO, my_rank, "Parallel Startgroup: Current time: %15.5f -- %15.5f",
                                       *currentTime, subblock_time);
                            ...
                            //              Otherwise, WANDA will hang in case tUpdate is very big (FLOW start time big,
                            //              related to ref time) It doesn't matter for the WANDA results; with this
                            //              coupling, WANDA doesn't have a time knowledge at all
                            ...
                            // Update
                            chdir(thisComponent->workingDir);
                            log->Write(INFO, my_rank, "%10.1f:    %s.Update(%10.1f)", *currentTime, thisComponent->name,
                                       tUpdate);
                            timerStart(thisComponent);
                            ...
                            if (state != 0)
                            {
                                ...
                                ss << *currentTime;
                                ss_nr << state;
                                string message = "Could not update the component " + std::string(thisComponent->name) +
                                                 " at time " + ss.str() + " because of errnr : " + ss_nr.str() + "\n";
                                printf(message.c_str());
                                log->Write(FATAL, my_rank, message.c_str(), configfile);
                        ...
                        else
                        {
                            ...
                            // log->Write(DEBUG, my_rank, "%10.1f:    %s.communicate (%d)", *currentTime,
                            // thisCoupler->name, timeIndexCounter);

                            // log netcdf time variable
                            int timeIndexCounter = static_cast<int>(
                                floor((*currentTime - cb->subBlocks[cb->masterSubBlockId].tStart) / tStep));
                            log->Write(DEBUG, my_rank, "%10.1f:    %s.communicate (%d -- %15.5f, %15.5f)", *currentTime,
                                       thisCoupler->name, timeIndexCounter, cb->subBlocks[cb->masterSubBlockId].tStart,
                                       tStep);
                            if (thisCoupler->logger != NULL)
                            {
                                string fileName =
                                    thisCoupler->logger->GetLoggerFilename(dimrWorkingDirectory, dirSeparator);

                                int ncid = ncfiles[fileName];
                                size_t index[] = {static_cast<size_t>(timeIndexCounter)};
                                nc_put_var1_double(ncid, thisCoupler->logger->netcdfReferences->timeVar, index,
                                                   currentTime);
                            }

                            for (int k = 0; k < thisCoupler->numItems; k++)
                            {
                                log->Write(ALL, my_rank, "    %s -> %s (%d)", thisCoupler->items[k].sourceName,
                                           thisCoupler->items[k].targetName, timeIndexCounter);
                                ...
                                {
                                    getAddress(thisCoupler->items[k].sourceName, thisCoupler->sourceComponent->type,
                                               thisCoupler->sourceComponent->dllGetVar,
                                               &(thisCoupler->items[k].sourceVarPtr),
                                               thisCoupler->sourceComponent->processes,
                                               thisCoupler->sourceComponent->numProcesses, transfer);
                                }
                                ...
                                transferValuePtr =
                                    send(thisCoupler->items[k].sourceName, thisCoupler->sourceComponent->type,
                                         thisCoupler->items[k].sourceVarPtr, thisCoupler->sourceComponent->processes,
                                         thisCoupler->sourceComponent->numProcesses, &transfer);

                                // Optional TODO: assuming one source(partition):
                                //    if there is only one target (partition), and all partitions can act as source
                                //    partition:
                                ...
                                {
                                    receive(thisCoupler->items[k].targetName, thisCoupler->targetComponent->type,
                                            thisCoupler->targetComponent->dllSetVar,
                                            thisCoupler->targetComponent->dllGetVar, thisCoupler->items[k].targetVarPtr,
                                            thisCoupler->targetComponent->processes,
                                            thisCoupler->targetComponent->numProcesses,
                                            thisCoupler->items[k].targetProcess, transferValuePtr);
                                }
                                else
                                {
                                    receive_ptr(
                                        thisCoupler->items[k].targetName, thisCoupler->items[k].sourceName,
                                        thisCoupler->targetComponent->type, thisCoupler->targetComponent->dllSetVar,
                                        thisCoupler->targetComponent->dllGetVar,
                                        thisCoupler->sourceComponent->dllGetVarShape,
                                        thisCoupler->items[k].targetVarPtr, thisCoupler->targetComponent->processes,
                                        thisCoupler->targetComponent->numProcesses, thisCoupler->items[k].targetProcess,
                                        thisCoupler->items[k].sourceVarPtr);
                                }

                                if (thisCoupler->logger != NULL && my_rank == 0)
                                {
                                    string fileName = thisCoupler->logger->GetLoggerFilename(this->dimrWorkingDirectory,
                                                                                             this->dirSeparator);

                                    int ncid = ncfiles[fileName];
                                    size_t indices[] = {static_cast<size_t>(timeIndexCounter), 0};
                                    int status = nc_put_var1_double(
                                        ncid, thisCoupler->logger->netcdfReferences->item_variables[k], indices,
                                        transferValuePtr);
                                    if (status != NC_NOERR)
                                        throw Exception(true, Exception::ERR_OS,
                                                        "Could not write value at index (%i, 0).", timeIndexCounter);
                                }
                    ...
                    // All child blocks are executed. Update tCur and define the next time step that this block has to
                    // be executed.
                    for (int j = 0; j < cb->subBlocks[i].numSubBlocks; j++)
                    {  // look for the WAVE component (wave-library)
                        if (j != cb->masterSubBlockId && cb->subBlocks[i].subBlocks[j].unit.component != nullptr &&
                    ...
                    vector<double>& subblock_timearray =
                        *(cb->subBlocks[i].computeTimes);  // Shortcut to keep the code readable
                    ...
                    else if (cb->subBlocks[i].computeTimesCurrent > 0)
                    {
                        ...
                        int subblock_timeindex =
                            cb->subBlocks[i].computeTimesCurrent;  // Shortcut to keep the code readable
...
void Dimr::receive(const char* name, int compType, BMI_SETVAR dllSetVar, BMI_GETVAR dllGetVar, double* targetVarPtr,
        ...
        for (int m = 0; m < nProc; m++)
        {
            if (my_rank == processes[m])
            {
                ...
                if (compType == COMP_TYPE_RTC
                    // || compType == COMP_TYPE_RR // SOBEK-51004: RR must use explicit set_var as long as get_var is
                    // not properly implemented for some variables.
                    || compType == COMP_TYPE_FLOW1D || compType == COMP_TYPE_FLOW1D2D || compType == COMP_TYPE_DSLE ||
                    compType == COMP_TYPE_WANDA)
                {
                    if (dllSetVar == NULL)
                    {
                        throw Exception(true, Exception::ERR_METHOD_NOT_IMPLEMENTED,
                                        "ABORT: Dimr::receive: set_var function not defined while processing %s", name);
                    }
                ...
                else
                {
                    ...
                    // NOTE: transferValuePtr contains the value calculated by RTCTools: it has been already
                    // communicated to all processes by a previous send call.
...
//------------------------------------------------------------------------------
// set "value" in the component target location
void Dimr::receive_ptr(const char* name, const char* sourceName, int compType, BMI_SETVAR dllSetVar,
                       BMI_GETVAR dllGetVar, BMI_GETVARSHAPE dllGetVarShape, double* targetVarPtr, int* processes,
                       int nProc, int targetProcess, double* sourceVarPtr)
{
...
void Dimr::getAddress(const char* name, int compType, BMI_GETVAR dllGetVar, double** sourceVarPtr, int* processes,
                      int nProc, double& transfer)
{
    ...
    std::set<int> second_component_set = {COMP_TYPE_DEFAULT_BMI, COMP_TYPE_RTC, COMP_TYPE_DSLE,  COMP_TYPE_FLOW1D,
                                          COMP_TYPE_FLOW1D2D,    COMP_TYPE_FM,  COMP_TYPE_DELWAQ};
...
double* Dimr::send(const char* name, int compType, double* sourceVarPtr, int* processes, int nProc, double* transfer)
{
    ...
    if (numranks > 1)
    {
        ...
        // We perform a reduction operation to collect the maximum values at each position and afterwards take the
        // maximum of all values. We could also use only one double instead of a transfer array and perform the
        // reduction on a single scalar (so avoiding the loop below)
    ...
    for (int i = 0; i < cb->numSubBlocks; i++)
    {
        if (cb->subBlocks[i].type == CT_START)
        {
            ...
            if (use_mpi && cb->subBlocks[i].unit.component->mpiComm != NULL)
            {
                ...
                // Is this really needed? In dflowfm the communicator is already freed in petsc finalize is solver 6 is
                // used and this line causes a runtime error at the end of a dimr run
...
void Dimr::scanConfigFile(void)
{
    ...
    if (rootXml == NULL)
        throw Exception(true, Exception::ERR_INVALID_INPUT,
                        "Configuration file \"%s\" does not have a <dimrConfig> root element", configfile);
    XmlTree* fileversion = rootXml->Lookup("documentation/fileVersion");
    if (fileversion == NULL)
        throw Exception(true, Exception::ERR_INVALID_INPUT,
                        "Configuration file \"%s\" does not have a deltaresHydro documentation->fileVersion element",
                        configfile);
    ...
    if (intRead != 1)
        throw Exception(true, Exception::ERR_INVALID_INPUT, "Configuration file \"%s\" does not have a version number",
                        configfile);
    if ((int)floor(versionnumber) != 1)
        throw Exception(true, Exception::ERR_INVALID_INPUT,
                        "Configuration file \"%s\": Version number (%3.2f) must have main version 2", configfile,
                        versionnumber);

    XmlTree* controlXml = rootXml->Lookup("control");
    if (controlXml == NULL)
        throw Exception(true, Exception::ERR_INVALID_INPUT,
                        "Configuration file \"%s\" does not have a deltaresHydro control element", configfile);
...
void Dimr::scanUnits(XmlTree* rootXml)
{
    ...
    for (int i = 0; i < rootXml->children.size(); i++)
    {
        if (strcmp(rootXml->children[i]->name, "component") == 0)
        {
            ...
            if (componentsList.components == NULL)
            {
                componentsList.components =
                    (dimr_component*)malloc(componentsList.numComponents * sizeof(dimr_component));
            }
            else
            {
                componentsList.components = (dimr_component*)realloc(
                    componentsList.components, componentsList.numComponents * sizeof(dimr_component));
                if (componentsList.components == NULL)
                {
        ...
        if (strcmp(rootXml->children[i]->name, "coupler") == 0)
        {
            ...
            {
                couplersList.couplers =
                    (dimr_coupler*)realloc(couplersList.couplers, couplersList.numCouplers * sizeof(dimr_coupler));
                if (couplersList.couplers == NULL)
                {
...
void Dimr::scanComponent(XmlTree* xmlComponent, dimr_component* newComp)
{
    ...
    if (libraryElement == NULL)
        throw Exception(true, Exception::ERR_INVALID_INPUT, "Component \"%s\" does not contain a library element",
                        newComp->name);
    ...
    else
    {
        ...
        log->Write(ALL, my_rank, "INFO: \"<process>\" Type for component \"%s\" is set to default value 0",
                   newComp->name);
    }
    ...
    if (processElement != NULL)
    {
        ...
        for (int i = 0; i < newComp->numProcesses; i++)
        {
            ...
            {
                throw Exception(true, Exception::ERR_INVALID_INPUT,
                                "Component \"%s\" configured for process #%d, but max running MPI rank is only %d.",
    ...
    else
    {
        ...
        log->Write(ALL, my_rank,
                   "INFO: \"<process>\" not specified for component \"%s\". Assuming it only runs on rank #0.",
                   newComp->name);
    ...
    if (chdir(combinedPath))
    {
        ...
        {
            throw Exception(true, Exception::ERR_INVALID_INPUT, "Component \"%s\" has an invalid workingDir \"%s\"",
...
void Dimr::scanCoupler(XmlTree* xmlCoupler, dimr_coupler* newCoup)
{
    ...
    if (sourceComponent == NULL)
        throw Exception(true, Exception::ERR_INVALID_INPUT,
                        "The coupler \"%s\" does not contain a sourceComponent element", newCoup->name);
    ...
    if (targetComponent == NULL)
        throw Exception(true, Exception::ERR_INVALID_INPUT,
                        "The coupler \"%s\" does not contain a targetComponent element", newCoup->name);
    ...
    for (int j = 0; j < xmlCoupler->children.size(); j++)
    {
        if (strcmp(xmlCoupler->children[j]->name, "item") == 0)
        {
            ...
            {
                newCoup->items =
                    (dimr_couple_item*)realloc(newCoup->items, newCoup->numItems * sizeof(dimr_couple_item));
                newCoup->itemTypes =
                    (unsigned int*)realloc(newCoup->itemTypes, newCoup->numItems * sizeof(unsigned int));
            ...
            if (xmlCoupler->children[j]->GetAttrib("type") == NULL ||
                strcmp(xmlCoupler->children[j]->GetAttrib("type"), "pointer") != 0)
            {
    ...
    if (logger != NULL)
    {
        ...
        if (newCoup->logger->workingDir == NULL)
            throw Exception(true, Exception::ERR_INVALID_INPUT,
                            "The coupler \"%s\"'s logger element does not contain a workingDir element.",
                            newCoup->name);
        ...
        if (newCoup->logger->outputFile == NULL)
            throw Exception(true, Exception::ERR_INVALID_INPUT,
                            "The coupler \"%s\"'s logger element does not contain an outputFile element.",
                            newCoup->name);
    }
}
...
void Dimr::scanControl(XmlTree* controlBlockXml, dimr_control_block* controlBlock)
{
    ...
    for (int i = 0; i < controlBlockXml->children.size(); i++)
    {
            ...
            strcmp(controlBlockXml->children[i]->name, "coupler") == 0)
        {
            ...
            {
                controlBlock->subBlocks =
                    (dimr_control_block*)malloc(controlBlock->numSubBlocks * sizeof(dimr_control_block));
            }
            else
            {
                controlBlock->subBlocks = (dimr_control_block*)realloc(
                    controlBlock->subBlocks, controlBlock->numSubBlocks * sizeof(dimr_control_block));
                if (controlBlock->subBlocks == NULL)
                {
...
//------------------------------------------------------------------------------
// Search for a named coupler in the list of couplers
bool Dimr::IsCouplerItemTypePTR(int couplerItem) { return couplerItem == ITEM_TYPE_PTR; }
...
void Dimr::connectLibs(void)
{
    ...
    for (int i = 0; i < componentsList.numComponents; i++)
    {
        ...
        {
            log->Write(ALL, my_rank, "Not necessary to load component library \"%s\" on this rank.",
                       componentsList.components[i].library);
            continue;
        }
...
#define GETPROCADDRESS GetProcAddress
        ...
        if (dllhandle == NULL)
        {
            ...
            if (GetLastError() == 193)
                throw Exception(true, Exception::ERR_OS,
                                "Cannot load component library \"%s\". Return code: %d\n    Most probably a 32bit - "
                                "64bit conflict.",
                                lib, GetLastError());
            else
                throw Exception(true, Exception::ERR_OS, "Cannot load component library \"%s\". Return code: %d", lib,
                                GetLastError());
#endif
        }
        ...
        {
            throw Exception(true, Exception::ERR_METHOD_NOT_IMPLEMENTED,
                            "Cannot find function \"%s\" in library \"%s\". Return code: %d", BmiInitializeEntryPoint,
                            lib, GetLastError());
        }
        ...
        {
            throw Exception(true, Exception::ERR_METHOD_NOT_IMPLEMENTED,
                            "Cannot find function \"%s\" in library \"%s\". Return code: %d", BmiUpdateEntryPoint, lib,
                            GetLastError());
        }
        ...
        {
            throw Exception(true, Exception::ERR_METHOD_NOT_IMPLEMENTED,
                            "Cannot find function \"%s\" in library \"%s\". Return code: %d", BmiFinalizeEntryPoint,
                            lib, GetLastError());
        }

        componentsList.components[i].dllGetStartTime =
            (BMI_GETSTARTTIME)GETPROCADDRESS(dllhandle, BmiGetStartTimeEntryPoint);
        if (componentsList.components[i].dllGetStartTime == NULL)
        {
            throw Exception(true, Exception::ERR_METHOD_NOT_IMPLEMENTED,
                            "Cannot find function \"%s\" in library \"%s\". Return code: %d", BmiGetStartTimeEntryPoint,
                            lib, GetLastError());
        }
        ...
        {
            throw Exception(true, Exception::ERR_METHOD_NOT_IMPLEMENTED,
                            "Cannot find function \"%s\" in library \"%s\". Return code: %d", BmiGetEndTimeEntryPoint,
                            lib, GetLastError());
        }

        componentsList.components[i].dllGetTimeStep =
            (BMI_GETTIMESTEP)GETPROCADDRESS(dllhandle, BmiGetTimeStepEntryPoint);
        ...
        componentsList.components[i].dllGetCurrentTime =
            (BMI_GETCURRENTTIME)GETPROCADDRESS(dllhandle, BmiGetCurrentTimeEntryPoint);
        if (componentsList.components[i].dllGetCurrentTime == NULL)
        {
            throw Exception(true, Exception::ERR_METHOD_NOT_IMPLEMENTED,
                            "Cannot find function \"%s\" in library \"%s\". Return code: %d",
                            BmiGetCurrentTimeEntryPoint, lib, GetLastError());
        }

        componentsList.components[i].dllGetAttribute =
            (BMI_GETATTRIBUTE)GETPROCADDRESS(dllhandle, BmiGetAttributeEntryPoint);
        ...
        //      if (componentsList.components[i].dllGetStartTime == NULL) {
        //          throw Exception (true,  Exception::ERR_METHOD_NOT_IMPLEMENTED, "Cannot find function \"%s\" in
        //          library \"%s\". Return code: %d", BmiGetAttributeEntryPoint, lib, GetLastError());
        //        }

        if (componentsList.components[i].type == COMP_TYPE_DEFAULT_BMI ||
            componentsList.components[i].type == COMP_TYPE_FM || componentsList.components[i].type == COMP_TYPE_RTC ||
            ...
            componentsList.components[i].type == COMP_TYPE_TEST || componentsList.components[i].type == COMP_TYPE_WANDA)
        {
            ...
            {
                throw Exception(true, Exception::ERR_METHOD_NOT_IMPLEMENTED,
                                "Cannot find function \"%s\" in library \"%s\". Return code: %d", BmiSetVarEntryPoint,
                                lib, GetLastError());
            }
        }
        else
        ...
        if (componentsList.components[i].type == COMP_TYPE_DEFAULT_BMI ||
            componentsList.components[i].type == COMP_TYPE_FM || componentsList.components[i].type == COMP_TYPE_DSLE ||
            componentsList.components[i].type == COMP_TYPE_COSUMO_BMI)
        {
            componentsList.components[i].dllGetVarShape =
                (BMI_GETVARSHAPE)GETPROCADDRESS(dllhandle, BmiGetVarShapeEntryPoint);
            if (componentsList.components[i].dllGetVarShape == NULL)
            {
                throw Exception(true, Exception::ERR_METHOD_NOT_IMPLEMENTED,
                                "Cannot find function \"%s\" in library \"%s\". Return code: %d",
                                BmiGetVarShapeEntryPoint, lib, GetLastError());
            }
        }
        else
        {
        ...
        if (componentsList.components[i].type == COMP_TYPE_FLOW1D)
        {
            ...
            {
                throw Exception(true, Exception::ERR_METHOD_NOT_IMPLEMENTED,
                                "Cannot find function \"%s\" in library \"%s\". Return code: %d", BmiSetLogger, lib,
                                GetLastError());
            }
            ...
            componentsList.components[i].type == COMP_TYPE_COSUMO_BMI)
        {
            ...
            componentsList.components[i].setLogger((BMILogger)&_log);
            // Is it possible to set the debugLevel in FM? componentsList.components[i].dllSetVar("debugLevel", (const
            // void *)&level);
        }
        ...
        {
            throw Exception(true, Exception::ERR_METHOD_NOT_IMPLEMENTED,
                            "Cannot find function \"%s\" in library \"%s\". Return code: %d", BmiGetVarEntryPoint, lib,
                            GetLastError());
        }
...
void Dimr::printComponentVersionStrings(Level my_level)
{
    ...
    for (int i = 0; i < componentsList.numComponents; i++)
    {
        ...
        {
            ierr = MPI_Bcast(versionstr, MAXSTRINGLEN, MPI_CHAR, componentsList.components[i].processes[0],
                             MPI_COMM_WORLD);
        }
...
void Dimr::freeLibs(void)
{
    ...
    for (int i = 0; i < componentsList.numComponents; i++)
    {
        ...
        {
            throw Exception(true, Exception::ERR_OS, "Cannot free component library \"%s\". Return code: %d.",
...
//------------------------------------------------------------------------------
void Dimr::timerStart(dimr_component* thisComponent) { thisComponent->timerStart = clock->Epoch(); }
...
void Dimr::timersFinish(void)
{
    ...
    for (int i = 0; i < componentsList.numComponents; i++)
    {
        ...
        log->Write(INFO, my_rank, "%s\t: %d.%d sec", componentsList.components[i].name,
                   componentsList.components[i].timerSum / 1000000, componentsList.components[i].timerSum % 1000000);
...
void Dimr::timerFinish(void)
{
    ...
    log->Write(INFO, my_rank, "%s\t: %d.%d sec", "DIMR_LIB", this->timerSumStamp / 1000000,
...
void Dimr::char_to_ints(const char* line, int** iarr, int* count)
{
    ...
    int p;
    while (1)
    {  // NOTE: there's no checking on a valid first number yet.
        stream >> p;
        if (!stream)
        {
            break;
