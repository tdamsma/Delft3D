...
TEST_CLASS(DimrGlobalTests){
    ...
    public :
        ...
        TEST_METHOD(initializeWithoutFileNameTest){Assert::AreEqual(initialize(""), (int)Exception::ERR_OS);
}

TEST_METHOD(initializeWithFileNameTest) { Assert::AreEqual(initialize("dimr.xml"), (int)Exception::ERR_INVALID_INPUT); }
...
TEST_CLASS(DimrTests){
    ...
    public :
        ...
        TEST_METHOD(ConstructDimr){Dimr* dimr = Dimr::GetInstance();
// thisDimr = dimr;
...
void           getAddress(const char * name, int compType, BMI_GETVAR dllGetVar, double ** sourceVarPtr, int *
processes, int nProc, double * transfer); double *       send(const char * name, int compType, double* sourceVarPtr,
int* processes, int nProc, double* transfer);
*/
...
public:
    ...
    TEST_METHOD(InitDll)
    {
        ...
        Assert::AreEqual(false, dimr->done);
        Assert::AreEqual("dimr_redirected.log", dimr->redirectFile);
        //Can not delete, destructor refers to the static global dimr instance, which is only assigned in dimr
initialize
        //auto func2 = [dimr] { delete dimr; };
        //Assert::ExpectException<std::exception>(func2);
    }
