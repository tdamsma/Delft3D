...
#if defined(MEMCHECK)
static void memAbort(enum mcheck_status status)
{
...
void DimrExe::lib_initialize(void)
{
    ...
    if (result != 0)
    {
        ...
        // Throw one now
        this->log->Write(FATAL, my_rank, "%s.Initialize(%s) returned error value %d", this->library, this->configfile,
                         result);
        // Re-throw the exception (so it will be handled in main)
        throw Exception((Exception::ErrorCode)result, "%s.Initialize(%s) returned error value %d", this->library,
...
void DimrExe::lib_update_test(void)
{
    ...
    do
    {
        ...
        // Inside dllSetVar, Dimr::send might be called to obtain the pointer to the variable inside the kernel that has
        // to be set This will reset the value that "transfer" points to into the current value.
        value = *transfer;
...
void DimrExe::timerFinish(void)
{
    ...
    log->Write(INFO, my_rank, "%s\t: %d.%d sec", "DIMR_EXE", this->timerSumStamp / 1000000,
...
DimrExe::DimrExe(void) { this->ready = false; }
...
void DimrExe::initialize(int argc, char* argv[], char* envp[])
{
    ...
    while ((c = getopt(argc, argv, (char*)"d:l:S:v:i:?")) != -1)
    {
        switch (c)
        {
            ...
            case 'l': {
                ...
                if (logFile == NULL) throw Exception(Exception::ERR_OS, "Cannot create log file \"%s\"", optarg);
            ...
            default: {
                throw Exception(Exception::ERR_INVALID_INPUT,
                                "Invalid command-line argument.  Execute \"%s -?\" for command-line syntax",
                                this->exeName);
            }
        }
    }
    ...
    if (argc - optind != 1)
    {
        throw Exception(Exception::ERR_INVALID_INPUT, "Improper usage.  Execute \"%s -?\" for command-line syntax",
                        this->exeName);
    }
...
void DimrExe::openLibrary(void)
{
    ...
    if (dllhandle == NULL)
    {
        ...
        if (GetLastError() == 193)
            throw Exception(
                Exception::ERR_OS,
                "Cannot load component library \"%s\". Return code: %d\n    Most probably a 32bit - 64bit conflict.",
                this->library, GetLastError());
        else
            throw Exception(Exception::ERR_OS, "Cannot load component library \"%s\". Return code: %d", this->library,
                            GetLastError());
#endif
    }
    ...
    if (set_logger_entry == NULL)
    {
        throw Exception(Exception::ERR_METHOD_NOT_IMPLEMENTED,
                        "Cannot find function \"%s\" in library \"%s\". Return code: %d", BmiDimrSetLogger,
                        this->library, GetLastError());
    }
    ...
    {
        throw Exception(Exception::ERR_METHOD_NOT_IMPLEMENTED,
                        "Cannot find function \"%s\" in library \"%s\". Return code: %d", BmiUpdateEntryPoint,
                        this->library, GetLastError());
    }
    ...
    {
        throw Exception(Exception::ERR_METHOD_NOT_IMPLEMENTED,
                        "Cannot find function \"%s\" in library \"%s\". Return code: %d", BmiGetCurrentTimeEntryPoint,
                        this->library, GetLastError());
    }
    ...
    {
        throw Exception(Exception::ERR_METHOD_NOT_IMPLEMENTED,
                        "Cannot find function \"%s\" in library \"%s\". Return code: %d", BmiGetVarEntryPoint,
                        this->library, GetLastError());
    }
}
...
void DimrExe::freeLib(void)
{
    ...
    {
        throw Exception(Exception::ERR_OS, "Cannot free component library \"%s\". Return code: %d.", this->library,
                        ierr);
    }
#endif
}
