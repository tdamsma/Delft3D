...
!!  rights reserved.
module m_integration_scheme_26
    use m_waq_precision
    use timers
    use delwaq2_data
    ...
    use m_process_calculation, only: calculate_processes, set_explicit_time_step_for_derivatives
...
contains
    ...
    subroutine scheme_26_adaptive_time_step_fractional_step(buffer, file_unit_list, file_name_list, action, dlwqd, gridps)
        ...
        use m_locally_adaptive_time_step, only: locally_adaptive_time_step
        use m_thatcher_harleman_bc, only: thatcher_harleman_bc
        use m_add_waste_loads, only: add_waste_loads_special
        ...
        associate (a => buffer%rbuf, j => buffer%ibuf, c => buffer%chbuf)
            ...
            ! Integrate the effect of the processes:
            ...
            ! add the waste loads
            call add_waste_loads_special (num_substances_transported, num_substances_total, num_cells, num_exchanges, num_waste_loads, &
            ...
            call locally_adaptive_time_step(file_unit_list(19), num_substances_transported, num_substances_total, num_substances_part, num_cells, &
