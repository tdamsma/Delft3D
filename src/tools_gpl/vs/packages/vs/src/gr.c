 ...
 *        added or updated.
 */
VsGrpData GR_add_group_to_chain(VsGrpData p,        /* pointer to a struct in chain */
                                const BText grpnam, /* name of group to add */
                                const BText grpdef, /* group def. name of this group */
                                const BInt4 grpdms[], VsAtrData att)

{
    ...
    if (p == NULL)
    {
        ...
        if (p != NULL)
        {
            ...
            for (p->nintat = 0; p->nintat < att->nintat; p->nintat++)
            {
                (BVoid) strcpy(p->IntAttrName[p->nintat], att->IntAttrName[p->nintat]);
                p->IntAttrValue[p->nintat] = att->IntAttrValue[p->nintat];
            }

            for (p->nreaat = 0; p->nreaat < att->nreaat; p->nreaat++)
            {
                (BVoid) strcpy(p->RealAttrName[p->nreaat], att->RealAttrName[p->nreaat]);
                p->RealAttrValue[p->nreaat] = att->RealAttrValue[p->nreaat];
            }

            for (p->nstrat = 0; p->nstrat < att->nstrat; p->nstrat++)
            {
                (BVoid) strcpy(p->StringAttrName[p->nstrat], att->StringAttrName[p->nstrat]);
                (BVoid) strcpy(p->StringAttrValue[p->nstrat], att->StringAttrValue[p->nstrat]);
            }
    ...
    else if (GEN_string_compare(grpnam, p->grpnam) != 0)
    {
        ...
        p->next = GR_add_group_to_chain(p->next, grpnam, grpdef, grpdms, att);
    }
    else
    {
        /* group found */
 ...
 */
VsGrpData GR_find_group_in_chain(VsGrpData p,       /* pntr to a structure in chain */
 ...
 */
BVoid GR_remove_groups_from_chain(VsGrpData p /* pointer to group-struct in chain */
)
{
    if (p != NULL)
    {
