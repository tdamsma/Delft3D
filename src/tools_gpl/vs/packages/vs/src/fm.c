 ...
 */
BInt4 FM_get_datafile_descriptor(BVoid) { return neffds; }

/*  @@  Convenience function for others to get pointer
 *      to definition file descriptor (see Nefis Manual)
 */
BInt4 FM_get_deftfile_descriptor(BVoid) { return neffds; }
...
/*  @@  Open Nefis files.
 */
BInt4 FM_open_nefis_files(const BText datfile, const BText deffile)
{
    BInt4 error = -1;
    ...
    if (FM_can_be_opened(datfile) && FM_can_be_opened(deffile))
    {
