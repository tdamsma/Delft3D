 ...
 */
BData VR__alloc_memory_for_data(BInt4 nbytes) { return GEN_malloc(nbytes); }

/*  @@ Allocate memory for a variable structure.
 */
VsVarData VR__alloc_memory_for_variable(BVoid) { return (VsVarData)GEN_malloc(sizeof(VsVar)); }

/*
 */
BInt4 VR_same_structure(VsVarData pa, VsVarData pb)
{
    if (pa->grpndm == pb->grpndm && pa->elmndm == pb->elmndm)
    {
        BInt4 i;
 ...
 *      may be updated.
 */
static VsVarData VR_free_variable(VsVarData p, const BText varnam)
{
    VsVarData retval;
...
/*  @@
 */
static VsVarData VR_add_var_to_chain(VsVarData p_var, VsVarData p)
{
    if (p_var == NULL)
    {
        p_var = p;
...
BVoid VR_add_variable_to_memo(VsVarData p) { varinf = VR_add_var_to_chain(varinf, p); }

/*  @@  Find memo-variable in chain, return pointer.
 */
static VsVarData VR_find_var_in_chain(VsVarData p, const BText varnam)
{
    if (p != NULL)
    {
        if (GEN_string_compare(varnam, p->varnam) == 0)
...
/*  @@
 */
static VsVarData VR_make_variable_struct(const BText varnam, const BText grpnam, const BText elmnam,
                                         BInt4 elm_uindex[][3], BInt4 uindex[][3], BData varpnt, VsDefData p_def,
                                         VsElmData p_elm)

{
    ...
    {
        dim = (uindex[i][1] - uindex[i][0] + uindex[i][2]) / uindex[i][2];
    ...
    {
        dim = (elm_uindex[i][1] - elm_uindex[i][0] + elm_uindex[i][2]) / elm_uindex[i][2];
 ...
 */
static BVoid VR_store_data_in_buffer(BText varpnt, BText buffer, BInt4 elm_uindex[][3], VsElmData p4)

{
    ...
    for (l_uindex[4][0] = l_uindex[4][1] = elm_uindex[4][0]; l_uindex[4][0] <= elm_uindex[4][1];
         l_uindex[4][1] = l_uindex[4][0] += elm_uindex[4][2])
    {
        offs[4] = (l_uindex[4][0] - 1) * offset[4];

        for (l_uindex[3][0] = l_uindex[3][1] = elm_uindex[3][0]; l_uindex[3][0] <= elm_uindex[3][1];
             l_uindex[3][1] = l_uindex[3][0] += elm_uindex[3][2])
        {
            offs[3] = offs[4] + (l_uindex[3][0] - 1) * offset[3];

            for (l_uindex[2][0] = l_uindex[2][1] = elm_uindex[2][0]; l_uindex[2][0] <= elm_uindex[2][1];
                 l_uindex[2][1] = l_uindex[2][0] += elm_uindex[2][2])
            {
                offs[2] = offs[3] + (l_uindex[2][0] - 1) * offset[2];

                for (l_uindex[1][0] = l_uindex[1][1] = elm_uindex[1][0]; l_uindex[1][0] <= elm_uindex[1][1];
                     l_uindex[1][1] = l_uindex[1][0] += elm_uindex[1][2])
                {
                    offs[1] = offs[2] + (l_uindex[1][0] - 1) * offset[1];

                    for (l_uindex[0][0] = l_uindex[0][1] = elm_uindex[0][0]; l_uindex[0][0] <= elm_uindex[0][1];
...
/* @@
 */
static BInt4 VR_read_variable_from_file(const BText varnam, const BText grpnam, const BText elmnam,
                                        BInt4 elm_uindex[][3], BInt4 uindex[][3], VsDefData p2, VsElmData p4,
                                        BData varpnt)
{
    BInt4 error;
    BInt4 i;
    ...
    */
    ...
    {
        nactelems *= (elm_uindex[i][1] - elm_uindex[i][0] + elm_uindex[i][2]) / elm_uindex[i][2];
    }
    ...
    for (l_uindex[4][0] = l_uindex[4][1] = uindex[4][0]; l_uindex[4][0] <= uindex[4][1];
         l_uindex[4][1] = l_uindex[4][0] += uindex[4][2])
    {
        for (l_uindex[3][0] = l_uindex[3][1] = uindex[3][0]; l_uindex[3][0] <= uindex[3][1];
             l_uindex[3][1] = l_uindex[3][0] += uindex[3][2])
        {
            for (l_uindex[2][0] = l_uindex[2][1] = uindex[2][0]; l_uindex[2][0] <= uindex[2][1];
                 l_uindex[2][1] = l_uindex[2][0] += uindex[2][2])
            {
                for (l_uindex[1][0] = l_uindex[1][1] = uindex[1][0]; l_uindex[1][0] <= uindex[1][1];
                     l_uindex[1][1] = l_uindex[1][0] += uindex[1][2])
                {
                    for (l_uindex[0][0] = l_uindex[0][1] = uindex[0][0]; l_uindex[0][0] <= uindex[0][1];
                         l_uindex[0][1] = l_uindex[0][0] += uindex[0][2])
                    {
                        /* retrieve data from one cel */
                        buflen = nbytecel;
                        error = Getelt(&neffds, grpnam, elmnam, l_uindex, usrord, &buflen, tmp);
                        if (error != 0)
                        {
                            return error;
                        }
                        ...
                        (BVoid) VR_store_data_in_buffer((BText)varpnt + count * nactelems * p4->nbytsg, (BText)tmp,
                                                        elm_uindex, p4);
                        count++;
                    }
                }
    ...
    p = VR_make_variable_struct(varnam, grpnam, elmnam, elm_uindex, uindex, varpnt, p2, p4);
...
/*  @@
 */
static BInt4 VR_is_element_in_group(const BText elmnam, const BText grpnam, VsDefData* p2)

{
    BInt4 error = -1;
    VsGrpData p1;

    if ((p1 = GR_find_group_in_chain(SM_get_group_pointer(), grpnam)) != NULL)
    {
        VsCelData p3;
        BInt4 i = 0;

        *p2 = DF_find_definition_in_tree(SM_get_definition_pointer(), p1->grpdef);
        p3 = CL_find_cell_in_tree(SM_get_cell_pointer(), (*p2)->celnam);

        while (error != 0 && i < p3->nelems && (error = GEN_string_compare(elmnam, p3->elmnms[i++])) != 0)
        {
        }
    }
    return error;
...
/*  @@
 */
static BInt4 VR_calc_number_of_variables(BInt4 grpndm, BInt4 elmndm, BInt4 elm_uindex[][3], BInt4 uindex[][3])

{
    ...
    {
        nvariabs *= (elm_uindex[i][1] - elm_uindex[i][0] + elm_uindex[i][2]) / elm_uindex[i][2];
    }

    for (i = 0; i < grpndm; i++)
    {
        nvariabs *= (uindex[i][1] - uindex[i][0] + uindex[i][2]) / uindex[i][2];
    }
    return nvariabs;
}
...
/*  @@
 */
static BInt4 VR_adjust_suppl_grp_indices(VsGrpData p1, VsDefData p2, BInt4 uindex[][3])
{
    BInt4 i;
...
/*  @@
 */
static BInt4 VR_adjust_suppl_elm_indices(VsElmData p4, BInt4 elm_uindex[][3])
{
    BInt4 i;
...
/*  @@
 */
BVoid PR_print_variable_info(FILE* tfp, VsVarData p)
...
/*  @@
 */
BInt4 VR_read_var_from_file(const BText varnam, const BText elmnam, const BText grpnam, BInt4 elm_uindex[][3],
                            BInt4 uindex[][3])

{
    ...
    if (VR_is_element_in_group(elmnam, grpnam, &p2) == 0)
    {
        ...
        p1 = GR_find_group_in_chain(SM_get_group_pointer(), grpnam);

        p4 = EL_find_element_in_tree(SM_get_element_pointer(), elmnam);
        ...
        nbytes = VR_calc_number_of_variables(p2->grpndm, p4->elmndm, elm_uindex, uindex) * p4->nbytsg;
        varpnt = VR__alloc_memory_for_data(nbytes);

        if (varpnt != NULL)
        {
            if ((error = VR_read_variable_from_file(varnam, grpnam, elmnam, elm_uindex, uindex, p2, p4, varpnt)) == 0)
            {
                GEN_mallinfo();
            }
...
/*  @@
 */
BVoid VR_release_variable(const BText varname) { varinf = VR_free_variable(varinf, varname); }
...
/* @@
 */
BInt4 VR_chck_existnce_of_variables(BText varnam[])
{
    if (varnam != NULL)
    {
        BInt4 i = 0;
...
VsVarData VR_get_pointer_to_variable(const BText varnam) { return VR_find_var_in_chain(varinf, varnam); }
