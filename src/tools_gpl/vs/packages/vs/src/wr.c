...
#define PRINTSIMPL(typeid, type)                                                          \
    static BVoid print##typeid(const BText format, const VsVarData p, const BInt4 offset) \
    {                                                                                     \
...
#define PRINTCOMPL(typeid, type)                                                                        \
    static BVoid print##typeid(const BText format, const VsVarData p, const BInt4 offset)               \
    {                                                                                                   \
        GEN_print(Fg_file, format, ((type*)p->varpnt)[offset * 2], ((type*)p->varpnt)[offset * 2 + 1]); \
    }
...
#define PRINTCHAR(typeid, type)                                                           \
    ...
    {                                                                                     \
        ...
        GEN_print(Fg_file, form, (type*)p->varpnt + offset * p->nbytsg);                  \
    }
...
/* Fg_prfunc is an aray with pointers to the printfunctions,
   generated above
*/
static BVoid (*Fg_prfunc[])(const BText, const VsVarData, const BInt4) = {
    printDBLCMPL, printCOMPLEX, printDOUBLE, printFLOAT, printINT, printSHORT, printBOOL, printBOOLSHRT, printCHAR};
...
static BText Fg_format[] = {
    "(% 13.6e,% 13.6e) ", "(% 13.6e,% 13.6e) ", "% 13.6e ", "% 13.6e ", "% 13ld ", "% 13d ", "% 13ld ", "% 13d ", ""};

/*  @@
 */
static BVoid WR_two_dim_write(VsVarData p, BInt4 offset, BInt4 type, BInt4 nrow, BInt4 ncol)
{
    BInt4 row;
    BInt4 col;
    BInt4 fc;
...
/*  @@
 */
static BVoid WR_tre_dim_write(VsVarData p, NfDtp type, BInt4 nrow, BInt4 ncol, BInt4 nplane)
{
    BInt4 plane;
...
/*  @@
 */
static BVoid WR_make_write_dimensions(VsVarData p, BInt2* ndim, BInt4* nrow, BInt4* ncol, BInt4* npla)
{
...
static BVoid WR_write_multi_variables(BText varnames[])
{
    ...
    for (fc = 0; fc < nvar; fc += Fg_ncp)
    {
        ...
        for (row = 0; row < nitems[0]; row++)
        {
            ...
            for (col = fc; col < min(nvar, fc + Fg_ncp); col++)
            {
                (*Fg_prfunc[type[col]])(Fg_format[type[col]], p[col], row);
            }
            GEN_print(Fg_file, "\n");
        }
    }
 ...
 */
BVoid WR_write_variables(BText filename,  /* file to write to, may be NULL */
