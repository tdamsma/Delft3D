...
static BText EX_make_command_string(BText path, BText parms[])
{
...
/*  @@
 */
BVoid EX_process(BText process, BInt4 retflag) { EX_process_with_vars(process, NULL, retflag); }

BVoid EX_process_with_parms(BText path, BText parms[], BInt4 retflag)

{
    EX_process_with_vars(EX_make_command_string(path, parms), NULL, retflag);
}

BVoid EX_write_variables_to_pipe(FILE* pipe, BText varname[])
{
    if (varname != NULL)
    {
        VsVarData p;
...
BVoid EX_process_with_vars(BText path, BText varnames[], BInt4 retflag)
{
    FILE* pipe;
...
BVoid EX_process_with_vars_and_parms(BText path, BText varnames[], BText parnms[], BInt4 retflag)
{
    EX_process_with_vars(EX_make_command_string(path, parnms), varnames, retflag);
}
