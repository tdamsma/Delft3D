 ...
 */
VsGrpData SM_get_group_pointer(BVoid) { return grpinf; }

/*  @@  return pointer to begin of definition-tree
 */
VsDefData SM_get_definition_pointer(BVoid) { return definf; }

/*  @@  return pointer to begin of cell-tree
 */
VsCelData SM_get_cell_pointer(BVoid) { return celinf; }

/*  @@  return pointer to begin of group-tree
 */
VsElmData SM_get_element_pointer(BVoid) { return elminf; }
 ...
 */
static BVoid SM_read_element_info(BInt4 neffds, VsElmData p)
{
    BInt4 error = -1;
    ...
    if (p != NULL)
    {
        ...
        error =
            Inqelm(&neffds, p->elmnam, p->elmtyp, &p->nbytsg, p->elmqty, p->elmunt, p->elmdes, &p->elmndm, p->elmdms);
...
/*  @@ Read info of cell with name p->celnam from def-file and
       place values in structure pointed at by p.
 */
static BVoid SM_read_cell_info(BInt4 neffds, VsCelData p)
{
    if (p != NULL)
    {
        BInt4 error = -1;
...
static int SM_get_groupdimensions(BInt4 neffds, const BText grpnam, BText grpdef, BInt4 grpdms[])
{
...
/*  @@ Read info of groupdef with name p->grpdef from def-file and
       place values in structure pointed at by p.
 */
static BVoid SM_read_definition_info(BInt4 neffds, const BText grpnam, VsDefData p)
{
    if (p != NULL)
    {
        BInt4 error = -1;
        ...
        error = Inqgrp(&neffds, p->grpdef, p->celnam, &p->grpndm, grpdms, p->grpord);
 ...
 */
static BVoid SM_get_groupattribs_from_file(BInt4 neffds, const BText grpnam, VsAtrData p /* pointer to structure */
)
{
...
BVoid SM_read_nefis_meta_data(BInt4 neffds)
{
    ...
    while (error == 0)
    {
        ...
        if (SM_get_groupdimensions(neffds, grpnam, grpdef, grpdms) == 0)
        {
            VsAtr att;
            ...
            grpinf = GR_add_group_to_chain(grpinf, grpnam, grpdef, grpdms, &att);
 ...
 */
BVoid PR_print_group_info(FILE* outfile, VsGrpData p)
{
    BInt4 i = 0;
    BInt4 j = 0;
    VsDefData p_def;
    ...
    if (p != NULL)
    {
        ...
        GEN_print(tfp, "Groupname:%s Dimensions:(", p->grpnam);
        ...
        if (p->nstrat + p->nreaat + p->nintat)
        {
            ...
            {
                GEN_print(tfp, "  %24s=%14ld\n", p->IntAttrName[i], p->IntAttrValue[i]);
            }

            for (i = 0; i < p->nreaat; i++)
            {
                GEN_print(tfp, "  %24s=%14.7f\n", p->RealAttrName[i], p->RealAttrValue[i]);
            }

            for (i = 0; i < p->nstrat; i++)
            {
                GEN_print(tfp, "  %24s=%s\n", p->StringAttrName[i], p->StringAttrValue[i]);
            }
        }
        else
        {
        ...
        {
            p_elm = EL_find_element_in_tree(elminf, p_cel->elmnms[i]);
            GEN_print(tfp, "    %s %s*%3ld %s %s ", p_cel->elmnms[i], p_elm->elmtyp, p_elm->nbytsg, p_elm->elmqty,
                      p_elm->elmunt);
            GEN_print(tfp, "( ");
