...
void main(int argc, char* argv[])
{
    ...
    char* Description[] = {"q1_description", "q2_description", "q2_1_description", "q2_2_description",
                           "q2_3_description"};
    ...
    n = RobQuantityAddQuantity((const char*)Quantity[1], (const char*)Quantity[2], (const char*)Description[2],
                               (const char*)Unit[1]);
    n = RobQuantityAddQuantity((const char*)Quantity[1], (const char*)Quantity[3], (const char*)Description[3],
                               (const char*)Unit[2]);
    n = RobQuantityAddQuantity((const char*)Quantity[1], (const char*)Quantity[4], (const char*)Description[4],
                               (const char*)Unit[2]);
    ...
    for (j = 2; j < 4; j++)
    {
        ...
        fprintf(fp, "RobPutDouble : Quantityset %s Quantity %s  ElementID %s PROVIDING ", Quantity[1], Quantity[j],
                TestSetID);
        for (m = 0; m < n; m++)
        {
            fprintf(fp, "%6.1f  ", (float)pD0[m]);
        }
        fprintf(fp, "\n");

        m = RobPutDouble(ModelID, Quantity[1], Quantity[j], TestSetID, ACCEPTING, n, pD1);
        fprintf(fp, "RobPutDouble : Quantityset %s Quantity %s  ElementID %s ACCEPTING ", Quantity[1], Quantity[j],
                TestSetID);
        for (m = 0; m < n; m++)
        {
            fprintf(fp, "%6.1f  ", (float)pD1[m]);
        }
        fprintf(fp, "\n");
    }
    fflush(fp);
    ...
    for (j = 2; j < 4; j++)
    {
        ...
        fprintf(fp, "RobGetDouble Quantityset %s Quantity %s  ElementID %s PROVIDING ", Quantity[1], Quantity[j],
                TestSetID);
        for (m = 0; m < n; m++)
        {
            fprintf(fp, "%6.1f  ", (float)pD0[m]);
        ...
        fprintf(fp, "RobGetDouble : Quantityset %s Quantity %s  ElementID %s ACCEPTING ", Quantity[1], Quantity[j],
                TestSetID);
        for (m = 0; m < n; m++)
        {
            fprintf(fp, "%6.1f  ", (float)pD1[m]);
