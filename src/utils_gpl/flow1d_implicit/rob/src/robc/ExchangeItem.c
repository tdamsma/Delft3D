...
static RobExchangeitemPntr freeExchangeitem(RobExchangeitemPntr p, const char* modelID, const char* quantityID,
                                            const char* elementsetID)
{
...
void RobFreeExchangeitems(void)
{
    ...
    {
        RobExchangeitems =
            freeExchangeitem(RobExchangeitems, RobExchangeitems->pElementset->pModel->modelID,
                             RobExchangeitems->pQuantity->quantityID, RobExchangeitems->pElementset->elementsetID);
    }
}
        ...
        while (p)
        {
            ...
            for (i = 0, qIndex = 0; i < p->nElements; i++)
            {
                ...
                if (quantityCount > 0)
                {
                    ...
                    if (i % (dumpRatio) == 0) sprintf(subQuantID, "\t(%s)", quantIDs[qIndex++]);
                }
            ...
            if (quantityCount > 0) RobFree(quantIDs);
...
static RobExchangeitemPntr findExchangeitem(RobExchangeitemPntr p, const char* modelID, const char* quantityID,
                                            const char* elementsetID, ROLE role)
{
    ...
    fflush(logFileHandle);
        ...
        if ((RobStringCompare(modelID, p->pElementset->pModel->modelID) == 0) &&
            (RobStringCompare(quantityID, p->pQuantity->quantityID) == 0) &&
            (RobStringCompare(elementsetID, p->pElementset->elementsetID) == 0) && role == p->role)
        {
#if 0
...
RobExchangeitemPntr RobExchangeitemFind(const char* modelID, const char* quantityID, const char* elementsetID,
                                        ROLE role)
{
#if 0
...
int APIENTRY RobExchangeitemGetInfo(const char* modelID, int index, char* quantityID, char* elementsetID, ROLE* role)
{
    ...
    sprintf(Message.Description, "exchangeitem of quantity %s and elementset %s not found", quantityID, elementsetID);
    Message.Code = -1;
    return -1;
}
...
void robExchangeitemResize(RobExchangeitemPntr p)
{
    ...
    if (NumberOfQuantities > 0)
    {
        ...
        if (nSize != p->nElements)
        {
            ...
            for (n = p->nElements; n < nSize; n++)
            {
                p->values[n] = MISSING_VALUE;
            }
            p->nElements = nSize;
        }
    }
