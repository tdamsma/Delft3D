...
double APIENTRY RobMissingValueGetValue(void) { return MISSING_VALUE; }
/// <summary>
...
int APIENTRY RobPutInt(const char* modelId, const char* quantitySetID, const char* quantityID, const char* elementSetID,
                       ROLE role, int nElements, int* iValues)
{
    ...
    else
    {
        ...
        sprintf(Message.Description, "Exchange item: quantity %s, Elementset %s not found", quantityID, elementSetID);
        Message.Code = -1;
        result = -1;
    }
...
/// <param name="dValues">elements vector</param>
/// <returns>return value</returns>
int APIENTRY RobPutReal(const char* modelId, const char* quantitySetID, const char* quantityID,
                        const char* elementSetID, ROLE role, int nElements, float* fValues)
{
    ...
    if (pEi != NULL && Offset >= 0)
    {
        ...
        {
            sprintf(Message.Description, "Number of elements is 0 for exchange item %s / %s", quantityID, elementSetID);
            Message.Code = -2;
            result = -2;
        }
    }
    ...
    {
        // Exchangeitem not found
        sprintf(Message.Description, "Exchange item %s / %s not found", quantityID, elementSetID);
        Message.Code = -1;
        result = -1;
    }
...
/// <param name="dValues">elements vector</param>
/// <returns>return value</returns>
int APIENTRY RobPutDouble(const char* modelId, const char* quantitySetID, const char* quantityID,
                          const char* elementSetID, ROLE role, int nElements, double* dValues)
{
    ...
    if (logFileHandle != NULL)
    {
        fprintf(logFileHandle, "%s calls RobPutDouble for %s %s\t(es:%s)\n",
                (role == PROVIDING) ? "Component" : "Wrapper", (IsSet) ? quantitySetID : "", quantityID, elementSetID);
    ...
    if (IsSet)
    {
        if ((pEi = RobExchangeitemFind(modelId, quantitySetID, elementSetID, role)) != NULL)
        {
            ...
            if (i == 0)
            {
                ...
                {
                    sprintf(Message.Description, "number of quantities in ElementSet %s / %s is zero", quantitySetID,
                            elementSetID);
                    Message.Code = -3;
                    return Message.Code;
                }
    ...
    if (pEi != NULL && Offset >= 0)
    {
        if (pEi->nElements > 0)
        {
            if (logFileHandle != NULL)
            {
                fprintf(logFileHandle, "\t%s put value(s) for %s %s\t(es:%s)\n",
                        (role == PROVIDING) ? "Component" : "Wrapper", (IsSet) ? quantitySetID : "", quantityID,
                        elementSetID);
                fflush(logFileHandle);
            }
            for (i = 0; i < pEi->nElements / nQuantity; i++)
        ...
        {
            sprintf(Message.Description, "number of elements in Exchange item %s / %s is zero", quantityID,
                    elementSetID);
            Message.Code = -2;
            result = -2;
        }
...
/// <param name="dValues">elements vector</param>
/// <returns>return value</returns>
int APIENTRY RobGetDouble(const char* modelId, const char* quantitySetID, const char* quantityID,
                          const char* elementSetID, ROLE role, int nElements, double* values)
{
    ...
    if (logFileHandle != NULL)
    {
        fprintf(logFileHandle, "%s calls RobGetDouble for %s %s\t(es:%s)\n",
                (role == ACCEPTING) ? "Component" : "Wrapper", (IsSet) ? quantitySetID : "", quantityID, elementSetID);
        fflush(logFileHandle);
    }
#endif
                ...
                if (logFileHandle != NULL)
                {
                    ...
                    if (valuesAvailable)
                    {
                        fprintf(logFileHandle, "\t%s got value(s) for %s %s\t(es:%s)\n",
                                (role == ACCEPTING) ? "Component" : "Wrapper", (IsSet) ? quantitySetID : "", quantityID,
                                elementSetID);
                        fflush(logFileHandle);
                    }
                }
...
void APIENTRY RobGetLastError(int Code, char* Description, int* Type)
{
    ...
    {
        sprintf(Description, "Internal error inconsistent codes, last reported error : %s", Message.Description);
    }
}
