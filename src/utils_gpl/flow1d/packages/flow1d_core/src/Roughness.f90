...
!
   ...
   type, public :: t_Roughness !< Derived type for roughness data
      ...
      real(kind=dp) :: frictionValue !< Global friction Value
      ...
      real(kind=dp), allocatable :: currentValues(:) !< Time Interpolated Friction values of time dependent items (same index as timeSeriesIds).
      real(kind=dp), allocatable :: timeDepValues(:, :) !< Friction values of time dependent items (same index as timeSeriesIds).
   ...
   real(kind=dp), parameter, public :: sixth = 1.0_dp / 6.0_dp !< 1/6
   real(kind=dp), parameter, public :: third = 1.0_dp / 3.0_dp !< 1/3
   real(kind=dp), parameter, public :: chlim = 0.001_dp !< Lowest Chezy value
...
!> Get the Chezy value for a given friction type and parameter value
   real(kind=dp) function GetChezy(frictType, cpar, rad, dep, u)
      ...
      real(kind=dp), intent(in) :: cpar !< parameter value
      real(kind=dp), intent(in) :: u !< velocity
      ...
      case (2)
         ...
         z0 = min(cpar / 30.0_dp, rad0 * 0.3_dp)
         sqcf = vonkar / log(rad0 / (ee * z0))
         getChezy = sag / sqcf
      case (3)
         !           White Colebrook WAQUA / Nikuradse-formula
         GetChezy = 18.0_dp * log10(12.0_dp * rad0 / cpar)
      case (7)
         !           Strickler-1 formula
         GetChezy = 25.0_dp * (rad0 / cpar)**sixth
      case (8)
         !           Strickler-2 formula
...
!> Retrieve the friction parameter for given branchIndex/chainage
   subroutine getFrictionParameters(rgh, ibranch, chainage, c_type, c_par)
      ...
      real(kind=dp), intent(in) :: chainage !< chainage (location on branch)
      integer, intent(out) :: c_type !< friction type
      real(kind=dp), intent(out) :: c_par !< friction parameter value
      ...
      if (rgh%useGlobalFriction) then
         ...
         if (fun_type(ibranch) == R_FunctionTimeSeries) then
            ...
            if (rgh%timeDepValues(timeseries_index, 2) > 0.0_dp) then
               ! The values are set
