   ...
   abstract interface
      subroutine compute_gs(genstr, direction, L0, maxWidth, bob0, fuL, ruL, auL, as1, as2, structwidth, s1m1, s1m2, &
...
!-------------------------------------------------------------------------------
   ...
   public compute_general_structure_fullwidth
   public dealloc
...
contains
   ...
   subroutine compute_general_structure(genstr, direction, L0, maxWidth, bob0, fuL, ruL, auL, as1, as2, structwidth, s1m1, s1m2, &
      ...
      if (genstr%au_max(l0) > 0.0_dp .and. gatefraction > 0.0_dp .and. gatefraction < 1.0_dp) then  ! Only width correc
      ...
      if (gatefraction > GATE_FRACTION_EPS) then
         ...
         au(1) = au(1)*gatefraction/width_correction_factor
         ...
         ! The flow area is now based ont the corrected structure width. Set au now to the correct value:
         au(2) = au(2)*gatefraction/width_correction_factor
      else
         fu(1) = 0.0_dp
         ru(1) = 0.0_dp
         fu(2) = 0.0_dp
      ...
      genstr%ru(:, L0) = ru
      genstr%au(:, L0) = au
      genstr%au_max(L0) = wstr * genstr%sOnCrest(L0)
      !TEMP = laatste statement

   end subroutine compute_general_structure

      subroutine compute_general_structure_fullwidth(genstr, direction, L0, maxWidth, bob0, fuL, ruL, auL, as1, as2, structwidth, s1m1, s1m2, &
                                      qtotal, Cz, dxL, dt, SkipDimensionChecks)
      ! modules
      ...
      if (gatefraction< 1d0 - GATE_FRACTION_EPS) then
         ! calculate flow asif no door is present
         dg = huge(1d0)
         u1L = ru(3) - fu(3)*dsL 
   ...
   end subroutine compute_general_structure_fullwidth
