...
! locals
      ...
      if (convtab%last_position == 0) then
         ...
         if (dpt <= convtab%height(i2)) then !  .and. convtab%jopen .eq. 0) then     ! weightfactors. If profile closed no extrapolation
...
!(*)!  document SOBEK-21942: Change of roughness formulations in "Y-Z" and
            ...
            !
            if (convtab%jopen == 1) then ! for open profiles add extra conveyance
               AR2 = WIDTH * (DPT - HU2)
               AREA = AREA + AR2
         ...
         if (i2 == nr .and. dpt >= convtab%height(i2)) then ! Weightfactors. If profile closed no extrapolation
            a1 = 0d0; dh1 = 0
         else
